...
| 🔵  | **P2 #34 (6/7+)** — FocusTrap wired into HyperlinkDialog + PageSetupDialog + ImagePositionDialog + ImagePropertiesDialog + SplitCellDialog + TablePropertiesDialog. FocusTrap's wrapper uses `display: contents` so it doesn't break parent flex centering. Remaining aria-modal dialogs (KeyboardShortcuts / PasteSpecial / InsertTable / InsertImage / InsertSymbol / FileProperties / CommandPalette) are a follow-up sweep on the same pattern. | `FocusTrap.tsx`, 6 dialogs                                                                       |
