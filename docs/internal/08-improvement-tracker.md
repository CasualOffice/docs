...
| 🔵  | **P2 #34 (2/7+)** — FocusTrap wired into HyperlinkDialog + PageSetupDialog. FocusTrap's wrapper switched to `display: contents` so it doesn't break parent flex centering. Other 8+ aria-modal dialogs are a follow-up sweep on the same pattern.                                          | `FocusTrap.tsx`, `HyperlinkDialog.tsx`, `PageSetupDialog.tsx`                                    |
