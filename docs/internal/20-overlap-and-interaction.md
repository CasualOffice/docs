...
|---|-----|------|-----------------------|--------|
| B1 | **Header logo image overlaps the "Powered by:" text** (medical-incident-form p1) | R | Reference stacks "Powered by:" *above* the Safetymint logo; our render places the anchored image ~one line too high so it collides with the text. Anchored-image vertical offset in the header path (`headerFooterLayout.ts` / `renderPage.ts` header image branch). | verified |
