...
| P2 | ⬜ **Complex header/footer layout breaks in edit mode** (user-reported) | Med·High | **Diagnosed.** The display path (layout-painter) draws headers with absolute-positioned text boxes, multi-column tables, and logo images correctly. The EDIT overlay (`InlineHeaderFooterEditor` → `headerFooterToProseDoc` → raw ProseMirror `EditorView`, class `hf-editor-pm`) renders the same content in **linear PM flow** — so on a letterhead header the positioned boxes collapse into a vertical stack, table columns break apart, and the logo image is dropped. **Not CJK-specific** (surfaced on a zh doc but it's any complex header). Text editing itself works; only layout fidelity in edit mode is wrong. **Fix is a dedicated effort:** make the editable overlay reproduce the painted positioned layout (floating boxes / table columns / images), which fights PM's flow model — or a pragmatic interim that at least preserves the logo + table structure. Repro: `sds-anti-t-zh.docx` (double-click the header). |
