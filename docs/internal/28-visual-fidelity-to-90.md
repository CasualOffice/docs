...
## Dead end: trHeight + cell padding (TRIED, REVERTED — 2026-06-27)

Hypothesis: Word/LibreOffice read `w:trHeight` (atLeast/auto) as the minimum **content-box** height and layer the cell's top+bottom insets ON TOP (`row = max(content, trHeight) + padding`), rather than ECMA-376 §17.4.81's reading of trHeight as the TOTAL row height (`row = max(content+padding, trHeight)`). On `medical-incident-form` it looked like a clean win — its field rows are `trHeight=737` (49px) with `tblCellMar` 57/57 twips, and the change collapsed band drift from **-88px to +2px**, score **33.6 → 39.4**, with the representative corpus unchanged.

**But the controlled corpus diff killed it.** Reverting just the `PagedEditor.tsx` hunk and re-measuring the table fixtures showed the rule does NOT generalize: `generic-render-regression` **71.2 → 66.4 (-4.8)** — a significant hit on a canonical regression-guard fixture — and `Form025U` 58.1 → 57.3. Net across table fixtures ≈ **+0.2**: the change just redistributes score (helps one form, hurts another). LibreOffice does not universally add insets atop trHeight; the medical-form gain is structure-specific. **Reverted** the row-height change (`PagedEditor.tsx`), kept only the sibling connector fix.

**Why it matters going forward:** the medical-form row drift is REAL (its `trHeight` rows genuinely render ~5px taller in LibreOffice), but a blanket rule is wrong — `generic-render-regression` diverges under the identical change. A correct fix must first explain WHY the two fixtures behave oppositely (hRule value? content-vs-min margin? specific tblCellMar vs tcMar source?) before touching table row sizing. **Lesson reinforced:** always run the controlled before/after across the whole corpus, never trust a single target fixture.

**Remaining on the form:** band 0 (title + grouped-logo header) is ~26px taller in the editor (grouped `wpg` drawing / `w:line=360` 1.5× title spacing — the known weak spot). The connector gray-bar artifact IS fixed (shipped).
