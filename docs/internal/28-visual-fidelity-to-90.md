...
## Form row-height fix — trHeight + cell padding (2026-06-27, cont.)

Cracked the medical-incident-form drift (the one that band-compare flagged as ~56px-CSS cumulative compression). **Root cause:** the editor read `w:trHeight` (atLeast/auto) as the TOTAL row height per ECMA-376 §17.4.81 — `row = max(content+padding, trHeight)`. But LibreOffice (and, by the form author's design, Word) render the value as the **minimum content-box height and layer the cell's top+bottom insets on top** — `row = max(content, trHeight) + padding`. The form's field rows are `trHeight=737` (49px CSS) with `tblCellMar` top/bottom = 57 twips each; the editor rendered them at exactly 49px while LibreOffice rendered ~54px, so every field row was ~5px short and the page compressed upward.

**Fix** (`PagedEditor.tsx` row-height calc): when the authored minimum dominates, add the row's max cell vertical padding on top of it. **Surgical:** `DEFAULT_CELL_PADDING_Y = 0`, so this is a no-op for tables without explicit cell margins — only `tblCellMar`/`tcMar`-bearing tables change. **Result:** band drift collapsed from **-26/-47/-88px** (bands 4/6/9) to **-12/-8/+2** — field rows now align with LibreOffice almost exactly. Score `medical-incident-form` **33.6 → 39.4**; representative corpus unchanged (syllabus 91.4, lab 85.1); Form025U -0.8 (hard fixture, page count held); `tableMeasure` unit tests pass. Regression test: `e2e/tests/connector-line-height.spec.ts` covers the sibling connector fix.

**Spec note:** this matches the project's reference renderer (LibreOffice) and the visible Word design intent, at the cost of a strict ECMA-376 §17.4.81 reading (trHeight as total). Revisit if a Word-authored doc regresses.

**Remaining on the form:** band 0 (the title + grouped-logo header) is still ~26px taller in the editor than LibreOffice — the grouped `wpg` drawing / title-spacing, a separate known-weak-spot item. The field-row body now tracks the reference.
