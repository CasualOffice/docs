...
export class ScenarioRunner {
	...
	private async executeAction(step: ActionStep): Promise<void> {
		...
		switch (action) {
			...
			case "typeTextSlowly":
				await this.editor.typeTextSlowly(
					args.text as string,
	...
	private async executeAssertion(step: AssertionStep): Promise<void> {
		...
		switch (assert) {
			...
			case "expectDocumentContains":
				await assertions.assertDocumentContainsText(
					this.page,
					args.text as string,
				);
				break;
			...
			case "expectDocumentNotContains":
				await assertions.assertDocumentNotContainsText(
					this.page,
					args.text as string,
				);
				break;
			...
			case "expectTextStrikethrough":
				await assertions.assertTextHasStrikethrough(
					this.page,
					args.text as string,
				);
				break;
			...
			case "expectTextColor":
				await assertions.assertTextHasColor(
					this.page,
					args.text as string,
			...
			case "expectSelectedText": {
				...
				if (selected !== (args.text as string)) {
					throw new Error(
						`Expected selected text "${args.text}", got "${selected}"`,
					);
				}
				break;
			}
			...
			case "expectVisualMatch":
				await assertions.assertVisualMatch(
					this.page,
					args.screenshotName as string,
					{
	...
	static filterScenarios(
		...
		return scenarios.filter((scenario) => {
			...
			if (filter.categories && filter.categories.length > 0) {
				if (
					!scenario.category ||
					!filter.categories.includes(scenario.category)
				) {
					return false;
				}
			}
			...
			if (filter.tags && filter.tags.length > 0) {
				if (
					!scenario.tags ||
					!filter.tags.some((tag) => scenario.tags?.includes(tag))
				) {
					return false;
				}
			}
			...
			if (filter.excludeTags && filter.excludeTags.length > 0) {
				if (
					scenario.tags &&
					filter.excludeTags.some((tag) => scenario.tags?.includes(tag))
				) {
					return false;
				}
			}
