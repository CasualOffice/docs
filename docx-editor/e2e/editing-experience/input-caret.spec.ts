...
test.describe("Input & caret contract", () => {
	...
	test("Shift+Arrow extends a selection that typing replaces", async ({
		page,
	}) => {
	...
	test("Ctrl/Cmd+Home and +End jump to document start and end", async ({
		page,
	}) => {
	...
	test("IME composition: start/update/end commits the composed text", async ({
		page,
	}) => {
		...
		const committed = await page.evaluate(async () => {
			...
			pm.dispatchEvent(
				new CompositionEvent("compositionend", { bubbles: true, data: "にほ" }),
			);
			return { ok: true as const };
		});

		if ("error" in committed)
			test.skip(true, "hidden PM not present in this harness");
