...
test.describe("Round-trip through edit", () => {
	...
	test("letter fixture: appended text survives save → reload", async ({
		page,
	}) => {
	...
	test("letter fixture: a formatting edit survives save → reload", async ({
		page,
	}) => {
		await ed.loadDocxFile(LETTER);
		await page.waitForTimeout(400);
	...
	test("table fixture: an edited cell survives save → reload", async ({
		page,
	}) => {
