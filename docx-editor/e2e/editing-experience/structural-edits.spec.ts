...
test.describe("Structural edits", () => {
	...
	test("insert an image, then resize it via a corner handle", async ({
		page,
	}) => {
		...
		if ((await handle.count()) === 0) {
			test.skip(
				true,
				"no resize handle exposed for this image kind in this harness",
			);
			return;
		}
	...
	test("insert a table, edit a cell, add a row and a column", async ({
		page,
	}) => {
	...
	test("insert an equation via Alt+= renders a math node", async ({ page }) => {
		...
		await expect(
			page.locator(".paged-editor__pages math mfrac").first(),
		).toBeVisible();
	});
