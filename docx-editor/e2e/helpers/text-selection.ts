		...
		const findParagraphIndex = (node: Node | null): number | null => {
			...
			while (current) {
				if (
					...
					return parseInt(
						current.getAttribute("data-paragraph-index") || "0",
						10,
					);
				}
 ...
 */
export async function selectTextBySearch(
	page: Page,
	searchText: string,
): Promise<boolean> {
...
export async function selectAcrossParagraphs(
	...
	return await page.evaluate(
		({ startP, startOff, endP, endOff }) => {
			const startParagraphEl = document.querySelector(
				`[data-paragraph-index="${startP}"]`,
			);
			const endParagraphEl = document.querySelector(
				`[data-paragraph-index="${endP}"]`,
			);
 ...
 */
export async function moveCursorToStart(
	page: Page,
	paragraphIndex: number,
): Promise<boolean> {
	return await page.evaluate((pIndex) => {
 ...
 */
export async function moveCursorToEnd(
	page: Page,
	paragraphIndex: number,
): Promise<boolean> {
	return await page.evaluate((pIndex) => {
...
export async function extendSelectionByWord(
	...
	await page.keyboard.press(
		`Shift+${modifier}+Arrow${direction === "left" ? "Left" : "Right"}`,
	);
}
...
export async function extendSelectionByCharacter(
	...
	for (let i = 0; i < count; i++) {
		await page.keyboard.press(
			`Shift+Arrow${direction === "left" ? "Left" : "Right"}`,
		);
	}
}
...
export async function extendSelectionByLine(
	...
	for (let i = 0; i < count; i++) {
		await page.keyboard.press(
			`Shift+Arrow${direction === "up" ? "Up" : "Down"}`,
		);
	}
}
