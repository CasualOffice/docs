...
test.describe("Agent bridge — formatting + paged reads", () => {
					...
					page.evaluate(() => {
						const runs = Array.from(
							document.querySelectorAll(
								'.paged-editor__pages [style*="font-weight"]',
							),
						) as HTMLElement[];
						return runs.some(
							(el) =>
	...
	test("applyFormatting returns false for an unknown paraId", async ({
		page,
	}) => {
		const ok = await page.evaluate(
			() =>
	...
	test("setParagraphStyle applies a known styleId to the first paragraph", async ({
		page,
	}) => {
		const paraId = await getFirstParaId(page);
		test.skip(!paraId, "Fixture has no paraIds");
		...
		const styleId = await page.evaluate(() => {
			const styles = (
				window.__DOCX_EDITOR_E2E__ as unknown as {
					getDocument?: () => {
						package?: {
							styles?: { styles?: Array<{ id: string; type: string }> };
						};
					};
				}
	...
	test("getPageContent returns paragraphs on page 1 with stable paraIds", async ({
		page,
	}) => {
		const paraId = await getFirstParaId(page);
		test.skip(!paraId, "Fixture has no paraIds");
	...
	test("getPageContent returns null for an out-of-range page", async ({
		page,
	}) => {
		const result = await page.evaluate(
