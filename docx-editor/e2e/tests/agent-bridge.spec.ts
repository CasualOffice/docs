...
async function pickUniqueMatch(page: import("@playwright/test").Page): Promise<{
	paraId: string;
	match: string;
	before: string;
	after: string;
...
test.describe("Agent bridge — paraId-anchored mutations", () => {
	...
	test("addComment via agent bridge anchors a comment on the right paraId", async ({
		page,
	}) => {
		...
		await expect(
			page.getByText("Agent says: review this paragraph."),
		).toBeVisible({
			timeout: 5000,
		});
	...
	test("programmatic addComment lays out sidebar cards without overlap (no click required)", async ({
		...
		await expect(page.locator(".docx-comment-card")).toHaveCount(1, {
			timeout: 5000,
		});
		...
		await expect(page.locator(".docx-comment-card")).toHaveCount(
			paraIds.length,
			{
				timeout: 5000,
			},
	...
	test("proposeChange creates a tracked change visible in the editor", async ({
		page,
	}) => {
		...
		await expect(
			page
				.locator(".docx-insertion")
				.filter({ hasText: "AGENT-INSERTED" })
				.first(),
		).toBeVisible({ timeout: 5000 });
		// Deletion mark renders red-strikethrough on the original text.
		await expect(
			page.locator(".docx-deletion").filter({ hasText: match }).first(),
		).toBeVisible({
			timeout: 5000,
		});
	});

	test("addComment + undo cleans the comment from the sidebar", async ({
		page,
	}) => {
		const paraId = await getFirstParaId(page);
		test.skip(!paraId, "Demo doc has no paraIds yet");
		...
		await expect(page.getByText("temp-agent-comment")).toBeVisible({
			timeout: 5000,
		});
		...
		await expect(page.getByText("temp-agent-comment")).toHaveCount(0, {
			timeout: 5000,
		});
	});

	test("proposeChange refuses to layer onto existing tracked change", async ({
		page,
	}) => {
		await getFirstParaId(page);
	...
	test("onContentChange listener fires when agent adds a comment", async ({
		page,
	}) => {
		const paraId = await getFirstParaId(page);
		test.skip(!paraId, "Demo doc has no paraIds yet");
	...
	test("findInDocument still returns the deletion text after a redline", async ({
		page,
	}) => {
		await getFirstParaId(page);
	...
	test("findInDocument does not return inserted text after a redline", async ({
		page,
	}) => {
		await getFirstParaId(page);
