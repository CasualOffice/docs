...
test.describe("paraId allocator — live document", () => {
	test("every paragraph in a freshly-loaded fixture has a paraId", async ({
		page,
	}) => {
	...
	test("typing Enter mid-paragraph allocates a new paraId for the second half", async ({
		...
		// No nulls.
		expect(after.every((id) => typeof id === "string" && id.length > 0)).toBe(
			true,
		);
		// No duplicates.
