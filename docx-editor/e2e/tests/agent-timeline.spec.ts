...
test.describe("AgentTimeline — streaming state", () => {
	...
	test("toggle button reports aria-expanded=true while streaming", async ({
		page,
	}) => {
		await gotoTimeline(page, "streaming");
...
test.describe("AgentTimeline — done state", () => {
	...
	test("clicking the summary re-expands the list", async ({ page }) => {
		...
		await expect(toggle).toHaveAttribute("aria-expanded", "true");
		await expect(
			page.getByTestId("agent-timeline").locator("ol li"),
		).toHaveCount(3);
	});

	test("done timeline shows the final assistant text bubble below", async ({
		page,
	}) => {
		await gotoTimeline(page, "done");
