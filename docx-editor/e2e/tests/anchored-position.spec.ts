...
test("a column-anchored floating image is painted at its posOffset (300px)", async ({
	page,
}) => {
...
test("textbox Format-panel Position X/Y anchors the painted box at the offset", async ({
	...
	const offset = await page.evaluate(() => {
		...
		const content = el
			...
			?.querySelector(".layout-page-content");
	...
	await expect(
		page.locator('[data-testid="properties-textbox-pos-x"]'),
	).toHaveValue("250");
