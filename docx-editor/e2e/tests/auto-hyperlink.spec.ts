...
test.describe("Auto-hyperlink", () => {
	...
	test("typing a URL followed by a space auto-links it", async ({ page }) => {
		...
		const link = page.locator(
			'.paged-editor__hidden-pm .ProseMirror a[href="http://example.com"]',
		);
	...
	test("normalises a bare www. URL to an absolute href", async ({ page }) => {
		...
		await expect(
			page.locator(
				'.paged-editor__hidden-pm .ProseMirror a[href="http://www.example.org"]',
			),
		).toHaveCount(1, { timeout: 2000 });
	});

	test("trailing sentence punctuation is left out of the link", async ({
		page,
	}) => {
	...
	test("a trailing unbalanced closing paren is left out of the link", async ({
		page,
	}) => {
	...
	test("pasting a URL over a selection links the selection (keeps the text)", async ({
		page,
	}) => {
