...
test.describe("Auto-list", () => {
	...
	test('"- " at line start creates a bullet list and consumes the marker', async ({
		page,
	}) => {
		await editor.typeText("- Buy milk");
		// A list marker is painted, and the typed "-" is gone from the content.
		await expect(page.locator(".layout-list-marker")).toHaveCount(1, {
			timeout: 2000,
		});
		const text = await page
			.locator(".paged-editor__hidden-pm .ProseMirror")
			.innerText();
	...
	test("a marker mid-paragraph does NOT trigger auto-list", async ({
		page,
	}) => {
