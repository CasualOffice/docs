	...
	await page.evaluate(async ({ name, openedAt }) => {
		...
		open.onupgradeneeded = () => {
			...
			if (!db.objectStoreNames.contains("recent-files")) {
				const os = db.createObjectStore("recent-files", {
					keyPath: "id",
					autoIncrement: true,
				});
			...
			if (!db.objectStoreNames.contains("versions")) {
				const os = db.createObjectStore("versions", {
					keyPath: "id",
					autoIncrement: true,
				});
...
test.describe("Auto-reopen banner", () => {
	test("renders when last-opened doc is fresh and reopens on click", async ({
		page,
	}) => {
		await page.goto("/");
	...
	test("hides when the last-opened doc is older than 7 days", async ({
		page,
	}) => {
		await page.goto("/");
		await page.waitForLoadState("domcontentloaded");
	...
	test("dismiss hides the banner + survives a reload via sessionStorage", async ({
		page,
	}) => {
		await page.goto("/");
		await page.waitForLoadState("domcontentloaded");
