...
async function flush(page: import("@playwright/test").Page) {
	await page.evaluate(async () => {
		...
		if (!hook)
			throw new Error("window.__autosaveE2E missing — fixture not wired");
		await hook.flush();
	});
}
...
test.describe("AutosaveStatus", () => {
	...
	test('saves PUT bytes + status transitions to "Saved just now"', async ({
		page,
	}) => {
		await mockAuthAlwaysSignedIn(page);
		let putCalled = false;
		...
		await expect(page.getByTestId("autosave-status")).toHaveAttribute(
			"data-status",
			"saved",
		);
	...
	test('status flips to "Save failed" on a 500 + recovers on next success', async ({
		page,
	}) => {
		await mockAuthAlwaysSignedIn(page);
		let failOnce = true;
