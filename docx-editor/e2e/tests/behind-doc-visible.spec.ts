...
test.describe("Behind-doc objects render above the page background", () => {
	test("page establishes a stacking context and behind-doc text is visible", async ({
		page,
	}) => {
		...
		const contained = await page.evaluate(() => {
			const els = [
				...document.querySelectorAll(".layout-page *"),
			] as HTMLElement[];
			const label = els.find(
				(e) =>
					e.children.length === 0 &&
					e.textContent?.trim().startsWith("外观") === true,
			);
			...
			return getComputedStyle(page).isolation === "isolate"
				? "contained"
				: "escapes";
		});
