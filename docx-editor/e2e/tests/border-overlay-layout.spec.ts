...
test.describe("Border overlay layout", () => {
	test("title and page borders render with Word-style inset geometry", async ({
		page,
	}) => {
		...
		const metrics = await page.evaluate(() => {
			...
			const pageBorder = pageEl?.querySelector<HTMLElement>(
				".layout-page-border",
			);
			...
			const titleBorder = titleParagraph?.querySelector<HTMLElement>(
			...
			return {
				floatingImages: pageEl.querySelectorAll(".layout-page-floating-image")
					.length,
		...
		expect(metrics!.pageBorderInsetLeft).toBeLessThan(
		...
		expect(metrics!.titleBorderWidth).toBeLessThan(
			metrics!.contentWidth * 0.55,
		);
	});
});
