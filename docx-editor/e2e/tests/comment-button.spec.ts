 ...
 */
async function findTextSpanY(
	page: import("@playwright/test").Page,
	text: string,
) {
	return page.evaluate((searchText) => {
		const spans = document.querySelectorAll(
			".paged-editor__pages span[data-pm-start]",
		);
...
test.describe("Comment Button - Scroll Position (#185)", () => {
	...
	test.beforeEach(async ({ page }) => {
		...
		await page.waitForSelector(".paged-editor__pages .layout-page", {
			timeout: 10000,
		});
	});

	test("comment button appears aligned with selection near top", async ({
		page,
	}) => {
		test.fixme(
			true,
...
test.describe("Comment Button - Geometry changes (#268 dedup)", () => {
	...
	test("floating button re-anchors to page right edge after window resize", async ({
		page,
	}) => {
		test.fixme(
			true,
	...
	test("floating button tracks page right edge across multiple resize steps", async ({
		page,
	}) => {
		test.fixme(
			true,
