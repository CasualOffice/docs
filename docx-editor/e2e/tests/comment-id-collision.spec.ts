...
test.describe("comment-id collision (issue #257)", () => {
	...
	test("without commentIdBase, IDs are small (regression baseline)", async ({
		page,
	}) => {
	...
	test("with commentIdBase=1_000_000, new comments use IDs > base", async ({
		page,
	}) => {
	...
	test("two distinct bases produce non-overlapping ID ranges (peer A vs peer B)", async ({
		browser,
	}) => {
