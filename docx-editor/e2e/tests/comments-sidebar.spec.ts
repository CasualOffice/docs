...
async function startAddComment(editor: EditorPage, searchText: string) {
	...
	await expect(
		floating,
		"floating add-comment button should mount",
	).toBeVisible({
		timeout: 3000,
	});
	await floating.click();
	await page.waitForSelector(`${SIDEBAR} textarea`, {
		state: "visible",
		timeout: 3000,
	});
}
...
function commentCard(page: Page, commentId: number) {
	return page.locator(
		`${SIDEBAR} ${COMMENT_CARD}[data-comment-id="${commentId}"]`,
	);
}

function resolvedMarker(page: Page, commentId: number) {
	return page.locator(
		`${SIDEBAR} [data-comment-id="${commentId}"][data-comment-resolved="true"]`,
	);
}
 ...
 */
async function getCommentHighlight(
	page: Page,
	commentId: number,
): Promise<string> {
...
async function getSidebarCommentIds(page: Page): Promise<number[]> {
	return page.evaluate(() => {
		const els = document.querySelectorAll(
			".docx-unified-sidebar [data-comment-id]",
		);
 ...
 */
async function isCommentExpanded(
	page: Page,
	commentId: number,
): Promise<boolean> {
	const card = commentCard(page, commentId);
	if ((await card.count()) === 0) return false;
	const actionBtn = card.locator(
		'button[title="Resolve"], button[title="Reopen"]',
	);
...
async function moveCursorTo(editor: EditorPage, cleanWord: string) {
	...
	expect(found, `moveCursorTo: word "${cleanWord}" should be in the doc`).toBe(
		true,
	);
...
test.describe("Comments sidebar — cursor-based expansion", () => {
	...
	test("only one card is expanded at a time, driven by cursor position", async ({
		page,
	}) => {
		const editor = await setupEmptyDoc(page);
		await openSidebar(page);
	...
	test("arrow-key navigation into a comment expands its card", async ({
		page,
	}) => {
		const editor = await setupEmptyDoc(page);
		await openSidebar(page);
...
test.describe("Comments sidebar — resolve / reopen (regression #268)", () => {
	...
	test("clicking the resolved checkmark expands the card", async ({ page }) => {
		...
		await editor.typeText(
			`Here is a resolvable target word in the doc ${CLEAN}.`,
		);
		...
		expect(await isCommentExpanded(page, id)).toBe(true);
		await expect(
			commentCard(page, id).locator('button[title="Reopen"]'),
		).toBeVisible();
	});
	...
	test("clicking the resolved checkmark expansion survives multiple resize frames", async ({
		...
		for (let i = 0; i < 5; i++) {
			await page.waitForTimeout(100);
			expect(
				await isCommentExpanded(page, id),
				`still expanded after ${i * 100}ms`,
			).toBe(true);
		}
	});
	...
	test("reopen converts a resolved comment back to a normal active comment", async ({
		page,
	}) => {
		const editor = await setupEmptyDoc(page);
		await openSidebar(page);
		...
		await expect(
			commentCard(page, id).locator('button[title="Resolve"]'),
		).toBeVisible();
	});

	test("resolved comments remain visible as checkmarks while sidebar is open", async ({
		page,
	}) => {
		const editor = await setupEmptyDoc(page);
		await openSidebar(page);
		await editor.typeText(
			`Alpha beta gamma delta epsilon zeta eta theta ${CLEAN}.`,
		);
	...
	test("resolved comment highlight is hidden in the doc body by default", async ({
		page,
	}) => {
		const editor = await setupEmptyDoc(page);
		await openSidebar(page);
		...
		expect(
			isYellowish(activeBg),
			`active bg "${activeBg}" should be yellowish`,
		).toBe(true);
		...
		expect(
			isYellowish(resolvedBg),
			`resolved bg "${resolvedBg}" should NOT be yellowish`,
		).toBe(false);
	});
	...
	test("expanding a resolved comment re-shows the highlight on its range only", async ({
		...
		expect(
			isYellowish(expandedBg),
			`expanded bg "${expandedBg}" should be yellowish`,
		).toBe(true);
	});

	test("clicking the grey gutter outside the page collapses an expanded card", async ({
		page,
	}) => {
		const editor = await setupEmptyDoc(page);
		await openSidebar(page);
...
test.describe("Comments sidebar — feedback-loop stability", () => {
	...
	test("expanding a card does not immediately re-collapse (state-identity dedup)", async ({
		...
		await openSidebar(page);
		await editor.typeText(
			`Stability test text with target word here ${CLEAN}.`,
		);
