...
test("grouped connector line renders thin, not as a tall filled bar", async ({
	page,
}) => {
	...
	const grayHeights = await page.evaluate(() =>
		Array.from(document.querySelectorAll(".layout-textbox"))
			.filter((e) =>
				getComputedStyle(e as HTMLElement).backgroundColor.includes(
					"217, 217, 217",
				),
			)
			.map((e) =>
				Math.round((e as HTMLElement).getBoundingClientRect().height),
			),
	);
