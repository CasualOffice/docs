...
test.describe("Insert > Convert table to text (B8 closeout)", () => {
	test("appears in the Insert menu only when the cursor is in a table", async ({
		page,
	}) => {
		...
		await expect(
			page.getByRole("menuitem", { name: /Convert table to text/ }),
		).toHaveCount(0);
		...
		await page.getByRole("button", { name: "Insert", exact: true }).click();
		await page
			.getByRole("menuitem", { name: /Convert selection to table/ })
			.click();
		...
		await expect(
			page.locator(".paged-editor__pages .layout-table"),
		).toHaveCount(0);
