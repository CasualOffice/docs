...
test.describe("Demo.docx - Tables", () => {
	...
	test("renders simple ITEM/NEEDED table", async ({ page }) => {
		...
		const itemCell = page
			.locator('table th:has-text("ITEM"), table td:has-text("ITEM")')
			.first();
	...
	test("renders table with Books, Pens, Pencils rows", async ({ page }) => {
		await expect(
			page.locator('table td:has-text("Books")').first(),
		).toBeVisible();
		await expect(
			page.locator('table td:has-text("Pens")').first(),
		).toBeVisible();
		await expect(
			page.locator('table td:has-text("Pencils")').first(),
		).toBeVisible();
	});

	test("renders distance table with Point A-E", async ({ page }) => {
		await expect(
			page.locator('table:has-text("Point A")').first(),
		).toBeVisible();
		await expect(
			page.locator('table:has-text("Point B")').first(),
		).toBeVisible();
	});

	test("renders college enrollment table", async ({ page }) => {
		await expect(
			page.locator('table:has-text("Cedar University")').first(),
		).toBeVisible();
	});
	...
	test("renders calendar table", async ({ page }) => {
		await expect(
			page.locator('table:has-text("December 2007")').first(),
		).toBeVisible();
	});

	test("renders table header cells with background color from style", async ({
		page,
	}) => {
		...
		const bgColor = await itemHeader.evaluate(
	...
	test("renders College table header with blue background from style", async ({
		page,
	}) => {
		...
		const bgColor = await collegeHeader.evaluate(
			(el) => getComputedStyle(el).backgroundColor,
		);
...
test.describe("Demo.docx - Hyperlinks", () => {
	...
	test("renders internal document link", async ({ page }) => {
		const link = page
			.locator('a:has-text("paragraph level formatting")')
			.first();
		await expect(link).toBeVisible();
					...
					page.evaluate(() => {
						...
						for (const p of paras) {
							...
							if (
								...
								p.textContent?.includes("crazy things")
							) {
