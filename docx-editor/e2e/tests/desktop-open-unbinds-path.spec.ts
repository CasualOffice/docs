...
test("opening a file in-window unbinds the previous desktop file path", async ({
	page,
}) => {
	...
	await page.evaluate(() => {
		(
			window as unknown as { __deskApp__: { filePath: string | null } }
		).__deskApp__.filePath = "/tmp/original.docx";
	});
	expect(
		await page.evaluate(
			() =>
				(window as unknown as { __deskApp__: { filePath: string | null } })
					.__deskApp__.filePath,
		),
