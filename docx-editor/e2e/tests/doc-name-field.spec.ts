...
const LONG_NAME =
	"Q3 2026 Regional Sales Performance Review and Strategic Outlook";

test.describe("Document name field — auto-grow, no baked-in ellipsis", () => {
	test('field grows with content and never stores "…" in the name', async ({
		page,
	}) => {
		...
		const shortWidth = await nameInput.evaluate(
		...
		await page.keyboard.press("Tab");
		const longWidth = await nameInput.evaluate(
			(el) => el.getBoundingClientRect().width,
		);
