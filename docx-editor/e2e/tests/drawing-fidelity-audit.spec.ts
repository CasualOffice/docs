	...
	return page.evaluate(() => {
		...
		if (pagesRoot) {
			for (const img of Array.from(pagesRoot.querySelectorAll("img"))) {
				...
				collected.push({
					kind: img.className.includes("layout-run-image")
						? "inline-image"
						: "bare-image",
...
test.describe("Drawing fidelity — sizes (every drawing renders at declared cx × cy)", () => {
	test("inline image renders at 100 × 80 px", async ({ page }) => {
		...
		expect(
			inline,
			`inline image must render (saw ${JSON.stringify(drawings)})`,
		).toBeDefined();
	...
	test("anchored image renders at 120 × 90 px", async ({ page }) => {
		await loadFixture(page);
		const drawings = await readDrawings(page);
		const anchored = drawings.find(
			(d) => d.w >= 115 && d.w <= 125 && d.h >= 85 && d.h <= 95,
		);
		expect(
			anchored,
			`anchored 120×90 image must render (saw ${JSON.stringify(drawings)})`,
		).toBeDefined();
	...
	test("standalone wps:wsp shape renders at 160 × 60 px with its text payload", async ({
		page,
	}) => {
		await loadFixture(page);
		const drawings = await readDrawings(page);
		...
		expect(
			shape,
			`SHAPE-3 must render (saw ${JSON.stringify(drawings)})`,
		).toBeDefined();
	...
	test("wpg group surfaces both child shapes at 100 × 40 px", async ({
		page,
	}) => {
		await loadFixture(page);
		const drawings = await readDrawings(page);
		...
		expect(
			childA,
			`group child A must render (saw ${JSON.stringify(drawings)})`,
		).toBeDefined();
...
test.describe("Drawing fidelity — inline + anchored image positions", () => {
	test("inline image flows at the column origin (left margin)", async ({
		page,
	}) => {
		await loadFixture(page);
	...
	test("anchored image lands at posOffset (100, 200) relative to margin", async ({
		page,
	}) => {
		await loadFixture(page);
		const origin = await flowOrigin(page);
...
test.describe("Drawing fidelity — known open positioning gaps", () => {
	...
	test.fixme("anchored wps:wsp shape lands at posOffset (200, 50), not at flow cursor", async ({
		page,
	}) => {
		await loadFixture(page);
		const origin = await flowOrigin(page);
	...
	test.fixme("wpg group child B sits 120 px to the right of child A (xfrm.off honored)", async ({
		page,
	}) => {
		await loadFixture(page);
		const drawings = await readDrawings(page);
