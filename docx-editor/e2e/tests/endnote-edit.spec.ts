...
test("endnote: renders at doc end → edit → save → reload persists", async ({
	page,
}) => {
	...
	await expect(
		page.locator('[data-testid="footnote-edit-dialog"]'),
	).toBeVisible();
	...
	const b64 = await page.evaluate(async () => {
		...
		const buf: ArrayBuffer | null =
			await window.__editorRef?.current?.exportDocx?.();
		if (!buf) return null;
		let bin = "";
