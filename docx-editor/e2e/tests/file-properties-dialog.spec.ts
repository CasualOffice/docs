...
test.describe("File → Properties dialog", () => {
	test("loads existing core properties, edits, and applies", async ({
		page,
	}) => {
		...
		await expect(page.getByTestId("fp-subject")).toHaveValue(
			"Original Subject",
		);
		...
		await page.getByRole("button", { name: /^File$/ }).click();
		await page.getByText("Properties", { exact: true }).click();
		await expect(page.getByTestId("fp-title")).toHaveValue(
			"Edited Title via Dialog",
		);
	});
});
