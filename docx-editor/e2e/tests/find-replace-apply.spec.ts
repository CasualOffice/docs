...
async function openReplace(page: import("@playwright/test").Page) {
	const mod = /Mac/i.test(await page.evaluate(() => navigator.platform))
		? "Meta"
		: "Control";
	await page.keyboard.press(`${mod}+h`);
	await page
		.locator('[data-testid="find-replace-dialog"]')
		.waitFor({ timeout: 3000 });
	return mod;
}

test("Replace and Replace All change the document; undo restores it", async ({
	page,
}) => {
	...
	expect(
		(await page.locator(".paged-editor__pages").innerText()).replace(
			/\s+/g,
			" ",
		),
	).toContain("f00 boo zoo");

	await dlg.getByRole("button", { name: /^Replace All$/ }).click();
	await page.waitForTimeout(400);
	expect(
		(await page.locator(".paged-editor__pages").innerText())
			.replace(/\s+/g, " ")
			.trim(),
	).toBe("f00 b00 z00");
...
test("Replace preserves surrounding formatting", async ({ page }) => {
	...
	).toContain("KEEP DONE end");
	const keepWeight = await page.evaluate(() => {
		const span = [
			...document.querySelectorAll(".paged-editor__pages span"),
		].find((s) => s.textContent?.includes("KEEP"));
