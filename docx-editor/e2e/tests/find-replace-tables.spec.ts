...
async function modKey(page: import("@playwright/test").Page) {
	return /Mac/i.test(await page.evaluate(() => navigator.platform))
		? "Meta"
		: "Control";
}

test("find counts matches in body and table cells; Replace All replaces all", async ({
	page,
}) => {
	...
	const body = (await page.locator(".paged-editor__pages").innerText()).replace(
		/\s+/g,
		" ",
	);
