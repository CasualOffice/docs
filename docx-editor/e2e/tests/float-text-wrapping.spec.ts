...
async function collectWrappingMetrics(page: Page) {
	return page.evaluate(() => {
		...
		const pageFloatLayers = document.querySelectorAll(
			".layout-floating-images-layer",
		);
		const pageFloatImgs = document.querySelectorAll(
			".layout-page-floating-image",
		);

		// Cell-level floating images
		const cellFloatLayers = document.querySelectorAll(
			".layout-cell-floating-images-layer",
		);
		const cellFloatImgs = document.querySelectorAll(
		...
		for (const imgContainer of pageFloatImgs) {
			...
			for (const line of pageLines) {
				...
				if (
					lineRect.bottom > imgRect.top + 2 &&
					lineRect.top < imgRect.bottom - 2
				) {
					...
					if (lineLeft < imgRect.right - 2 && lineRight > imgRect.left + 2) {
						...
						if (
							adjustedLeft < imgRect.right - 5 &&
							lineRight > imgRect.left + 5
						) {
							overlapCount++;
						}
					}
...
test.describe("Float Text Wrapping (Issues #143 & #188)", () => {
	test("page-level floating images have text wrapping applied", async ({
		page,
	}) => {
	...
	test("wrapSquare text wraps to the right of left-aligned image", async ({
		page,
	}) => {
		await loadFixture(page);
		...
		const hasWrapping = await page.evaluate(() => {
			const floatImgs = document.querySelectorAll(
				".layout-page-floating-image",
			);
	...
	test("wrapSquare with right-side image has text on left only", async ({
		page,
	}) => {
		await loadFixture(page);
		...
		const hasRightWrap = await page.evaluate(() => {
			...
			for (const container of floatImgs) {
				...
				if (left > contentWidth * 0.4) {
					...
					const lines = pageEl.querySelectorAll(
						".layout-paragraph .layout-line",
					);
					for (const line of lines) {
						const lineEl = line as HTMLElement;
	...
	test("cross-paragraph wrapping: tall image affects multiple paragraphs", async ({
		page,
	}) => {
		await loadFixture(page);
	...
	test("multiple floating images create combined exclusion zones", async ({
		page,
	}) => {
		await loadFixture(page);
	...
	test("table cell floating images are positioned at anchor offset", async ({
		page,
	}) => {
		await loadFixture(page);
		const metrics = await collectWrappingMetrics(page);
	...
	test("table cell text wraps around floating images", async ({ page }) => {
		...
		const cellWrapping = await page.evaluate(() => {
			const cellContents = document.querySelectorAll(
				".layout-table-cell-content",
			);
			let cellsWithOffset = 0;

			for (const cell of cellContents) {
				const hasFloatLayer = cell.querySelector(
					".layout-cell-floating-images-layer",
				);
	...
	test("no rendering regressions on existing image fixtures", async ({
		page,
	}) => {
