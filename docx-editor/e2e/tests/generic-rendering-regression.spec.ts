...
async function collectRenderAnomalies(page: Page): Promise<RenderAnomalies> {
	return page.evaluate(() => {
		...
		for (const headerEl of headerEls) {
			...
			const pageRect = headerEl
				.closest<HTMLElement>(".layout-page")
			...
			for (const img of images) {
				...
				if (
					pageRect &&
					(imgRect.top < pageRect.top - 0.5 ||
						imgRect.bottom > pageRect.bottom + 0.5)
				) {
...
test.describe("Generic Rendering Regression", () => {
	test("does not duplicate body images or clip header media", async ({
		page,
	}) => {
