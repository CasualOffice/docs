...
test.describe("Grammar check", () => {
	test('flags "a apple" and the fix menu replaces it with "an apple"', async ({
		page,
	}) => {
		...
		const squiggle = page
			.locator(".paged-editor__decoration-overlay .grammar-error")
			.first();
	...
	test("toggling grammar off clears the squiggles", async ({ page }) => {
		...
		await page.getByRole("menuitem", { name: /Grammar check/i }).click();
		await expect(
			page.locator(".paged-editor__decoration-overlay .grammar-error").first(),
		).toBeVisible({
			timeout: 5000,
		});
