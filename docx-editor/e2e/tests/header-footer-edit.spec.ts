...
test("header: hover shows the edit affordance, double-click edits", async ({
	page,
}) => {
	...
	await expect(
		page.locator(`${PAGES}.paged-editor--editing-header`),
	).toBeAttached();
	...
	await expect(
		page.locator('[data-testid="docx-editor"] .hf-editor-pm'),
	).toContainText("ZZTOP");
});
