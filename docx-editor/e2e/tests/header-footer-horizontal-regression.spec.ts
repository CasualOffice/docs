...
const HORIZONTAL_FIXTURE =
	"fixtures/generic-header-footer-horizontal-regression.docx";
...
async function collectAreaMetrics(page: Page): Promise<AreaMetrics[]> {
	return page.evaluate(() => {
		...
		return (["header", "footer"] as const)
			.map((section) => {
				const areaEl = pageEl.querySelector<HTMLElement>(
					`.layout-page-${section}`,
				);
...
test.describe("Header/Footer Horizontal Regression", () => {
	test("preserves full-width anchored header and footer media", async ({
		page,
	}) => {
