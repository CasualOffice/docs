...
test("header grows after editing: typing extra lines does not produce body overlap", async ({
	...
	await expect(page.locator(".layout-page-header [data-from-row]")).toHaveCount(
		1,
		{
			timeout: 15000,
		},
	...
	const measure = async () =>
		page.evaluate(() => {
			const page1 = document.querySelector<HTMLElement>(
				'[data-page-number="1"]',
			);
...
test("diagnostic: tall header (paragraphs + table + paragraphs) does not overlap body", async ({
	...
	if (metrics!.headerInnerHeight != null) {
		expect(metrics!.headerInnerHeight).toBeLessThanOrEqual(
			metrics!.headerHeight + 2,
		);
	}
});
