...
test.describe("Help menu", () => {
	...
	test("Help > Report issue opens GitHub issues URL with pre-filled body", async ({
		page,
	}) => {
		await page.evaluate(() => {
		...
		expect(url.origin + url.pathname).toBe(
			"https://github.com/CasualOffice/docs/issues/new",
		);
	...
	test("Help > Search the menus opens the command palette", async ({
		page,
	}) => {
		await page.getByRole("button", { name: "Help" }).click();
	...
	test("menu items show a focus ring on keyboard navigation", async ({
		page,
	}) => {
		await page.getByRole("button", { name: "Help" }).click();
		...
		const ring = await page.evaluate(() => {
			...
			if (!el)
				return { role: null as string | null, width: "0px", style: "none" };
