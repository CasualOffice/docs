...
test.describe("HF inline editor: toolbar + context menu + z-index (#384, #385)", () => {
	test("removing a header still allows recreating it from the blank header area", async ({
		...
		await expect(
			page.locator(".layout-page-header [data-from-row]"),
		).toHaveCount(1, {
			timeout: 15000,
		});
	...
	test("clicking a header table cell shows the Table toolbar group (#384)", async ({
		page,
	}) => {
		const editor = new EditorPage(page);
		await editor.goto();
		...
		await expect(
			page.locator('[role="group"][aria-label="Table"]'),
		).toHaveCount(0);
		...
		const headerCell = page
			.locator(".hf-inline-editor td, .hf-inline-editor th")
			.first();
	...
	test("right-click in a header table cell shows context menu with table actions (#384)", async ({
		...
		const insertRow = page
			.getByRole("menuitem", { name: /Insert row/i })
			.first();
		await expect(insertRow).toBeVisible({ timeout: 3000 });
	});

	test("horizontal ruler stays above the inline HF editor (#385)", async ({
		page,
	}) => {
		const editor = new EditorPage(page);
		await editor.goto();
