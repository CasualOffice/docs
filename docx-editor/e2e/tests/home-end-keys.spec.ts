...
test("Home and End move to line start and end", async ({ page }) => {
	...
	await ed.typeText("X");
	await page.waitForTimeout(120);
	expect(await page.locator(".paged-editor__pages").innerText()).toContain(
		"Xhello world",
	);
...
test("Shift+Home extends the selection to line start", async ({ page }) => {
	...
	await page.waitForTimeout(120);
	expect((await page.locator(".paged-editor__pages").innerText()).trim()).toBe(
		"Z",
	);
});

test("Ctrl/Cmd+Home/End move the caret to document start/end", async ({
	page,
}) => {
	const ed = new EditorPage(page);
	await ed.goto();
...
test("Home goes to the visual-line start on a wrapped paragraph", async ({
	page,
}) => {
	const ed = new EditorPage(page);
	await ed.goto();
