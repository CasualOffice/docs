...
test.describe("Home page", () => {
	test("renders template gallery at / (no e2e flag)", async ({ page }) => {
		...
		await expect(
			page.locator('[data-testid="template-card-blank"]').first(),
		).toBeVisible();
		// Open-from-disk affordance is always there too.
		await expect(
			page.locator('[data-testid="home-open-from-disk"]'),
		).toBeVisible();
		// The editor should NOT be mounted yet.
		await expect(page.locator('[data-testid="docx-editor"]')).toHaveCount(0);
	});

	test("clicking Blank document transitions to the editor", async ({
		page,
	}) => {
		await page.goto("/");
	...
	test("?e2e=1 skips the home page and mounts the editor directly", async ({
		page,
	}) => {
	...
	test("?skipHome=1 also skips home (embedder escape hatch)", async ({
		page,
	}) => {
	...
	test("category filter narrows results (Career shows Resume + Cover letter)", async ({
		...
		await expect(
			page.locator('[data-testid="template-card-resume"]'),
		).toBeVisible();
		await expect(
			page.locator('[data-testid="template-card-cover-letter"]'),
		).toBeVisible();
		// Meeting notes (Work) should NOT appear.
		await expect(
			page.locator('[data-testid="template-card-meeting-notes"]'),
		).toHaveCount(0);
	});
	...
	test('search narrows results (typing "recipe" shows only the recipe card)', async ({
		...
		await expect(
			page.locator('[data-testid="template-card-recipe"]'),
		).toBeVisible();
		await expect(
			page.locator('[data-testid="template-card-resume"]'),
		).toHaveCount(0);
