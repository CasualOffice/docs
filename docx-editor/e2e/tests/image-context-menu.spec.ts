...
test.describe("Image right-click layout menu", () => {
	test("right-click on floating image opens the layout menu", async ({
		page,
	}) => {
		await loadFixture(page);
		await page
			.locator(".layout-page-floating-image")
			.first()
			.click({ button: "right" });
		await expect(
			page.locator('[data-testid="image-context-menu"]'),
		).toBeVisible();
		// Text context menu should NOT have opened.
		await expect(page.locator(".docx-text-context-menu")).toHaveCount(0);
	});

	test("menu highlights current wrap type for a floating image", async ({
		page,
	}) => {
		await loadFixture(page);
		await page
		...
		await expect(
			menu.locator('[data-wrap-type="squareLeft"][data-current="true"]'),
		).toHaveCount(1);
		// "In Line with Text" is enabled when the image is floating (Scope C).
		await expect(
			menu.locator('[data-wrap-type="inline"][data-disabled="false"]'),
		).toHaveCount(1);
	});

	test('clicking "Square Right" flips the float to right-anchored', async ({
		page,
	}) => {
		await loadFixture(page);
		...
		await page
			.locator(
				'[data-testid="image-context-menu"] [data-wrap-type="squareRight"]',
			)
			.click();
	...
	test('clicking "Behind Text" keeps the image in the floating layer (wrapNone)', async ({
		...
		await page
			...
			.click({ button: "right" });
		await page
			.locator('[data-testid="image-context-menu"] [data-wrap-type="behind"]')
			.click();
		await page.waitForTimeout(400);
	...
	test("right-click on inline image: every option is enabled, inline marked current", async ({
		...
		await expect(
			menu.locator('[data-wrap-type="inline"][data-current="true"]'),
		).toHaveCount(1);
		...
		for (const wt of [
			...
			await expect(
				menu.locator(`[data-wrap-type="${wt}"][data-disabled="false"]`),
			).toHaveCount(1);
		}
	});

	test("inline → Square Left: image promotes to a left-anchored float", async ({
		page,
	}) => {
		await loadFixture(page);
		expect(await page.locator(".layout-line .layout-run-image").count()).toBe(
			1,
		);
		expect(await page.locator(".layout-page-floating-image").count()).toBe(1);
		...
		await page
			.locator(
				'[data-testid="image-context-menu"] [data-wrap-type="squareLeft"]',
			)
			.click();
		await page.waitForTimeout(400);
	...
	test("floating → In Line with Text: image returns to text flow", async ({
		page,
	}) => {
		await loadFixture(page);
		expect(await page.locator(".layout-page-floating-image").count()).toBe(1);
		...
		await page
			.locator('[data-testid="image-context-menu"] [data-wrap-type="inline"]')
			.click();
		await page.waitForTimeout(400);
