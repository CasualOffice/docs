...
test.describe("Image layout modes — inline, wrap, block", () => {
	test("all three modes render in the expected DOM containers", async ({
		page,
	}) => {
		await loadFixture(page);

		const counts = await page.evaluate(() => ({
			inline: document.querySelectorAll(".layout-line .layout-run-image")
				.length,
			block: document.querySelectorAll(".layout-block-image").length,
			pageFloat: document.querySelectorAll(".layout-page-floating-image")
				.length,
		}));
	...
	test("inline image lives inside a layout-line and is in text flow", async ({
		page,
	}) => {
		await loadFixture(page);
		...
		const inline = await page.evaluate(() => {
			...
			const totalTextRuns =
				paragraph.querySelectorAll(".layout-run-text").length;
			return {
	...
	test("wrap-square float creates left margins on overlapping lines", async ({
		page,
	}) => {
		await loadFixture(page);
	...
	test("topAndBottom image stands on its own line and does not create wrap margins", async ({
		...
		const block = await page.evaluate(() => {
			...
			for (const line of lines) {
				...
				const lineRect = lineEl.getBoundingClientRect();
				if (
					lineRect.bottom > blockRect.top + 2 &&
					lineRect.top < blockRect.bottom - 2
				) {
				...
				const adjacent =
					(lineRect.top > blockRect.bottom &&
						lineRect.top - blockRect.bottom < 40) ||
					(lineRect.bottom < blockRect.top &&
						blockRect.top - lineRect.bottom < 40);
