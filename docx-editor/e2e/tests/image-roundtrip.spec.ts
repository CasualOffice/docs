...
test.describe("Image Round-Trip (Issue #45)", () => {
	...
	test("existing image survives save with correct EMU dimensions", async ({
		page,
	}) => {
		...
		expect((await mediaFile!.async("arraybuffer")).byteLength).toBeGreaterThan(
			0,
		);
	...
	test("newly inserted image is saved with media, rels, and content type", async ({
		page,
	}) => {
		...
		expect(
			(await zip.file(pngFile!)!.async("arraybuffer")).byteLength,
		).toBeGreaterThan(0);
	...
	test("existing image is visible after save round-trip reload", async ({
		page,
	}) => {
		test.fixme(
