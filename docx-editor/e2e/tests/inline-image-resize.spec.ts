...
test("selecting an inline image shows resize handles and dragging resizes it", async ({
	page,
}) => {
	...
	const img = page
		.locator('[data-testid="docx-editor"] img.layout-run-image')
		.first();
	const b0 = await img.boundingBox();
	if (!b0) throw new Error("no image");

	await img.click({
		position: { x: Math.round(b0.width / 2), y: Math.round(b0.height / 2) },
	});
