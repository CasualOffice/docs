...
test.describe("List paste", () => {
	...
	test("plain <ul> pastes as a bullet list with a marker per item", async ({
		page,
	}) => {
		await pasteHtml(
			page,
	...
	test("Word mso-list paragraphs paste as a list without literal bullets", async ({
		page,
	}) => {
		const word =
		...
		await expect(page.locator(".paged-editor__pages")).not.toContainText(
			"· Word",
		);
	});
});
