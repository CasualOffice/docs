...
test(".txt opens as source-only — no preview pane or view toggle", async ({
	page,
}) => {
	await page.goto("/");
	const fileInput = page.getByTestId("home-file-input");
	...
	await expect(page.getByTestId("markdown-source")).toContainText(
...
test("Download saves the edited source back to a .md file", async ({
	page,
}) => {
	await openMarkdown(page);
