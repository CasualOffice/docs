...
test.describe("Mobile floating format bar", () => {
	test.describe("desktop viewport — chip hidden", () => {
		...
		test("does not render on desktop even when text is selected", async ({
			page,
		}) => {
			...
			await expect(
				page.locator('[data-testid="mobile-format-bar"]'),
			).toHaveCount(0);
		});
		...
		test("desktop floating bar appears on selection (C1)", async ({ page }) => {
			...
			await expect(
				page.locator('[data-testid="desktop-format-bar"]'),
			).toHaveCount(0);
			...
			await expect(
				page.locator('[data-testid="desktop-format-bold"]'),
			).toBeVisible();
		...
		test("link button on the desktop bar opens the hyperlink dialog", async ({
			page,
		}) => {
			await page.goto("/?e2e=1");
			await page.waitForSelector('[data-testid="docx-editor"]');
			...
			await expect(
				page.locator('[data-testid="hyperlink-dialog"]'),
			).toBeVisible();
		});
	...
	test.describe("phone viewport — chip appears + tap formats", () => {
		...
		test("chip appears on range selection, hides on collapse, bold applies on tap", async ({
			...
			for (const cmd of ["bold", "italic", "underline", "strikethrough"]) {
				await expect(
					page.locator(`[data-testid="mobile-format-${cmd}"]`),
				).toBeVisible();
			}
