...
test.describe("Paged Editor - Click Positioning", () => {
	...
	test("click at start of text positions cursor at beginning", async ({
		page,
	}) => {
		// Type some text
		await editor.typeText("Hello World");
...
test.describe("Paged Editor - Tab Click Handling", () => {
	...
	test("click after tab positions at first char of next run", async ({
		page,
	}) => {
		// Type text with a tab
		await editor.typeText("Before");
...
test.describe("Paged Editor - Double/Triple Click Selection", () => {
	...
	test("double-click selects word", async ({ page }) => {
		...
		const textSpan = page
			.locator('.layout-page span:has-text("beautiful")')
			.first();
		const boundingBox = await textSpan.boundingBox();
	...
	test("double-click on last word selects it", async ({ page }) => {
		...
		const textSpan = page
			.locator('.layout-page span:has-text("World")')
			.first();
		const boundingBox = await textSpan.boundingBox();
	...
	test("double-click selects Unicode word with diacritics", async ({
		page,
	}) => {
		await editor.typeText("PROTOKÓŁ test");

		const textSpan = page
			.locator('.layout-page span:has-text("PROTOKÓŁ")')
			.first();
		const boundingBox = await textSpan.boundingBox();
	...
	test("triple-click in middle paragraph selects only that paragraph", async ({
		page,
	}) => {
...
test.describe("Paged Editor - Click on Empty Areas", () => {
	...
	test("click after end of line positions cursor at line end", async ({
		page,
	}) => {
		// Type a short line
...
test.describe("Paged Editor - Cursor Position Accuracy", () => {
	...
	test("clicking between characters positions cursor accurately", async ({
		page,
	}) => {
		// Type a word
