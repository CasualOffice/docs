...
test.describe("PanelRail (X7)", () => {
	test("renders three icon buttons in the right-edge rail", async ({
		page,
	}) => {
	...
	test("clicking Outline opens the panel and shows the pressed state", async ({
		page,
	}) => {
		const editor = new EditorPage(page);
		await editor.goto();
	...
	test("Comments and Version history are mutually exclusive", async ({
		page,
	}) => {
		const editor = new EditorPage(page);
		await editor.goto();
		...
		await expect(page.getByTestId("rail-comments")).toHaveAttribute(
			"aria-pressed",
			"true",
		);

		await page.getByTestId("rail-history").click();
		await expect(page.getByTestId("rail-history")).toHaveAttribute(
			"aria-pressed",
			"true",
		);
