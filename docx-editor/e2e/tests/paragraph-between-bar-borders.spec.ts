	...
	return page.evaluate(() => {
		...
		view.state.doc.descendants(
			(node: {
				...
				const borders = node.attrs.borders as {
					between?: unknown;
					bar?: unknown;
				} | null;
				out.push({
...
test("w:between + w:bar paragraph borders survive load and save → reload", async ({
	page,
}) => {
...
test("w:between renders a horizontal rule between two grouped paragraphs", async ({
	page,
}) => {
	const editor = new EditorPage(page);
	await editor.goto();
		...
		for (const pageEl of pages) {
			...
			for (const p of paras) {
				if (p.textContent?.includes("BETWEEN-B")) {
					...
					const overlay = p.querySelector<HTMLElement>(
