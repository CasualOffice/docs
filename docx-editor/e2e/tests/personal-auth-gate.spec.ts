...
test.describe("PersonalAuthGate", () => {
	...
	test("login happy path → modal hides + signed-in content renders", async ({
		page,
	}) => {
		await mockAuth(page);
		await page.goto("/?e2e=auth-gate");
	...
	test("login wrong password → inline error, modal stays open", async ({
		page,
	}) => {
		await mockAuth(page);
		await page.goto("/?e2e=auth-gate");
		...
		await page.getByTestId("personal-auth-submit").click();
		await expect(page.getByTestId("personal-auth-error")).toContainText(
			/don.t match/i,
		);
		await expect(page.getByTestId("personal-auth-gate")).toBeVisible();
	});
	...
	test("toggle to signup → create account → success", async ({ page }) => {
		...
		await expect(page.getByTestId("personal-auth-submit")).toContainText(
			/create account/i,
		);
	...
	test("UserMenu — shows the username and toggles dropdown", async ({
		page,
	}) => {
		await mockAuth(page, { signedInAtBoot: true });
		await page.goto("/?e2e=auth-gate");
	...
	test("ProfileSettings — opens, loads, saves, updates UserMenu", async ({
		page,
	}) => {
		await mockAuth(page, { signedInAtBoot: true });
		...
		await expect(page.getByTestId("profile-settings-displayname")).toHaveValue(
			"Alex",
		);
		await expect(page.getByTestId("profile-settings-timezone")).toHaveValue(
			"America/Los_Angeles",
		);
	...
	test("ProfileSettings — cancel discards edits", async ({ page }) => {
		...
		await page.getByTestId("user-menu-profile").click();
		await page
			.getByTestId("profile-settings-displayname")
			.fill("changed-but-cancelled");
