...
test("Format panel: image chip → panel beside doc, wrap applies", async ({
	page,
}) => {
	...
	await img.click({
		position: { x: Math.round(ib!.width / 2), y: Math.round(ib!.height / 2) },
	});
	...
	await expect(
		page.locator('[data-testid="properties-image-section"]'),
	).toBeVisible();
	...
	expect((imgBox?.x ?? 0) + (imgBox?.width ?? 0)).toBeLessThan(
		panelBox?.x ?? 0,
	);
...
test("Format panel: image Top&bottom wrap + distance-from-text apply", async ({
	page,
}) => {
	const editor = new EditorPage(page);
	await editor.goto();
	...
	await expect(
		page.locator('[data-testid="properties-image-dist"]'),
	).toBeVisible();
...
test("Format panel: image Arrange (rotate) + Border apply to the document", async ({
	page,
}) => {
	const editor = new EditorPage(page);
	await editor.goto();
	...
	const transform = await paintedImg().evaluate(
		(el) =>
			(el as HTMLElement).style.transform || getComputedStyle(el).transform,
	);
...
test("Format panel: table chip → table section (not empty), delete row applies", async ({
	...
	await expect(page.locator(PANEL)).toBeVisible();
	await expect(
		page.locator('[data-testid="properties-table-section"]'),
	).toBeVisible();
	await expect(page.locator(PANEL)).not.toContainText(
		"Select an image, table, or shape",
	);
...
test("Format panel: textbox chip → section (resize + fill apply)", async ({
	page,
}) => {
	const editor = new EditorPage(page);
	await editor.goto();
	...
	await expect(
		page.locator('[data-testid="textbox-format-chip"]'),
	).toBeVisible();
	...
	await expect(page.locator(PANEL)).toBeVisible();
	await expect(
		page.locator('[data-testid="properties-textbox-section"]'),
	).toBeVisible();

	// outline applies: "Thick" gives the painted box a visible border width
	await page
		.locator('[data-testid="properties-textbox-outline-thick"]')
		.click();
	await page.waitForTimeout(500);
...
test('Format panel: textbox "No fill" removes a pre-filled background', async ({
	page,
}) => {
	const editor = new EditorPage(page);
	await editor.goto();
...
test("Format panel: only one right-side surface open at a time", async ({
	page,
}) => {
	const editor = new EditorPage(page);
	await editor.goto();
