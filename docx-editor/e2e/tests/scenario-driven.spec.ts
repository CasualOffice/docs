...
test.describe("Keyboard Shortcuts Suite", () => {
	test("run all shortcut scenarios", async ({ page }) => {
		...
		for (const scenarios of allScenarios.values()) {
			shortcutScenarios.push(
				...scenarios.filter((s) => !s.skip && s.tags?.includes("shortcut")),
			);
		}
		...
		if (failed.length > 0) {
			const failedNames = failed.map((r) => r.scenario.name).join(", ");
			throw new Error(
				`${failed.length} shortcut scenarios failed: ${failedNames}`,
			);
		}
	});
});
...
test.describe("Edge Cases Suite", () => {
	...
	test("run race condition scenarios", async ({ page }) => {
		...
		if (failed.length > 0) {
			const failedNames = failed.map((r) => r.scenario.name).join(", ");
			throw new Error(
				`${failed.length} race condition scenarios failed: ${failedNames}`,
			);
		}
	});
	...
	test("run boundary condition scenarios", async ({ page }) => {
		...
		if (failed.length > 0) {
			const failedNames = failed.map((r) => r.scenario.name).join(", ");
			throw new Error(
				`${failed.length} boundary scenarios failed: ${failedNames}`,
			);
		}
	});
});
