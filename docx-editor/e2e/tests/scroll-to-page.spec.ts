...
test.describe("scrollToPage / getTotalPages (issue #280)", () => {
	test.beforeEach(async ({ page }) => {
		...
		await page.waitForFunction(
			() => (window.__DOCX_EDITOR_E2E__?.getTotalPages() ?? 0) > 1,
			{
				timeout: 10000,
			},
		);
	...
	test("getTotalPages reports the layout page count (>1) without scrolling", async ({
		page,
	}) => {
		const total = await page.evaluate(
			() => window.__DOCX_EDITOR_E2E__?.getTotalPages() ?? 0,
		);
		// paraid-test.docx is intentionally long; multi-page is the whole point.
		expect(total).toBeGreaterThan(1);
	});

	test("scrollToPage(2) brings the second page into the viewport", async ({
		page,
	}) => {
	...
	test("scrollToPage(N) brings the last page into the viewport", async ({
		page,
	}) => {
		const total = await page.evaluate(
			() => window.__DOCX_EDITOR_E2E__?.getTotalPages() ?? 0,
		...
		await page.evaluate(
			(n) => window.__DOCX_EDITOR_E2E__?.scrollToPage(n),
			total,
		);
		await waitPageShellInViewport(page, total, 25000);
	});

	test("scrollToPage with out-of-range / invalid input is a no-op (no crash)", async ({
		page,
	}) => {
