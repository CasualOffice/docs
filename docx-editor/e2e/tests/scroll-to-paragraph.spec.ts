...
test.describe("Scroll to paragraph / position (E2E)", () => {
	...
	test("scrollToParaId returns false for unknown paraId", async ({ page }) => {
		const ok = await page.evaluate(
			() =>
				window.__DOCX_EDITOR_E2E__?.scrollToParaId("__NO_SUCH_PARA_ID__") ??
				null,
		);
		expect(ok).toBe(false);
	});

	test("scrollToParaId brings first paragraph paraId into viewport", async ({
		page,
	}) => {
	...
	test("scrollToParaId places the cursor at the end of the target paragraph", async ({
		page,
	}) => {
		const firstId = await page.evaluate(
			() => window.__DOCX_EDITOR_E2E__?.getFirstTextblockParaId() ?? null,
	...
	test("long jump: scroll to last paraId after scrolling to first", async ({
		page,
	}) => {
		const firstId = await page.evaluate(
			() => window.__DOCX_EDITOR_E2E__?.getFirstTextblockParaId() ?? null,
		...
		test.skip(
			firstId === lastId,
			"Demo needs at least two distinct paragraph paraIds",
		);
	...
	test("scrollToPosition brings pm start into viewport (P2)", async ({
		page,
	}) => {
		const firstId = await page.evaluate(
			() => window.__DOCX_EDITOR_E2E__?.getFirstTextblockParaId() ?? null,
	...
	test("hidden ProseMirror is portaled outside the paged scroller", async ({
		page,
	}) => {
		const isPortaled = await page.evaluate(() => {
			...
			return (
				hiddenPm != null &&
				hiddenPm.parentElement === document.body &&
				!pages?.contains(hiddenPm)
			);
		});
