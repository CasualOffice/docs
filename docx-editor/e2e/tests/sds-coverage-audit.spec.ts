...
test("SDS real-world doc — every major content type paints", async ({
	page,
}) => {
	...
	const report = await page.evaluate(() => {
		...
		return {
			...
			).filter((el) => visible(el as unknown as HTMLElement)).length,
			bodyText:
	...
	expect(
		report.textBoxes,
		"VML textboxes from textbox-render-vml fix",
