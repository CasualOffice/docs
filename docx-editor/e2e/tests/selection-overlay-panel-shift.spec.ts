...
test("image selection overlay follows the canvas when the Format panel opens", async ({
	page,
}) => {
	...
	const delta = await page.evaluate(() => {
		...
		return Math.abs(
			box.getBoundingClientRect().left - im.getBoundingClientRect().left,
		);
	});
...
test("text box selection box follows the canvas when the Format panel opens", async ({
	page,
}) => {
	const editor = new EditorPage(page);
	await editor.goto();
	...
	expect(
		delta,
		"textbox handle must re-anchor to the shifted box",
	).not.toBeNull();
