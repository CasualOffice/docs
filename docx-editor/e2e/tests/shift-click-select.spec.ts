 ...
 * the Shift modifier.
 */
test("Shift+Click extends the selection to the click point", async ({
	page,
}) => {
	...
	const span = page
		.locator(".layout-page-content span", { hasText: "AAAA" })
		.first();
...
test("a plain click still just places the cursor (no extend)", async ({
	page,
}) => {
	const ed = new EditorPage(page);
	await ed.goto();
	...
	const span = page
		.locator(".layout-page-content span", { hasText: "one" })
		.first();
	const box = await span.boundingBox();
	if (!box) throw new Error("no span box");
