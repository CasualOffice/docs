...
test.describe("Split color button (issue #130)", () => {
	...
	test("text color: apply-half re-uses the last picked color (no dropdown)", async ({
		page,
	}) => {
		...
		const colors = await page.evaluate(() => {
			...
			document
				.querySelectorAll('.paged-editor__pages [style*="color"]')
				.forEach((el) => {
	...
	test("highlight color: apply-half re-uses the last picked color", async ({
		page,
	}) => {
	...
	test("arrow half opens the dropdown (existing flow still works)", async ({
		page,
	}) => {
