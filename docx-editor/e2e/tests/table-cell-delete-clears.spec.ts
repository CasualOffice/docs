 ...
 */
test("full-table selection + Delete clears contents and keeps the table", async ({
	page,
}) => {
	...
	expect((await cells.allInnerTexts()).every((t) => t.trim() === "")).toBe(
		true,
	);
});

test("partial cell selection + Backspace clears only the selected cells", async ({
	page,
}) => {
	test.setTimeout(60000);
	const editor = new EditorPage(page);
