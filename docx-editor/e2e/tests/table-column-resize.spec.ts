...
test.describe("Table column resize", () => {
	test("dragging the 0/1 handle right widens col 0 and narrows col 1", async ({
		page,
	}) => {
	...
	test("dragging the 1/2 handle right widens col 1 and narrows col 2", async ({
		page,
	}) => {
		await loadFixture(page);
		const before = await rowWidths(page);
