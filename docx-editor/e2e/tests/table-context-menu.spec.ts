...
test.describe("Table Context Menu", () => {
	...
	test("right-click table menu shows merge and split actions and can insert a row", async ({
		...
		await expect(
			menu.locator('[role="menuitem"]').filter({ hasText: /^Split cell$/ }),
		).toHaveCount(1);
	...
	test('right-click in a table cell shows "Delete table" (discoverability)', async ({
		page,
	}) => {
	...
	test("right-click split cell applies a one-by-two split", async ({
		page,
	}) => {
		await editor.loadDocxFile("fixtures/with-tables.docx");
		await editor.rightClickTableCell(0, 0, 0);
