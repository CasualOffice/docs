...
test("table border/color/fill render in the Format panel, not the toolbar", async ({
	page,
}) => {
	...
	await input.setInputFiles(
		join(process.cwd(), "e2e/fixtures/medical-incident-form.docx"),
	);
	...
	await page
		.getByText("Date and time of the incident", { exact: false })
		.first()
		.click();
	await page.waitForTimeout(700);

	// Table group is active (so the toolbar absence below is meaningful)…
	await expect(page.locator('[data-testid="toolbar-table-more"]')).toHaveCount(
		1,
	);
	// …but the border picker is NOT in the toolbar anymore.
	await expect(
		page.locator('[data-testid="toolbar-table-borders"]'),
	).toHaveCount(0);

	// Open the table Format chip → the properties panel.
	await page
		.locator('[data-testid="table-format-chip"]')
		.click({ timeout: 2500 });
	await page.waitForTimeout(700);
	...
	await expect(
		section.locator('[data-testid="toolbar-table-borders"]'),
	).toHaveCount(1);
});
