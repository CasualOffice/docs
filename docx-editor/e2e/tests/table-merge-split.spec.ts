...
test.describe("Table Cell Merge/Split", () => {
	...
	test("table with merged cells renders colspan attribute correctly", async ({
		page,
	}) => {
		// Insert a 3x3 table
	...
	test("split cell button enabled with a single active cell", async ({
		page,
	}) => {
	...
	test("merge cells button disabled with single cell cursor", async ({
		page,
	}) => {
		await editor.insertTable(2, 2);
		await page.waitForTimeout(300);
