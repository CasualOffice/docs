...
test.describe("Merged table cells render correctly beyond column 0", () => {
	test("horizontal and vertical merges paint with correct geometry", async ({
		page,
	}) => {
		...
		const data = await page.evaluate(() => {
			...
			const rows = Array.from(
				table.querySelectorAll<HTMLElement>(".layout-table-row"),
			);
			const rowReports = rows.map((row) => {
				const cells = Array.from(
					row.querySelectorAll<HTMLElement>(".layout-table-cell"),
				);
				return {
		...
		expect(mergedWidth, "merged cell is wider than single").toBeGreaterThan(
			a0Width * 1.5,
		);
		...
		expect(
			r2.cellCount,
			"row 2 has 2 painted cells (vMerge continue skipped)",
		).toBe(2);
