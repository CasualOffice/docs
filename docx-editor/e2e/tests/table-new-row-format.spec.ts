...
async function snapshotCells(
	...
	return page.evaluate(() => {
		...
		view.state.doc.descendants(
			(node: { type: { name: string } }, _pos: number) => {
								...
								row.descendants((cell) => {
									if (cell.type.name === "tableCell") {
										out.push({
											...
											colIdx,
											backgroundColor:
												(cell.attrs.backgroundColor as string | null) ?? null,
										});
										colIdx++;
									}
								});
...
test.describe("Insert row preserves cell formatting", () => {
	test("addRowAbove + addRowBelow clone the source row cell backgroundColor", async ({
		...
		expect(baseline.filter((c) => c.rowIdx === 0)[0].backgroundColor).toBe(
			"#FFCDD2",
		);
