	...
	test("paragraph after a tall table renders below the table (no overlap)", async ({
		...
		const markerTop = await page.evaluate(() => {
			...
			const marker = paragraphs.find((p) =>
				p.textContent?.includes("POST-T1-MARKER"),
			);
		...
		expect(
			markerTop,
			"POST-T1-MARKER paragraph should be rendered",
		).not.toBeNull();
	...
	test("paragraph after second table is also below it (multi-table run)", async ({
		...
		const data = await page.evaluate(() => {
			...
			const ids = Array.from(
				new Set(allTables.map((t) => (t as HTMLElement).dataset.blockId)),
			);
			...
			const marker = paragraphs.find((p) =>
				p.textContent?.includes("POST-T2-MARKER"),
			);
		...
		expect(
			data.tableBottom,
			"second table bottom should be measurable",
		).not.toBeNull();
		expect(
			data.markerTop,
			"POST-T2-MARKER paragraph should be rendered",
		).not.toBeNull();
		expect(data.markerTop!).toBeGreaterThanOrEqual(data.tableBottom! - 1);
	});

	test("no phantom blank page between table and following paragraph", async ({
		page,
	}) => {
		...
		const blankPages = await page.evaluate(() => {
			...
			for (const p of pages) {
				const hasContent =
					p.querySelector(".layout-paragraph") ||
					p.querySelector(".layout-table");
