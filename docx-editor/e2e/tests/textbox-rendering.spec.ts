...
test.describe("Textbox rendering — content presence (issue #318)", () => {
	...
	test("regular paragraph between textboxes renders (sanity)", async ({
		page,
	}) => {
		await expect(
			page
				.getByTestId(EDITOR)
				.getByText(/regular paragraph between text boxes/i),
	...
	test('"Warning:" heading is in painter output', async ({ page }) => {
		await expect(page.getByTestId(EDITOR).getByText("Warning:")).toBeVisible({
			timeout: 5000,
		});
	});
	...
	test("first-textbox body text is in painter output", async ({ page }) => {
		await expect(
			page
				.getByTestId(EDITOR)
				.getByText("This is a basic text box with a thin black border."),
		).toBeVisible({ timeout: 5000 });
	});

	test("green-fill textbox body text is in painter output", async ({
		page,
	}) => {
		await expect(
			page
...
test.describe("Textbox rendering — visual containers (issue #318, real gap)", () => {
	...
	test(`renders ${EXPECTED_TEXTBOX_COUNT} .layout-textbox containers`, async ({
		page,
	}) => {
		const containers = page.locator(".layout-textbox");
		await expect(containers).toHaveCount(EXPECTED_TEXTBOX_COUNT, {
			timeout: 5000,
		});
	});

	test("every .layout-textbox is absolutely positioned (per renderTextBox.ts)", async ({
		page,
	}) => {
		const containers = page.locator(".layout-textbox");
		...
		for (let i = 0; i < count; i++) {
			const position = await containers
				.nth(i)
				.evaluate((el) => getComputedStyle(el).position);
			expect(position, `textbox #${i} should be absolutely positioned`).toBe(
				"absolute",
			);
		}
	});

	test("the bordered textbox container has a non-zero border", async ({
		page,
	}) => {
		...
		expect(
			borderWidth,
			"bordered textbox should have a top border > 0",
		).toBeGreaterThan(0);
	});
});
