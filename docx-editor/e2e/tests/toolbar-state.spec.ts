...
test.describe("Underline Detection", () => {
	...
	test("cursor inside underlined word shows underline active", async ({
		page,
	}) => {
...
test.describe("Combined Formatting Detection", () => {
	...
	test("cursor inside bold+italic+underline shows all active", async ({
		page,
	}) => {
...
test.describe("Partial Selection Detection", () => {
	...
	test("selection spanning bold and non-bold shows mixed state", async ({
		page,
	}) => {
		await editor.typeText("Normal bold normal");
		await editor.selectText("bold");
		...
		await page.evaluate(() => {
			...
			if (nodes.length >= 2) {
				...
				if (firstNode.textContent && lastNode.textContent) {
					range.setStart(
						firstNode,
						Math.max(0, firstNode.textContent.length - 2),
					);
...
test.describe("Alignment Detection at Cursor", () => {
	...
	test("cursor in right-aligned paragraph shows right active", async ({
		page,
	}) => {
...
test.describe("Font Detection at Cursor", () => {
	...
	test("cursor in different font shows that font in picker", async ({
		page,
	}) => {
...
test.describe("Edge Cases for Detection", () => {
	...
	test("empty paragraph inherits previous formatting detection", async ({
		page,
	}) => {
