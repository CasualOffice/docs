...
test("suggesting mode: typed text becomes a tracked insertion on the page", async ({
	page,
}) => {
...
test("suggesting mode: deleting existing text marks it as a tracked deletion", async ({
	page,
}) => {
	const editor = new EditorPage(page);
	await editor.goto();
...
test("accept-all resolves tracked insertions (mark gone, text kept)", async ({
	page,
}) => {
	const editor = new EditorPage(page);
	await editor.goto();
