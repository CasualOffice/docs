...
test.describe("Version history panel", () => {
	...
	test("toolbar toggle mounts the single version timeline", async ({
		page,
	}) => {
		...
		await expect(page.getByTestId("version-history-tab-activity")).toHaveCount(
			0,
		);
		...
		await expect(
			panel.getByTestId("version-history-save-version"),
		).toBeVisible();

		// A named save produces a row.
		await editor.saveNamedVersion("Checkpoint");
		await expect(
			panel.getByTestId("version-history-version-row").first(),
		).toBeVisible();
	});
	...
	test("opening version history closes the comments sidebar (and vice versa)", async ({
		page,
	}) => {
