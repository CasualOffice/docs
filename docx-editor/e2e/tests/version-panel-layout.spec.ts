...
test.describe("Version panel layout", () => {
	...
	test("kebab menu, current-version row, and named-only filter", async ({
		page,
	}) => {
		...
		await expect(
			menu.getByRole("menuitem", { name: /Restore this version/i }),
		).toBeVisible();
		...
		await page.screenshot({
			path: "screenshots/audit/vh-8-kebab.png",
			fullPage: false,
		});
		...
		await expect(
			page.locator('[data-testid="version-history-version-row"]'),
		).toHaveCount(1);
