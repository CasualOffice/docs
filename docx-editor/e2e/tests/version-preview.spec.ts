...
test.describe("Version preview", () => {
	...
	test("clicking a version opens an in-canvas preview with inline changes", async ({
		page,
	}) => {
		...
		await page.waitForFunction(
			() =>
				document.querySelectorAll('[data-testid="version-history-version-row"]')
					.length >= 2,
		);
	...
	test("the first (oldest) version previews with Show changes disabled", async ({
		page,
	}) => {
		const editor = new EditorPage(page);
		await editor.goto();
