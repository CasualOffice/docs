...
test.describe("Vertical ruler — sits beside the page and stays draggable", () => {
	test("ruler hugs the page left edge and its margin marker drags content", async ({
		page,
	}) => {
		...
		await page.mouse.move(mb.x + mb.width / 2, mb.y + mb.height / 2 + 50, {
			steps: 8,
		});
		await page.mouse.up();
	...
	test("dragging a margin marker does not scroll the viewport", async ({
		page,
	}) => {
		const editor = new EditorPage(page);
		await editor.goto();
		...
		for (const dy of [10, 20, 30, 40]) {
			await page.mouse.move(mb.x + mb.width / 2, mb.y + mb.height / 2 + dy, {
				steps: 3,
			});
	...
	test("stays aligned with the page top across zoom levels", async ({
		page,
	}) => {
		const editor = new EditorPage(page);
		await editor.goto();
