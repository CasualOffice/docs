...
test.describe("Component Screenshots", () => {
	...
	test("screenshot editor area (when exists)", async ({ page }) => {
		...
		const editor = page.locator(
			'[data-testid="editor"], .editor, #editor, [contenteditable]',
		);

		if ((await editor.count()) > 0) {
			await editor
				.first()
				.screenshot({ path: "screenshots/component-editor.png" });
		}
	});
	...
	test("screenshot document viewer (when exists)", async ({ page }) => {
		...
		const viewer = page.locator(
			'[data-testid="document-viewer"], .document-viewer, .page',
		);

		if ((await viewer.count()) > 0) {
			await viewer
				.first()
				.screenshot({ path: "screenshots/component-viewer.png" });
		}
	});
});
