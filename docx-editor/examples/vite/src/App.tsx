...
function useResponsiveLayout() {
	...
	const calcZoom = () => {
		...
		const target =
			vw <= 720
				? usableVw / CONTENT_WIDTH
				: usableVw / (PAGE_WIDTH + PAGE_PADDING);
...
function AutosaveE2E() {
	...
	useEffect(() => {
		(
			window as unknown as { __autosaveE2E?: { flush: () => Promise<void> } }
		).__autosaveE2E = {
			flush: state.flush,
		};
		return () => {
...
function parseEmbedConfig(): EmbedConfig | { error: string } {
	try {
		...
		if (cfg.app !== "docs")
			return { error: `Wrong app build (got ${cfg.app}, want docs)` };
...
function EmbedRoute() {
	...
	return (
		<div
			data-testid="embed-route"
			style={{ padding: 24, fontFamily: "system-ui, sans-serif" }}
		>
			<p style={{ color: "#475569", fontSize: 14 }}>
				Iframe embed scaffold ready. Host origin:{" "}
				<code>{config.hostOrigin}</code>.
			</p>
			<p style={{ color: "#94a3b8", fontSize: 12, marginTop: 6 }}>
				Full editor wiring lands in the EmbedRoute follow-up; this scaffold
				proves the route responds and the EmbedConfig parser works. The
				EmbedTransport class (exported from <code>@casualoffice/docs</code>) is
...
export function App() {
	...
	const [currentDocument, setCurrentDocument] = useState<DocxDocument | null>(
		null,
	);
	...
	const [recoveryBuffer, setRecoveryBuffer] = useState<ArrayBuffer | null>(
		null,
	);
	...
	const [themeMode, setThemeMode] = useState<"light" | "dark" | "system">(
		() =>
			(typeof window !== "undefined" && window.__deskApp__?.themeMode) ||
			"system",
	);
	useEffect(() => {
		if (typeof window === "undefined") return;
		const onTheme = (e: Event) => {
			const detail = (e as CustomEvent<{ mode?: "light" | "dark" | "system" }>)
				.detail;
			if (detail?.mode) setThemeMode(detail.mode);
		};
		window.addEventListener("deskapp:theme", onTheme as EventListener);
		return () =>
			window.removeEventListener("deskapp:theme", onTheme as EventListener);
	}, []);
	...
	useEffect(() => {
		...
		(window as unknown as { __editorRef?: typeof editorRef }).__editorRef =
			editorRef;

		const helpers = {
			getTotalPages: () => editorRef.current?.getTotalPages() ?? 0,
			scrollToPage: (n: number) => editorRef.current?.scrollToPage(n),
			scrollToParaId: (id: string) =>
				editorRef.current?.scrollToParaId(id) ?? false,
			...
			getFirstTextblockParaId: (): string | null => {
				const el = document.querySelector(
					".paged-editor__pages [data-para-id]",
				);
		...
		(
			window as unknown as { __DOCX_EDITOR_E2E__?: typeof helpers }
		).__DOCX_EDITOR_E2E__ = helpers;

		return () => {
			delete (window as unknown as { __editorRef?: typeof editorRef })
				.__editorRef;
			delete (window as unknown as { __DOCX_EDITOR_E2E__?: typeof helpers })
				.__DOCX_EDITOR_E2E__;
		};
	}, []);
	...
	useEffect(() => {
		...
		const bridge =
			typeof window !== "undefined" ? window.__deskApp__ : undefined;
		if (bridge?.isDesktop) {
			if (bridge.filePath) {
	...
	const handleDocumentNameChange = useCallback((name: string) => {
		...
		if (bridge?.isDesktop && bridge.filePath && bridge.rename) {
			void bridge
				.rename(name)
				.catch((err) => console.error("[deskApp] rename failed", err));
		}
	}, []);

	const onExportDesktop = useCallback(
	...
	useEffect(() => {
		...
		return () =>
			window.removeEventListener("deskapp:file-changed", onFileChanged);
	}, [isDesktop]);
	...
	if (textDoc) {
		return (
			<MarkdownEditor
				...
				kind={textDoc.kind}
				onRenameFile={(name) =>
					setTextDoc((d) => (d ? { ...d, fileName: name } : d))
				}
				onBack={handleGoHome}
				renderLogo={renderLogo}
			/>
		...
		<div style={styles.container}>
			<main style={styles.main}>
				{recoveryBuffer && (
					...
					>
						<span style={{ flex: 1 }}>
							This document had unsaved changes from a previous session. Restore
							them?
						</span>
						<button
							type="button"
...
function CollabApp({
	...
	const { plugins, status, peers, metaMap } = useCollab({
		room,
		backend,
		user,
	});
