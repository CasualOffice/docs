		...
		>
			...
			>
				<img
					...
					style={{
						...styles.cardThumb,
						...(hovered ? styles.cardThumbHover : null),
					}}
				/>
				<span
					style={{
						...styles.cardIconBadge,
						...(isMobile && mobile.cardIconBadge),
					}}
					aria-hidden="true"
				>
					<span
			...
			<div style={{ ...styles.cardBody, ...(isMobile && mobile.cardBody) }}>
				<div style={{ ...styles.cardTitle, ...(isMobile && mobile.cardTitle) }}>
					{entry.name}
				</div>
				<div
					style={{
						...styles.cardCategory,
						...(isMobile && mobile.cardCategory),
					}}
				>
					{entry.category}
				</div>
...
export function Home({
	...
	const filtered = useMemo(() => {
		...
		return TEMPLATES.filter((t) => {
			...
			if (category !== "All" && t.category !== category && t.id !== "blank")
				return false;
			if (!q) return true;
			return (
		...
		<div style={styles.page} data-testid="home-page">
			...
			<section style={{ ...styles.hero, ...(isMobile && mobile.hero) }}>
				...
				<p style={{ ...styles.heroLede, ...(isMobile && mobile.heroLede) }}>
					A real-time collaborative <code>.docx</code> editor that runs in the
					browser. Pick a template designed for the way you actually work — or
			...
			<section style={{ ...styles.controls, ...(isMobile && mobile.controls) }}>
				<label
					style={{ ...styles.searchWrap, ...(isMobile && mobile.searchWrap) }}
				>
					<span
						className="material-symbols-outlined"
				...
				<section style={{ ...styles.section, ...(isMobile && mobile.section) }}>
					...
					{filtered.length === 0 ? (
						<div style={styles.empty}>
							No templates match. Try a different keyword.
						</div>
					) : (
			...
			<footer style={{ ...styles.footer, ...(isMobile && mobile.footer) }}>
				<span>
					MIT fork of eigenpal/docx-editor · stateless real-time backend in Go
				</span>
				<a
					href="https://github.com/schnsrw/docx"
					target="_blank"
