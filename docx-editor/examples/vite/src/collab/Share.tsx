...
export function ShareDialog({
	...
	const [state, setState] = useState<"idle" | "uploading" | "ready" | "error">(
		"idle",
	);
	...
	useEffect(() => {
		...
		const mintRoom = async (): Promise<string> => {
			...
			if (!createRes.ok) {
				throw new Error(
					`HTTP ${createRes.status}: ${await createRes.text().catch(() => "")}`,
				);
			}
			...
			const seedRes = await fetch(
				`${collabHttp}/api/rooms/${encodeURIComponent(roomId)}/seed`,
				{
					method: "POST",
					body: formData,
				},
			...
			if (!seedRes.ok) {
				throw new Error(
					`HTTP ${seedRes.status}: ${await seedRes.text().catch(() => "")}`,
				);
			}
			return roomId;
		};
		...
		<div style={styles.backdrop} onClick={onClose}>
			<div style={styles.card} onClick={(e) => e.stopPropagation()}>
				...
				<p style={styles.subtitle}>
					Anyone with this link can open the document and edit it live. The
					session lives only while someone has it open — when everyone leaves,
