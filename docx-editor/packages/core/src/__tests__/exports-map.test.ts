...
const copyAssets = readFileSync(
	resolve(pkgRoot, "scripts/copy-assets.mjs"),
	"utf8",
);
...
describe("package.json exports map", () => {
	test("every JS subpath has a matching tsup entry", () => {
		...
		for (const [subpath, target] of Object.entries(pkg.exports)) {
			...
			if (
				!tsupConfig.includes(quoted) &&
				!tsupConfig.includes(dquoted) &&
				!matchesBare
			) {
	...
	test("every JS subpath has a typesVersions entry", () => {
		...
		for (const [subpath, target] of Object.entries(pkgFull.exports)) {
			...
			if (!tv[key])
				missing.push(`${subpath} → missing typesVersions key "${key}"`);
		}
		expect(missing).toEqual([]);
	});
