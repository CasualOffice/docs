...
export function executeCommand(doc: Document, command: AgentCommand): Document {
	...
	if (pluginHandler) {
		...
		return pluginHandler(
			doc,
			command as unknown as import("../core-plugins/types").PluginCommand,
		);
	}
	...
	switch (command.type) {
		...
		default: {
			...
			throw new Error(
				`Unknown command type: ${(_exhaustive as AgentCommand).type}`,
			);
		}
	}
}
...
export function executeCommands(
	...
	return commands.reduce(
		(currentDoc, command) => executeCommand(currentDoc, command),
		doc,
	);
}
			...
			if (runEnd <= startOffset || runStart >= endOffset) {
				...
				if (runStart < overlapStart) {
					newContent.push({
						...item,
						content: [
							{ type: "text", text: runText.slice(0, overlapStart - runStart) },
						],
					});
				}
				...
				if (runEnd > overlapEnd) {
					newContent.push({
						...item,
						content: [
							{ type: "text", text: runText.slice(overlapEnd - runStart) },
						],
					});
				}
			}
...
function executeInsertText(
	...
	const blockIndex = getBlockIndexForParagraph(
		body,
		command.position.paragraphIndex,
	);

	if (blockIndex === -1) {
		throw new Error(
			`Paragraph index ${command.position.paragraphIndex} not found`,
		);
	}
 ...
 */
function executeFormatParagraph(
	doc: Document,
	command: FormatParagraphCommand,
): Document {
	const newDoc = cloneDocument(doc);
	const body = newDoc.package.document;
	...
	if ("numPr" in command.formatting) {
		...
		if (numPr && numPr.numId !== undefined && numPr.numId !== 0) {
			...
			if (newDoc.package.numbering) {
				const num = newDoc.package.numbering.nums.find(
					(n) => n.numId === numPr.numId,
				);
				if (num) {
...
function executeInsertImage(
	...
	const newContent = insertTextAtOffset(
		paragraph,
		command.position.offset,
		"",
		undefined,
 ...
 */
function executeInsertHyperlink(
	doc: Document,
	command: InsertHyperlinkCommand,
): Document {
	const newDoc = cloneDocument(doc);
	const body = newDoc.package.document;
	...
	const linkText =
		command.displayText || paragraphText.slice(start.offset, end.offset);
 ...
 */
function executeRemoveHyperlink(
	doc: Document,
	command: RemoveHyperlinkCommand,
): Document {
	const newDoc = cloneDocument(doc);
	const body = newDoc.package.document;
 ...
 * Merge paragraphs
 */
function executeMergeParagraphs(
	doc: Document,
	command: MergeParagraphsCommand,
): Document {
	const newDoc = cloneDocument(doc);
	const body = newDoc.package.document;
 ...
 */
function executeApplyVariables(
	doc: Document,
	command: ApplyVariablesCommand,
): Document {
	const newDoc = cloneDocument(doc);
	const body = newDoc.package.document;
