...
export function buildSelectionContext(
	...
	if (range.start.paragraphIndex >= paragraphs.length) {
		throw new Error(
			`Invalid start paragraph index: ${range.start.paragraphIndex}`,
		);
	}
...
export function buildExtendedSelectionContext(
	...
	const documentSummary = includeDocumentSummary
...
export function getSelectionFormattingSummary(
	...
	for (
		let paraIdx = range.start.paragraphIndex;
		paraIdx <= range.end.paragraphIndex;
		paraIdx++
	) {
		...
		const startOffset =
			paraIdx === range.start.paragraphIndex ? range.start.offset : 0;
		const endOffset =
			paraIdx === range.end.paragraphIndex ? range.end.offset : Infinity;
...
function getSuggestedActions(
	...
	actions.push(
		...
		},
		{
			id: "rewrite",
			label: "Rewrite",
			description: "Rewrite this text differently",
			priority: 10,
		},
	);
