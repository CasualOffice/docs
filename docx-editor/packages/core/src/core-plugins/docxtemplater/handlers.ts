 ...
 */
export function handleInsertTemplateVariable(
	doc: Document,
 ...
 */
export function handleReplaceWithTemplateVariable(
	doc: Document,
	command: PluginCommand,
): Document {
	...
	if (range.start.paragraphIndex !== range.end.paragraphIndex) {
		throw new Error(
			"Template variable replacement cannot span multiple paragraphs",
		);
	}

	if (range.start.paragraphIndex >= paragraphs.length) {
		throw new Error(
			`Paragraph index ${range.start.paragraphIndex} out of bounds`,
		);
	}
