		...
		try {
			...
			return {
				...
				content: [
					{
						type: "text",
						text: `Failed to detect variables: ${(error as Error).message}`,
					},
				],
			};
		}
...
Variable names should follow the pattern: letters, numbers, underscores, starting with a letter.`,
		...
		try {
			...
			return {
				...
				content: [
					{
						type: "text",
						text: `Failed to insert variable: ${(error as Error).message}`,
					},
				],
			};
		}
	...
	annotations: {
		...
		examples: [
			{
				description:
					"Insert customer name variable at start of first paragraph",
				input: {
					documentId: "doc_123",
		...
		try {
			...
			return {
				...
				content: [
					{
						type: "text",
						text: `Failed to apply template: ${(error as Error).message}`,
					},
				],
			};
		}
		...
		try {
			...
			return {
				...
				content: [
					{
						type: "text",
						text: `Failed to validate template: ${(error as Error).message}`,
					},
				],
			};
		}
