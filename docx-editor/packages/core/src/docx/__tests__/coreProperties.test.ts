...
describe("applyCorePropertiesToXml", () => {
	...
	test("inserts a missing field before </cp:coreProperties>", () => {
		...
		const out = applyCorePropertiesToXml(minimal, {
			title: "Fresh",
			creator: "Bot",
		});
		...
		expect(out.indexOf("<dc:title>")).toBeLessThan(
			out.indexOf("</cp:coreProperties>"),
		);
	});
	...
	test("round-trip: parse → apply edit → re-parse preserves untouched fields", () => {
		...
		expect(reparsed.created?.toISOString()).toBe(
