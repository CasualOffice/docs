...
describe("New blank-doc header/footer save (#274)", () => {
	test("header added to blank doc persists through createDocx", async () => {
		...
		expect(documentXml).toMatch(
			/<w:headerReference [^/]*r:id="rId_new_header_default"[^/]*\/>/,
		);
	});
	...
	test("both header and footer added together both persist", async () => {
		...
		expect(await zip.file("word/header1.xml")!.async("text")).toContain(
			"Top of page",
		);
		expect(await zip.file("word/footer1.xml")!.async("text")).toContain(
			"Bottom of page",
		);
