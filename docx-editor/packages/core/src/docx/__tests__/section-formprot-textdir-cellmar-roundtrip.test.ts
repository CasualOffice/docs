...
function findElement(node: XmlElement, tag: string): XmlElement | null {
	if (node.name === tag || node.name?.endsWith(`:${tag.replace(/^w:/, "")}`))
		return node;
...
describe("<w:formProt> round-trip", () => {
	...
	test('parses w:val="true"', () => {
		const props = parseSectPr(
			'<w:sectPr><w:formProt w:val="true"/></w:sectPr>',
		);
...
describe("<w:textDirection> round-trip", () => {
	test('parses w:val="lrTb"', () => {
		const props = parseSectPr(
			'<w:sectPr><w:textDirection w:val="lrTb"/></w:sectPr>',
		);
	...
	test("parses other ECMA-376 values", () => {
		expect(
			parseSectPr('<w:sectPr><w:textDirection w:val="tbRl"/></w:sectPr>')
				.textDirection,
		).toBe("tbRl");
		expect(
			parseSectPr('<w:sectPr><w:textDirection w:val="btLr"/></w:sectPr>')
				.textDirection,
		).toBe("btLr");
	});
...
describe("<w:tblCellMar> w:start / w:end (logical sides)", () => {
	...
	test("parses w:end as margins.right and records useLogicalSides", () => {
		const margins = parseTblCellMar(
			'<w:tblCellMar><w:end w:w="240" w:type="dxa"/></w:tblCellMar>',
		);
