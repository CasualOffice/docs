...
export function parseComments(
	...
	for (const child of children) {
		...
		const rawDone =
			getAttribute(child, "w", "done") ?? child.attributes?.["w:done"];
		...
		const parentId =
			rawParentId != null ? parseInt(String(rawParentId), 10) : undefined;
		...
		for (const contentChild of getChildElements(child)) {
			...
			if (contentName === "p") {
				...
				const pParaId =
					getAttribute(contentChild, "w14", "paraId") ??
					contentChild.attributes?.["w14:paraId"];
