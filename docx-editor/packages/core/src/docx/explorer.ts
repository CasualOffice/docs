 ...
 */
export async function exploreDocx(
	buffer: ArrayBuffer,
): Promise<DocxExploration> {
...
function formatXml(xml: string): string {
	...
	while (i < cleaned.length) {
		if (cleaned[i] === "<") {
			...
			const text =
				nextTag === -1 ? cleaned.substring(i) : cleaned.substring(i, nextTag);
			if (text.trim()) {
				formatted += text;
			}
...
export function getKeyFiles(exploration: DocxExploration): DocxKeyFiles {
	...
	const headerCount = paths.filter((p) =>
		p.match(/word\/header\d+\.xml/),
	).length;
	const footerCount = paths.filter((p) =>
