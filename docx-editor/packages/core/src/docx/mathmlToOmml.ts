...
export function mathmlToOmml(
	...
	try {
		...
		root = (parsed.elements ?? []).find(
			(e) => e.type === "element" && local(e.name) === "math",
		);
	...
	const rootDisplay =
		(root.attributes && (root.attributes["display"] as string | undefined)) ||
		"inline";
	const display =
		options.display ?? (rootDisplay === "block" ? "block" : "inline");
