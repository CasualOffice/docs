	...
	for (const el of sdtPr.elements) {
		...
		switch (name) {
			...
			case "tag":
				...
				break;
			case "lock":
				props.lock = (getAttribute(el, "w", "val") ??
					"unlocked") as SdtProperties["lock"];
				break;
			case "id": {
			...
			case "color": {
				...
				const val =
					getAttribute(el, "w15", "val") ?? getAttribute(el, "w", "val");
			...
			case "placeholder": {
				...
				if (docPart) {
					...
					props.placeholder = valEl
						? (getAttribute(valEl, "w", "val") ?? undefined)
						: undefined;
				}
				break;
			}
...
function isEmptyXmlElement(el: XmlElement): boolean {
	const hasAttrs =
		el.attributes != null && Object.keys(el.attributes).length > 0;
	...
	const changes = findChildren(pPr, "w", "pPrChange")
		.map((changeElement): ParagraphPropertyChange => {
			...
			return {
				...
				previousFormatting: parseParagraphProperties(
					previousPPr,
					theme,
					styles ?? undefined,
				),
...
export function parseParagraph(
	...
	if (pPr) {
		paragraph.formatting = parseParagraphProperties(
			pPr,
			theme,
			styles ?? undefined,
		);
		...
		if (numId !== undefined && numId !== 0) {
			...
			if (level) {
				...
				paragraph.listRendering = {
					...
					markerFontSize: level.rPr?.fontSize
						? level.rPr.fontSize / 2
						: undefined,
					levelNumFmts,
