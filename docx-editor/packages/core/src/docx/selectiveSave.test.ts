		...
		for (const para of paragraphs) {
			if (para.paraId && para.paraId !== paraId) {
				...
				if (origOffsets && resultOffsets) {
					const origPara = originalXml.substring(
						origOffsets.start,
						origOffsets.end,
					);
					const resultPara = resultXml.substring(
						resultOffsets.start,
						resultOffsets.end,
					);
...
describe("buildPatchedDocumentXml with real DOCX XML", () => {
	test("patches real document.xml correctly", async () => {
		...
		const origParagraph = originalXml.substring(
			origOffsets.start,
			origOffsets.end,
		);
		...
		if (patched) {
			...
			const patchedPara = patched.substring(
			...
			if (result) {
				...
				for (const para of paragraphs.slice(0, 10)) {
					if (para.paraId && para.paraId !== target.paraId) {
						...
						const resultOffsets = findParagraphOffsets(resultXml, para.paraId);
						if (origOffsets && resultOffsets) {
							const origText = originalXml.substring(
								origOffsets.start,
								origOffsets.end,
							);
							const resultText = resultXml.substring(
								resultOffsets.start,
								resultOffsets.end,
							);
	...
	test("multiple paragraphs edited selectively", async () => {
		...
		if (result) {
			...
			if (uneditedPara?.paraId) {
				...
				if (origOffsets && resultOffsets) {
					expect(
						resultXml.substring(resultOffsets.start, resultOffsets.end),
					).toBe(originalXml.substring(origOffsets.start, origOffsets.end));
				}
			}
		}
	});
