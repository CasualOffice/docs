...
const blockOf = (xml: string, id: number) =>
	new RegExp(
		`<w:endnote\\b[^>]*\\bw:id="${id}"[^>]*>[\\s\\S]*?</w:endnote>`,
	).exec(xml)![0];
...
describe("endnoteSerializer (text-surgery, opt-in)", () => {
	test("replaces only the edited endnote text; marker + others byte-identical", async () => {
		...
		const edited = map.endnotes.find(
			(e) =>
				(e.noteType ?? "normal") === "normal" &&
				getEndnoteText(e).trim().length > 0,
		)!;
		...
		expect(getEndnoteText(parseEndnotes(out).byId.get(edited.id)!)).toBe(
			"UNIQUE_ENDNOTE_EDIT_99",
		);
	});
