...
export function serializeTextFormatting(
	...
	if (formatting.noProof !== undefined) {
		parts.push(
			formatting.noProof ? "<w:noProof/>" : '<w:noProof w:val="false"/>',
		);
	}
	if (formatting.webHidden !== undefined) {
		parts.push(
			formatting.webHidden ? "<w:webHidden/>" : '<w:webHidden w:val="false"/>',
		);
	}
...
function serializeFill(fill: ShapeFill | undefined): string {
	...
	if (fill.type === "gradient" && fill.gradient) {
		...
		const direction =
			g.type === "linear"
				? `<a:lin ang="${(g.angle || 0) * 60000}" scaled="1"/>`
				: "";
