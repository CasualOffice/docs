...
function serializeCellMargins(
	...
	if (margins.left) {
		...
		parts.push(
			serializeMeasurement(
				margins.left,
				margins.useLogicalSides ? "start" : "left",
			),
		);
	}
	...
	if (margins.right) {
		parts.push(
			serializeMeasurement(
				margins.right,
				margins.useLogicalSides ? "end" : "right",
			),
		);
	}
...
export function serializeTableRowFormatting(
	...
	if (formatting) {
		...
		if (formatting.conditionalFormat) {
			const cnfXml = serializeConditionalFormatStyle(
	...
	if (table.trailingBookmarks) {
		for (const marker of table.trailingBookmarks) {
			if (marker.type === "bookmarkStart") {
				const colFirst =
					marker.colFirst !== undefined
						? ` w:colFirst="${marker.colFirst}"`
						: "";
				const colLast =
					marker.colLast !== undefined ? ` w:colLast="${marker.colLast}"` : "";
				parts.push(
