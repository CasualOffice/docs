	...
	if (rFonts) {
		...
		if (eastAsiaTheme) {
			...
			if (theme && !formatting.fontFamily.eastAsia) {
				formatting.fontFamily.eastAsia = resolveThemeFontRef(
					theme,
					eastAsiaTheme,
				);
			}
	...
	if (spacing) {
		...
		if (lineRule)
			formatting.lineSpacingRule =
				lineRule as ParagraphFormatting["lineSpacingRule"];
...
function parseTableCellProperties(
	...
	if (textDirection) {
		...
		if (val)
			formatting.textDirection = val as TableCellFormatting["textDirection"];
	}
	...
	for (const key of Object.keys(source) as (keyof ParagraphFormatting)[]) {
		...
		if (value !== undefined) {
			if (key === "runProperties") {
				result.runProperties = mergeTextFormatting(
	...
	try {
		...
		if (latentStylesEl) {
			result.latentStyles = {
				...
				defUIPriority: parseNumericAttribute(
					latentStylesEl,
					"w",
					"defUIPriority",
				),
