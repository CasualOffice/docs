...
describe("resolveListTemplate", () => {
	...
	describe("multi-level templates", () => {
		test("resolves %1.%2. with two-level counters", () => {
			expect(
				resolveListTemplate("%1.%2.", [1, 1, 0], ["decimal", "decimal"]),
			).toBe("1.1.");
			expect(
				resolveListTemplate("%1.%2.", [2, 3, 0], ["decimal", "decimal"]),
			).toBe("2.3.");
		});
		...
		test("mixes formats per level (upperRoman parent + decimal child)", () => {
			expect(
				resolveListTemplate("%1.%2.", [3, 2], ["upperRoman", "decimal"]),
			).toBe("III.2.");
		});
	});
	...
	describe("edge cases", () => {
		...
		test('"%1.%2." at level 0 renders "1." not "1.."', () => {
			expect(
				resolveListTemplate("%1.%2.", [1, 0], ["decimal", "decimal"]),
			).toBe("1.");
		});
		...
		test('"none" numFmt mid-template only drops its own segment', () => {
			expect(resolveListTemplate("%1.%2.", [3, 5], ["decimal", "none"])).toBe(
				"3.",
			);
		});
	});
