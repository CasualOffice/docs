...
function emptyParagraph(
	attrs: Partial<ParagraphBlock["attrs"]> = {},
): ParagraphBlock {
	return {
		kind: "paragraph",
		id: "b1",
...
describe("hashParagraphBlock", () => {
	...
	test("empty paragraphs with different defaultFontFamily produce different hashes", () => {
		const inter = hashParagraphBlock(
			emptyParagraph({ defaultFontFamily: "Inter" }),
		);
		const georgia = hashParagraphBlock(
			emptyParagraph({ defaultFontFamily: "Georgia" }),
		);
	...
	test("empty paragraph with no defaults differs from one with defaults", () => {
		...
		const withSize = hashParagraphBlock(
			emptyParagraph({ defaultFontSize: 48 }),
		);
	...
	test("a field run changes the hash (no collision with a field-less paragraph)", () => {
		...
		expect(hashParagraphBlock(space)).not.toBe(
			hashParagraphBlock(spaceThenField),
		);
	});
	...
	test("different field types produce different hashes", () => {
		const mk = (fieldType: string): ParagraphBlock => ({
			...emptyParagraph(),
			runs: [
				{ kind: "field", fieldType, fallback: "" },
			] as ParagraphBlock["runs"],
		});
		expect(hashParagraphBlock(mk("DATE"))).not.toBe(
			hashParagraphBlock(mk("TIME")),
		);
	});
});
