		...
		switch (mark.type.name) {
			...
			case "underline": {
				...
				if (attrs.style || attrs.color) {
					const underlineColor = attrs.color
						? resolveColor(attrs.color, theme)
						: undefined;
			...
			case "footnoteRef": {
				...
				const id =
					typeof attrs.id === "string" ? parseInt(attrs.id, 10) : attrs.id;
	...
	node.forEach((child, childOffset) => {
		...
		if (child.isText && child.text) {
			...
			const run: ImageRun = {
				...
				displayMode: attrs.displayMode as
					| "inline"
					| "block"
					| "float"
					| undefined,
			...
			const display =
				(child.attrs.display as string) === "block" ? "block" : "inline";
			...
			const mathml =
				authoredMathml ||
				(ommlXml
					? (ommlToMathml(ommlXml, { display }) ?? undefined)
					: undefined);
			const run: TextRun = {
				kind: "text",
				text,
...
function computeListMarker(
	...
	if (pmAttrs.listMarker && pmAttrs.listMarker.includes("%")) {
		return resolveListTemplate(
			pmAttrs.listMarker,
			counters,
			pmAttrs.listLevelNumFmts ?? undefined,
		);
	}
...
function convertParagraphAttrs(
	...
	if (dtf) {
		...
		if (dtf.fontFamily) {
			attrs.defaultFontFamily = (dtf.fontFamily.ascii ||
				dtf.fontFamily.hAnsi) as string | undefined;
		}
	}
...
function convertTable(
	...
	const tableCellMargins = (node.attrs.resolvedCellMargins ??
		node.attrs.cellMargins) as
		| { top?: number; bottom?: number; left?: number; right?: number }
		| undefined;
	...
	const originalFormatting = node.attrs._originalFormatting as
		...
		| undefined;
	const leftCellMarginTwips =
		tableCellMargins?.left ?? DEFAULT_LEFT_CELL_MARGIN_TWIPS;
	const indentPx =
	...
	const floatingPx = floating
		? {
				...
				tblpX:
					floating.tblpX !== undefined
						? twipsToPixels(floating.tblpX)
						: undefined,
				tblpXSpec: floating.tblpXSpec,
				tblpY:
					floating.tblpY !== undefined
						? twipsToPixels(floating.tblpY)
						: undefined,
				tblpYSpec: floating.tblpYSpec,
				topFromText:
					floating.topFromText !== undefined
						? twipsToPixels(floating.topFromText)
						: undefined,
				bottomFromText:
					floating.bottomFromText !== undefined
						...
						: undefined,
				leftFromText:
					floating.leftFromText !== undefined
						? twipsToPixels(floating.leftFromText)
						: undefined,
				rightFromText:
					floating.rightFromText !== undefined
						? twipsToPixels(floating.rightFromText)
						: undefined,
			}
		: undefined;
			...
			case "paragraph":
				{
					...
					if (secProps || pmAttrs.sectionBreakType) {
						...
						if (secProps) {
							...
							if (
								secProps.pageWidth !== undefined ||
								secProps.pageHeight !== undefined
							) {
							...
							if (
								...
								const mergedTwips = {
									...
									bottom:
										secProps.marginBottom ?? lastSectionMarginsTwips.bottom,
							...
							if (colCount > 1) {
								...
								if (
									...
									secProps.columns.every(
										(c) => typeof c.width === "number" && c.width > 0,
									)
								) {
