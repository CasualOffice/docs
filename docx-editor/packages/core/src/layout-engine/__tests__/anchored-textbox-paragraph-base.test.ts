...
describe("paragraph-anchored textbox base", () => {
	test("anchors to the source paragraph TOP, ordered by offsetV", () => {
		...
		const tbTop = frags.find(
			(f) =>
				f.kind === "textBox" && (f as TextBoxFragment).blockId === "tb-top",
		) as TextBoxFragment;
		const tbBottom = frags.find(
			(f) =>
				f.kind === "textBox" && (f as TextBoxFragment).blockId === "tb-bottom",
		) as TextBoxFragment;
	...
	test("falls back to the flow cursor when no paragraph precedes the box", () => {
		...
		const layout = layoutDocument([tb.block], [tb.measure], {
			pageSize: PAGE,
			margins: MARGINS,
		});
		const frag = layout.pages[0].fragments.find(
			(f) => f.kind === "textBox",
		) as TextBoxFragment;
