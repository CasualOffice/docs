		...
		switch (block.kind) {
			...
			case "sectionBreak": {
				...
				if (nextType === "continuous" && nextCols && nextCols.count > 1) {
					const regionHeight = computeColumnRegionHeight(
						blocks,
						measures,
						i,
						nextCols.count,
		...
		if (lines.length >= 2 && fittingLines > 0) {
			...
			if (remainingAfter === 1) {
				...
				if (fittingLines - 1 >= minThisSide) {
					...
					linesHeight = 0;
					for (
						let j = currentLineIndex;
						j < currentLineIndex + fittingLines;
						j++
					) {
...
function layoutTable(
	...
	while (currentRowIndex < rows.length) {
		...
		const fragment: TableFragment = {
			...
			continuesOnNext: !isLastFragment,
			headerRowCount:
				!isFirstFragment && headerRowCount > 0 ? headerRowCount : undefined,
		};
...
function collectBehindDocCluster(
	...
	for (let k = startIndex; k < blocks.length; k++) {
		const b = blocks[k];
		if (b.kind !== "textBox" || !reservesBehindDocBand(b as TextBoxBlock))
			break;
...
function layoutAnchoredTextBox(
	...
	const toEmu = (px: number | undefined) =>
		px != null ? pixelsToEmu(px) : undefined;
	const hSpec = {
	...
	const isParaRelative =
		anchor.relFromV === "paragraph" || anchor.relFromV === "line";
	const paraTopOnThisPage =
		anchorParagraphTop !== undefined &&
		anchorParagraphTop >= margins.top &&
		anchorParagraphTop <= state.cursorY;
	const baseY =
		isParaRelative && paraTopOnThisPage
			? (anchorParagraphTop as number)
			: state.cursorY;
