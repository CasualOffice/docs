...
describe("Layout Engine - Page Production", () => {
	describe("single page scenarios", () => {
		...
		test("uses the next section page size and margins after a section break", () => {
			...
			const firstSectionMargins: PageMargins = {
				top: 80,
				right: 80,
				bottom: 80,
				left: 80,
			};
			const secondSectionMargins: PageMargins = {
				top: 40,
				right: 120,
				bottom: 40,
				left: 120,
...
describe("Layout Engine - Behind-doc anchored text box flow reservation", () => {
	...
	function makeTextBoxMeasure(
		width: number,
		height: number,
	): import("./types").TextBoxMeasure {
	...
	function layoutWithTrailingParagraph(
		...
		const layout = layoutDocument(blocks, measures, makeLayoutOptions());
		const frags = layout.pages[0].fragments;
		const headingFrag = frags.find(
			(f) => f.kind === "paragraph" && f.blockId === 0,
		)!;
		const trailingFrag = frags.find(
			(f) => f.kind === "paragraph" && f.blockId === 200,
		)!;
		return {
			firstY: headingFrag.y + headingFrag.height,
			trailingY: trailingFrag.y,
		};
	}
	...
	test("the behind-doc cluster fragments are still painted (behind, z-index -1)", () => {
		...
		const layout = layoutDocument(blocks, measures, makeLayoutOptions());
		const textBoxFrags = layout.pages[0].fragments.filter(
			(f) => f.kind === "textBox",
		);
