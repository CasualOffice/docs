...
describe("border overlay layout", () => {
	...
	test("page borders render as a text-relative inset overlay with visible double lines", () => {
		...
		expect(border).toBeTruthy();
		expect(parseFloat(border!.style.top)).toBeCloseTo(
			54 - pointsToPixels(15),
			5,
		);
		expect(parseFloat(border!.style.left)).toBeCloseTo(
			54 - pointsToPixels(15),
			5,
		);
		expect(parseFloat(border!.style.bottom)).toBeCloseTo(
			52 - pointsToPixels(11),
			5,
		);
		expect(parseFloat(border!.style.right)).toBeCloseTo(
			36 - pointsToPixels(2),
			5,
		);
