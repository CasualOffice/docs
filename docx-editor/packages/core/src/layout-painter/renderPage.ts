...
function renderLineNumberGutter(
	...
	const distancePx = twipsToPixels(
		ln.distance ?? DEFAULT_LINE_NUMBER_DISTANCE_TWIPS,
	);
...
export function renderPage(
	...
	const behindFloatingImages = allFloatingImages.filter(
		floatingImageIsBehindDoc,
	);
	const frontFloatingImages = allFloatingImages.filter(
			...
			if (
				...
				const nextBorders =
					i + 1 < page.fragments.length
						? getParaBorders(page.fragments[i + 1])
						: undefined;
...
export function renderPages(
	...
	if (canIncremental) {
		...
		for (let i = 0; i < commonCount; i++) {
			...
			const sizeChanged =
				!oldPage ||
				oldPage.size.w !== newPage.size.w ||
				oldPage.size.h !== newPage.size.h;
			if (sizeChanged) {
