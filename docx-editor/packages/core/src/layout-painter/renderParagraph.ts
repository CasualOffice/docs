...
function applyRunStyles(
	...
	if (run.textOutline) {
		...
		(
			element.style as CSSStyleDeclaration & { webkitTextFillColor?: string }
	...
	const changeColor =
		run.isInsertion || run.isDeletion ? colorForAuthor(run.changeAuthor) : null;
	...
	if (run.isInsertion) {
		element.style.backgroundColor =
			changeColor?.bg ?? "rgba(52, 168, 83, 0.08)";
	...
	if (run.isDeletion) {
		...
		element.style.backgroundColor =
			changeColor?.bg ?? "rgba(211, 47, 47, 0.08)";
 ...
 */
export function renderInlineImageRun(
	run: ImageRun,
	doc: Document,
): HTMLElement {
		...
		if (isTextRun(run)) {
			...
			if (startChar > 0 || endChar < run.text.length) {
				...
				result.push({
					...
					text: slicedText,
					pmStart:
						run.pmStart !== undefined ? run.pmStart + startChar : undefined,
...
export function renderParagraphFragment(
	...
	fragmentEl.style.position =
		context.positioning === "absolute" ? "absolute" : "relative";
