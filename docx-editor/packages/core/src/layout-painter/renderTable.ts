...
function renderCellContent(
	...
	if (cellFloatingImages.length > 0) {
		...
		const behindFloatingImages = cellFloatingImages.filter(
			floatingImageIsBehindDoc,
		);
	...
	for (let i = 0; i < cell.blocks.length; i++) {
		...
		if (block?.kind === "paragraph" && measure?.kind === "paragraph") {
			...
			cumulativeY +=
				effectiveSpaceBefore + ((measure as TableMeasure).totalHeight ?? 0);
	...
	const frontFloatingImages = cellFloatingImages.filter(
		(img) => !floatingImageIsBehindDoc(img),
	);
...
function computeWordCompatClosingBorders(
	...
	const out: Array<CellBorderSpec | undefined> = new Array(columnCount).fill(
		undefined,
	);
	let columnIndex = 0;
...
export function renderTableFragment(
	...
	tableEl.style.position =
		context.positioning === "flow" ? "relative" : "absolute";
