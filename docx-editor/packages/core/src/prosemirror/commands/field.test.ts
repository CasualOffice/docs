...
function stateWithCursorIn(text: string, offset: number): EditorState {
	...
	state = state.apply(
		state.tr.setSelection(TextSelection.create(state.doc, 1 + offset)),
	);
	return state;
}

function findField(state: EditorState): {
	fieldType: string;
	instruction: string;
	fieldKind: string;
	dirty: boolean;
...
describe("insertField", () => {
	...
	test("returns false when the schema has no field node", () => {
		...
		const doc = bare.node("doc", null, [
			bare.node("paragraph", null, [bare.text("x")]),
		]);
