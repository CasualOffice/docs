...
function collectPMAnchors(table: PMNode): {
	anchors: CellAnchor<PMNode>[];
	totalCols: number;
} {
...
function getExistingColumnWidths(table: PMNode, totalCols: number): number[] {
	...
	return Array.isArray(table.attrs.columnWidths) &&
		...
		: Array.from({ length: totalCols }, () =>
				Math.floor(tableWidth / Math.max(totalCols, 1)),
			);
}
...
export function getSplitCellDialogConfig(
	state: EditorState,
): SplitCellDialogConfig | null {
...
export function splitActiveTableCell(
	...
	if (targetRow != null && targetCol != null) {
		if (targetRow !== resolved.row || targetCol !== resolved.col) {
			...
			const originalTarget = anchors.find(
	...
	const rowAttrs = Array.from({ length: layout.newRowCount }, (_, rowIndex) => {
		...
		return {
			...(activeCell.table.child(rowIndex - layout.deltaRows)?.attrs ?? {}),
		};
	});

	// Sort anchors and build row children
	const rowChildren = Array.from(
		{ length: layout.newRowCount },
		() => [] as PMNode[],
	);
	layout.anchors
		...
		.forEach((anchor) => {
			...
			rowChildren[anchor.row].push(
				anchor.data.type.create(attrs, anchor.data.content),
			);
		});

	const rowNodes = rowChildren.map((cells, rowIndex) =>
		activeCell.table.type.schema.nodes.tableRow.create(
			rowAttrs[rowIndex],
			cells,
		),
	);
	...
	if (replacedTable) {
		...
		if (selectionCellPos != null) {
			tr = tr.setSelection(
				TextSelection.near(tr.doc.resolve(selectionCellPos + 1)),
			);
		}
	}
