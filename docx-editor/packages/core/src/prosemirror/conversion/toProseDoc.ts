...
function paragraphFormattingToAttrs(
	...
	const attrs: ParagraphAttrs = {
		...
		listMarkerFontFamily:
			paragraph.listRendering?.markerFontFamily || undefined,
	...
	if (styleResolver) {
		...
		attrs.defaultTextFormatting = resolveTextFormatting(
			formatting?.runProperties,
			styleResolver,
		);
	}
...
function convertTable(
	...
	const resolvedCellMarginsAttr = cellMarginSources.some(
		(source) => source != null,
	)
		? {
	...
	const conditionalStyles = {
		wholeTable: resolveTableStyleConditional(
			styleResolver,
			tableStyleId,
			"wholeTable",
		),
		firstRow: resolveTableStyleConditional(
			styleResolver,
			tableStyleId,
			"firstRow",
		),
		lastRow: resolveTableStyleConditional(
			styleResolver,
			tableStyleId,
			"lastRow",
		),
		firstCol: resolveTableStyleConditional(
			styleResolver,
			tableStyleId,
			"firstCol",
		),
		lastCol: resolveTableStyleConditional(
			styleResolver,
			tableStyleId,
			"lastCol",
		),
		band1Horz: resolveTableStyleConditional(
			styleResolver,
			tableStyleId,
			"band1Horz",
		),
		band2Horz: resolveTableStyleConditional(
			styleResolver,
			tableStyleId,
			"band2Horz",
		),
		band1Vert: resolveTableStyleConditional(
			styleResolver,
			tableStyleId,
			"band1Vert",
		),
		band2Vert: resolveTableStyleConditional(
			styleResolver,
			tableStyleId,
			"band2Vert",
		),
...
function convertRunContent(
	...
	switch (content.type) {
		...
		case "symbol": {
			...
			if (
				isTranslatedUnicode(ch, content.font, content.char) &&
				!isTallSymbolFont(content.font)
			) {
...
function symbolToUnicodeChar(font: string, char: string): string | null {
	...
	const wingdings2: Record<number, string> = {
		163: "☐", // ☐ empty checkbox
		82: "☑", // ☑ checked checkbox (boxed tick)
		83: "☒", // ☒ x-marked checkbox
		168: "☐", // ☐ alt empty
		251: "☒", // ☒ alt x
		252: "☑", // ☑ alt check
	};
...
function convertImage(image: Image): PMNode {
	...
	const effectExtentTop = image.padding?.top
		? emuToPixels(image.padding.top)
		: undefined;
	const effectExtentBottom = image.padding?.bottom
		? emuToPixels(image.padding.bottom)
		: undefined;
	const effectExtentLeft = image.padding?.left
		? emuToPixels(image.padding.left)
		: undefined;
	const effectExtentRight = image.padding?.right
		? emuToPixels(image.padding.right)
		: undefined;
...
function textFormattingToMarks(
	...
	if (
		formatting.color &&
		(!formatting.color.auto || formatting.color.themeColor)
	) {
		marks.push(
...
function convertShape(shape: Shape): PMNode {
	...
	const posOffsetH =
		posH?.posOffset != null ? emuToPixels(posH.posOffset) : null;
	const posOffsetV =
		posV?.posOffset != null ? emuToPixels(posV.posOffset) : null;
...
function convertTextBox(
	...
	if (
		textBox.outline &&
		(textBox.outline.width || resolveShapeColor(textBox.outline.color))
	) {
	...
	const marginTop =
		textBox.margins?.top != null ? emuToPixels(textBox.margins.top) : 4;
	const marginBottom =
		textBox.margins?.bottom != null ? emuToPixels(textBox.margins.bottom) : 4;
	const marginLeft =
		textBox.margins?.left != null ? emuToPixels(textBox.margins.left) : 7;
	const marginRight =
		textBox.margins?.right != null ? emuToPixels(textBox.margins.right) : 7;
 ...
 * fresh page.
 */
function paragraphForcedBreakType(
	paragraph: Paragraph,
): "page" | "column" | null {
