 ...
 */
export function extractParagraphAttrsFromStyle(
	element: HTMLElement,
	...
	parseDOM: [
		{
			...
			getAttrs(dom): ParagraphAttrs {
				...
				const attrs: ParagraphAttrs = {
					...
					alignment: element.dataset.alignment as
						| ParagraphAlignment
						| undefined,
					styleId: element.dataset.styleId || undefined,
					sectionBreakType:
						(element.dataset
							.sectionBreak as ParagraphAttrs["sectionBreakType"]) || undefined,
				};
						...
						for (const h of headings) {
							...
							if (paragraphNode && paragraphNode.type.name === "paragraph") {
								...
								const existingBookmarks =
									(paragraphNode.attrs.bookmarks as Array<{
										id: number;
										name: string;
									}>) || [];
