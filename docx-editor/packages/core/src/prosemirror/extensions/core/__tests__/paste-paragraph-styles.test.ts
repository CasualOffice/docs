...
describe("parseDOM CSS extraction — Google Docs paste", () => {
	test('text-align: center → alignment "center"', () => {
		expect(
			extractParagraphAttrsFromStyle(paraWithStyle("text-align: center"))
				.alignment,
		).toBe("center");
	});
	...
	test('text-align: justify → alignment "both"', () => {
		expect(
			extractParagraphAttrsFromStyle(paraWithStyle("text-align: justify"))
				.alignment,
		).toBe("both");
	});
	...
	test("text-align: right / left / start / end map correctly", () => {
		expect(
			extractParagraphAttrsFromStyle(paraWithStyle("text-align: right"))
				.alignment,
		).toBe("right");
		expect(
			extractParagraphAttrsFromStyle(paraWithStyle("text-align: left"))
				.alignment,
		).toBe("left");
		expect(
			extractParagraphAttrsFromStyle(paraWithStyle("text-align: start"))
				.alignment,
		).toBe("left");
		expect(
			extractParagraphAttrsFromStyle(paraWithStyle("text-align: end"))
				.alignment,
		).toBe("right");
	});
	...
	test("margin-left: 30pt → indentLeft 600 twips (30pt × 20)", () => {
		expect(
			extractParagraphAttrsFromStyle(paraWithStyle("margin-left: 30pt"))
				.indentLeft,
		).toBe(600);
	});
	...
	test("margin-right: 0.5in → indentRight 720 twips (1in = 1440 twips)", () => {
		expect(
			extractParagraphAttrsFromStyle(paraWithStyle("margin-right: 0.5in"))
				.indentRight,
		).toBe(720);
	});
	...
	test("text-indent: -18pt → indentFirstLine 360 + hangingIndent true", () => {
		const attrs = extractParagraphAttrsFromStyle(
			paraWithStyle("text-indent: -18pt"),
		);
	...
	test("text-indent: 24pt → indentFirstLine 480, no hanging", () => {
		const attrs = extractParagraphAttrsFromStyle(
			paraWithStyle("text-indent: 24pt"),
		);
	...
	test('line-height: 18pt → lineSpacing 360, rule "exact"', () => {
		const attrs = extractParagraphAttrsFromStyle(
			paraWithStyle("line-height: 18pt"),
		);
		expect(attrs.lineSpacing).toBe(360);
	...
	test("line-height: normal → no spacing extracted", () => {
		const attrs = extractParagraphAttrsFromStyle(
			paraWithStyle("line-height: normal"),
		);
	...
	test("empty style yields empty attrs object", () => {
		expect(
			Object.keys(extractParagraphAttrsFromStyle(paraWithStyle(""))).length,
		).toBe(0);
	});
});
