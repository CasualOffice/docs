...
export const spellcheckPluginKey = new PluginKey<{
	version: number;
	decos: DecorationSet;
}>("spellcheck");
			...
			plugins: [
				new Plugin({
					...
					state: {
						init(_, { doc }) {
							return {
								version: checker?.version() ?? 0,
								decos: buildDecorations(doc),
							};
						},
						apply(tr, prev) {
							...
							const forceRefresh =
								tr.getMeta(spellcheckPluginKey) === "refresh";
						...
						return {
							update(view: EditorView, prevState) {
								...
								timer = setTimeout(() => {
									...
									if ((view as unknown as { docView: unknown }).docView == null)
										return;
									view.dispatch(
										view.state.tr.setMeta(spellcheckPluginKey, "refresh"),
									);
					...
					props: {
						decorations(state) {
							return (
								spellcheckPluginKey.getState(state)?.decos ??
								DecorationSet.empty
							);
						},
					},
