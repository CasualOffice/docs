	...
	switch (target) {
		...
		case "squareRight": {
			const cssFloat: ImageAttrs["cssFloat"] =
				target === "squareLeft" ? "left" : "right";
			// Image-on-left ⇒ text flows on the right, and vice versa.
			const wrapText: ImageAttrs["wrapText"] =
				target === "squareLeft" ? "right" : "left";
				...
				getAttrs(dom): ImageAttrs {
					...
					return {
						...
						displayMode:
							...
							"inline",
						cssFloat:
							(element.dataset.cssFloat as ImageAttrs["cssFloat"]) || undefined,
