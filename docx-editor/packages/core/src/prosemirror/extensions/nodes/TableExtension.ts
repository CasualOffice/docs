	...
	parseDOM: [
		{
			...
			getAttrs(dom): TableAttrs {
				...
				return {
					...
					justification: element.dataset.justification as
						| TableAttrs["justification"]
						| undefined,
				};
			},
		},
...
function getTableContext(state: EditorState): TableContextInfo {
	...
	const hasMultiCellSelection =
		isCellSel && selection.$anchorCell.pos !== selection.$headCell.pos;
	...
	return (state, dispatch) => {
		...
		if (dispatch) {
			...
			const newRow = schema.nodes.tableRow.create(
				{
					height: row.attrs.height ?? 360,
					heightRule: row.attrs.heightRule ?? "atLeast",
				},
				newCells,
			);
	...
	onSchemaReady(ctx: ExtensionContext): ExtensionRuntime {
		...
		function insertTable(rows: number, cols: number): Command {
			return (state, dispatch) => {
				...
				if (dispatch) {
					...
					tr.setSelection(
						Selection.near(tr.doc.resolve(firstCellContentPos), 1),
					);
					dispatch(tr.scrollIntoView());
				}
		...
		function selectColumn(
			...
			if (dispatch) {
				...
				const cellPosForColumn = (
					rowNode: PMNode,
					rowInnerStart: number,
				): number => {
		...
		function sortTable(direction: "asc" | "desc"): Command {
			return (state, dispatch) => {
				...
				const sorted = [...sortable].sort((a, b) => {
					...
					const nb = Number(kb);
					const bothNumeric =
						ka !== "" && kb !== "" && !Number.isNaN(na) && !Number.isNaN(nb);
		...
		function setTableProperties(props: {
			...
			return (state, dispatch) => {
				...
				if (dispatch) {
					...
					if (
						props.bandedRows !== undefined ||
						props.bandedColumns !== undefined
					) {
						const currentLook =
							(newAttrs.look as {
								firstRow?: boolean;
