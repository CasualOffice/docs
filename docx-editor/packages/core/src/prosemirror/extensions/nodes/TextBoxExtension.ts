	...
	nodeSpec: {
		...
		parseDOM: [
			{
				...
				getAttrs(dom): TextBoxAttrs {
					...
					return {
						...
						outlineWidth: el.dataset.outlineWidth
							? Number(el.dataset.outlineWidth)
							: undefined,
						outlineColor: el.dataset.outlineColor || undefined,
						outlineStyle: el.dataset.outlineStyle || undefined,
						marginTop: el.dataset.marginTop
							? Number(el.dataset.marginTop)
							: undefined,
						marginBottom: el.dataset.marginBottom
							? Number(el.dataset.marginBottom)
							: undefined,
						marginLeft: el.dataset.marginLeft
							? Number(el.dataset.marginLeft)
							: undefined,
						marginRight: el.dataset.marginRight
							? Number(el.dataset.marginRight)
							: undefined,
						verticalAlign: el.dataset.verticalAlign || undefined,
						displayMode:
							(el.dataset.displayMode as TextBoxAttrs["displayMode"]) ||
							undefined,
						cssFloat:
							(el.dataset.cssFloat as TextBoxAttrs["cssFloat"]) || undefined,
						wrapType: el.dataset.wrapType || undefined,
						autoFit:
							(el.dataset.autoFit as TextBoxAttrs["autoFit"]) || undefined,
						posOffsetH: el.dataset.posOffsetH
							? Number(el.dataset.posOffsetH)
							: undefined,
						posOffsetV: el.dataset.posOffsetV
							? Number(el.dataset.posOffsetV)
							: undefined,
		...
		toDOM(node) {
			...
			if (attrs.height) {
				...
				styles.push(
					isFilledRule
						? `height: ${Math.max(attrs.height, 1)}px`
