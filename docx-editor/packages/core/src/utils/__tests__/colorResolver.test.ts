...
describe("resolveColor — OOXML theme color resolution", () => {
	...
	test('themeTint "FF" (255) keeps original color', () => {
		...
		const result = resolveColor(
			{ themeColor: "accent1", themeTint: "FF" },
			theme,
		);
		expect(result).toBe("#4472C4");
	});
	...
	test('themeTint "00" (0) produces white', () => {
		const result = resolveColor(
			{ themeColor: "accent1", themeTint: "00" },
			theme,
		);
		expect(result).toBe("#FFFFFF");
	});
	...
	test('themeTint "33" (0x33 = 20%) produces near-white with slight color', () => {
		...
		const result = resolveColor(
			{ themeColor: "accent1", themeTint: "33" },
			theme,
		);
	...
	test('themeTint "99" (0x99 = 60%) produces medium-light variant', () => {
		...
		const result = resolveColor(
			{ themeColor: "accent1", themeTint: "99" },
			theme,
		);
	...
	test('themeShade "FF" (255) keeps original color', () => {
		const result = resolveColor(
			{ themeColor: "accent1", themeShade: "FF" },
			theme,
		);
		expect(result).toBe("#4472C4");
	});
	...
	test('themeShade "00" (0) produces black', () => {
		const result = resolveColor(
			{ themeColor: "accent1", themeShade: "00" },
			theme,
		);
		expect(result).toBe("#000000");
	});
	...
	test('themeShade "F2" (0x F2 ≈ 95%) slightly darkens color', () => {
		...
		const result = resolveColor(
			{ themeColor: "accent1", themeShade: "F2" },
			theme,
		);
...
describe("resolveColorToHex — shared display-side color resolution", () => {
	...
	test("applies themeTint correctly", () => {
		expect(
			resolveColorToHex({ themeColor: "accent1", themeTint: "33" }, theme),
		).toBe("DAE3F3");
	});

	test("applies themeShade correctly", () => {
		expect(
			resolveColorToHex({ themeColor: "background1", themeShade: "F2" }, theme),
		).toBe("F2F2F2");
	});
	...
	test("themeColor without theme falls back to rgb if present", () => {
		expect(
			resolveColorToHex(
				{ themeColor: "accent1", themeTint: "33", rgb: "DAE3F3" },
				undefined,
			),
		).toBe("DAE3F3");
	});
...
describe("resolveColor — DrawingML lumMod / lumOff (HSL luminance)", () => {
	test("lumMod 0.85 on white → light gray (the bg1-border case)", () => {
		...
		expect(resolveColor({ rgb: "FFFFFF", themeLumMod: 0.85 }, null)).toBe(
			"#D9D9D9",
		);
	});
	...
	test("lumMod keeps chromatic colors chromatic (red, not washed to gray)", () => {
		...
		expect(resolveColor({ rgb: "FF0000", themeLumMod: 0.75 }, null)).toBe(
			"#BF0000",
		);
	});
	...
	test("lumMod then lumOff combine (black + lumMod0.5 + lumOff0.6 → mid gray)", () => {
		...
		expect(
			resolveColor({ rgb: "000000", themeLumMod: 0.5, themeLumOff: 0.6 }, null),
		).toBe("#999999");
	});
