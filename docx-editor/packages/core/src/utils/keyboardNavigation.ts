...
export function findVisualLineStart(
	...
	if (container.nodeType === Node.TEXT_NODE) {
		...
		range.setStart(
			container,
			Math.min(offset, container.textContent?.length || 0),
		);
		range.collapse(true);
 ...
 */
export function moveToLineEdge(
	edge: "start" | "end",
	extend: boolean = false,
): boolean {
	const selection = window.getSelection();
...
export function getNavigationShortcutDescriptions(): Array<{
	...
	return [
		{
			action: "Move by word (left)",
			shortcut: describeShortcut(NAVIGATION_SHORTCUTS.wordLeft),
		},
		{
			action: "Move by word (right)",
			shortcut: describeShortcut(NAVIGATION_SHORTCUTS.wordRight),
		},
		{
		...
		},
		{
			...
			shortcut: describeShortcut(NAVIGATION_SHORTCUTS.lineStart),
		},
		{
			action: "Go to line end",
			shortcut: describeShortcut(NAVIGATION_SHORTCUTS.lineEnd),
		},
		{
		...
		{
			action: "Go to document end",
			shortcut: describeShortcut(NAVIGATION_SHORTCUTS.documentEnd),
		},
		{
