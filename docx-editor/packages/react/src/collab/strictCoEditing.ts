...
export const strictCoEditingKey = new PluginKey<StrictPluginState>(
	"strictCoEditing",
);
...
export function setStrictCoEditing(enabled: boolean) {
	return (
		...
		if (dispatch) {
			dispatch(
				state.tr.setMeta(strictCoEditingKey, {
					setEnabled: enabled,
				} satisfies StrictMeta),
			);
		}
		return true;
	};
...
export function createStrictCoEditingPlugin(
	options: StrictCoEditingOptions,
): Plugin {
	const {
		getLocks,
	...
	return new Plugin<StrictPluginState>({
		...
		state: {
			init(_config, state) {
				...
				return {
					enabled,
					locks,
					decorations: buildDecorations(state.doc, locks),
				};
			},
			apply(tr, prev, _oldState, newState) {
				...
				const shouldRefresh =
					meta?.setEnabled != null || meta?.refreshLocks || tr.docChanged;
				if (shouldRefresh) {
					const locks = getLocks(newState);
					return {
						enabled,
						locks,
						decorations: buildDecorations(newState.doc, locks),
					};
				}
				return {
					enabled,
		...
		props: {
			decorations(state) {
				return (
					strictCoEditingKey.getState(state)?.decorations ?? DecorationSet.empty
				);
			},
		},
		...
		view(view) {
			...
			const refresh = (): void => {
				...
				view.dispatch(
					view.state.tr.setMeta(strictCoEditingKey, { refreshLocks: true }),
				);
			};
