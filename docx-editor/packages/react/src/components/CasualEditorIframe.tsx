...
export const CasualEditorIframe = forwardRef<
	CasualEditorIframeRef,
	CasualEditorIframeProps
>(function CasualEditorIframe(props, ref) {
	...
	const onLoad = useCallback(
		async (req: { docId: string }): Promise<LoadResponseData> => {
			try {
				const { bytes, name, etag } = await fileSourceRef.current.open(
					req.docId,
				);
				return {
					ok: true,
	...
	const onSave = useCallback(
		async (req: {
			...
			try {
				const opts =
					req.baseEtag !== undefined ? { etag: req.baseEtag } : undefined;
				const { etag } = await fileSourceRef.current.save(
					req.docId,
					req.bytes,
					opts,
				);
	...
	if (ref) {
		...
		apiRef.current = {
			setViewMode: (mode) =>
				transportRef.current?.sendSetViewMode({ viewMode: mode }),
