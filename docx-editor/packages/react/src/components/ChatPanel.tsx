...
function selChipStyle(active: boolean): CSSProperties {
	return {
		...
		color: active
			...
			: "var(--doc-text-on-surface-muted, #5f6368)",
		cursor: "pointer",
	};
}
...
const SLASH_COMMANDS: SlashCommand[] = [
	...
	},
	{
		...
		build: () =>
			"Summarise the selected passage in a single concise paragraph.",
	},
	{
		cmd: "/grammar",
		label: "Grammar fix",
...
function matchSlash(
	value: string,
): { command: SlashCommand; rest: string } | null {
...
export function ChatPanel({
	...
	const [useDocContext, setUseDocContextState] = useState(() =>
		loadDocContextPref(),
	);
	...
	const onSend = async () => {
		...
		try {
			...
			if (result.kind === "chat") {
				...
				const where = result.replaceRange
					? "over your selection"
					: "below your cursor";
...
function ChatEmptyState({
	...
	return (
		<div style={emptyStyle}>
			...
			<span>
				Ask anything about the open document, request rewrites, or brainstorm.
				Toggle "Use document context" above to send the doc text along with your
				question.
			</span>
