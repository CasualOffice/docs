...
const MenuItem: React.FC<MenuItemProps> = ({
	...
	return (
		...
		>
			<span style={{ display: "flex", color: "var(--doc-text-muted)" }}>
				{icon}
			</span>
			<span>{label}</span>
		</button>
...
export const ContextMenu: React.FC<ContextMenuProps> = ({
	...
	const allActions = showCustomPrompt
		? [...actions, "custom" as AIAction]
		: actions;
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			switch (e.key) {
				...
				case "ArrowUp":
					e.preventDefault();
					setHighlightedIndex(
						(prev) => (prev - 1 + allActions.length) % allActions.length,
					);
					break;
				case "Enter": {
					e.preventDefault();
	...
	const getMenuStyle = useCallback((): React.CSSProperties => {
		...
		const menuHeight = Math.min(
			measured?.h ?? allActions.length * 36 + 16,
			maxH,
		);
