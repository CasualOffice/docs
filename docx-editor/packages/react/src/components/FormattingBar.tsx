...
export function FormattingBar(explicitProps: FormattingBarProps) {
	...
	const normalizedFonts = React.useMemo(
		() => normalizeFontFamilies(fontFamilies),
		[fontFamilies],
	);
				...
				<ToolbarGroup label={t("formattingBar.groups.listFormatting")}>
					...
					{showLineSpacingPicker && (
						<LineSpacingPicker
							...
							onSpaceBeforeChange={(twips) =>
								onFormat?.({ type: "spaceBefore", value: twips })
							}
							onSpaceAfterChange={(twips) =>
								onFormat?.({ type: "spaceAfter", value: twips })
							}
							...
							onTogglePagination={(key) => {
								const current =
									currentFormatting[key as keyof typeof currentFormatting];
								onFormat?.({
									type: key,
									value: !current,
								} as FormattingAction);
