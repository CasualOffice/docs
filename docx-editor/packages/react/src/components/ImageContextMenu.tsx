...
export const ImageContextMenu: React.FC<ImageContextMenuProps> = ({
	...
	const enabledLayout = useMemo(
		() =>
			IMAGE_LAYOUT_OPTIONS.filter((o) =>
				isImageLayoutOptionEnabled(o, currentWrapType),
			),
		[currentWrapType],
	);
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (e.key === "ArrowDown") {
				...
				if (highlightedIndex < enabledLayout.length) {
					...
					onApplyLayout(
						enabledLayout[highlightedIndex].choice as ImageLayoutTarget,
					);
					onClose();
				} else if (onTextAction) {
					const ta =
						navigableTextActions[highlightedIndex - enabledLayout.length];
					if (ta) {
	...
	const getMenuStyle = useCallback((): React.CSSProperties => {
		...
		const menuHeight =
			layoutRows * 36 + textRows * 32 + (textRows > 0 ? 9 : 0) + 16;
		let x = position.x;
		let y = position.y;
		...
		>
			{IMAGE_LAYOUT_OPTIONS.map((option: ImageLayoutOptionDef) => {
				...
				const navIdx = isEnabled
					? enabledLayout.findIndex((o) => o.choice === option.choice)
					: -1;
						...
						>
							<span
								style={{
									display: "flex",
									color: isCurrent
										? "var(--doc-primary, #2563eb)"
										: "var(--doc-text-muted, #666)",
									width: ICON_SIZE,
								}}
							>
								<MaterialSymbol
									name={ICON_BY_HINT[option.iconHint]}
									size={ICON_SIZE}
								/>
							</span>
							<span style={{ flex: 1 }}>
...
export function useImageContextMenu(): UseImageContextMenuReturn {
	...
	const [currentCssFloat, setCurrentCssFloat] =
		useState<ImageAttrsCssFloat>(null);
