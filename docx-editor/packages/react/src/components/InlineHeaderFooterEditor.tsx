...
>(function InlineHeaderFooterEditor(
	...
	const syncBoxPositions = useCallback(() => {
		...
		const viewBoxes = Array.from(
			targetElement.querySelectorAll<HTMLElement>(".layout-textbox"),
		);
		const overlayBoxes = Array.from(
			container.querySelectorAll<HTMLElement>(".docx-textbox"),
		);
		...
		const viewImgs = Array.from(
			targetElement.querySelectorAll<HTMLImageElement>("img"),
		).filter((i) => i.getBoundingClientRect().width > 0);
		const ovImgs = Array.from(
			container.querySelectorAll<HTMLImageElement>("img.docx-image"),
		);
	...
	useEffect(() => {
		...
		const navKeymap = keymap({
			...
			PageDown: (state, dispatch) => {
				...
				if (dispatch)
					dispatch(
						state.tr.setSelection(Selection.atEnd(state.doc)).scrollIntoView(),
					);
				return true;
			},
	...
	const label =
		position === "header" ? t("headerFooter.header") : t("headerFooter.footer");
