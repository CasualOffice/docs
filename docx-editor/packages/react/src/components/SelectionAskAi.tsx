...
const panelStyle: CSSProperties = {
	...
	borderRadius: 10,
	boxShadow:
		"0 12px 28px -8px rgba(15,23,42,0.22), 0 4px 10px -2px rgba(15,23,42,0.08)",
	padding: 10,
	display: "flex",
	...
	useEffect(() => {
		...
		const place = () => {
			...
			const top = fitsBelow
				...
				: fitsAbove
					? aboveTop
					: TOOLBAR_BOTTOM_PX + VIEWPORT_PAD;
			...
			<button
				...
				onClick={() => {
					...
					if (view) {
						...
						if (from !== to) {
							capturedSelectionRef.current = view.state.doc.textBetween(
								from,
								to,
								"\n",
								" ",
				...
				aria-label={
					busy ? "AI is processing your request" : "Ask AI about the selection"
				}
			>
				{busy ? (
