...
export function StatusBar({
	...
	useEffect(() => {
		...
		const onDown = (e: MouseEvent) => {
			if (!checklistWrapRef.current?.contains(e.target as Node))
	...
	const zoomIn = () =>
		onZoomChange?.(Math.min((zoom ?? 1) * ZOOM_STEP, maxZoom));
	const zoomOut = () =>
		onZoomChange?.(Math.max((zoom ?? 1) / ZOOM_STEP, minZoom));
								...
								{ZOOM_PRESETS.map((preset) => {
									...
									return (
										<button
											...
											role="menuitem"
											style={
												isActive ? zoomMenuItemActiveStyle : zoomMenuItemStyle
											}
											onClick={() => {
												onZoomChange?.(preset);
												setPresetsOpen(false);
