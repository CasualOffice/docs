...
const swatchStyle = (selected: boolean): CSSProperties => ({
	...
	aspectRatio: "1 / 1",
	border: selected
		? "2px solid var(--doc-accent, #2563eb)"
	...
	return (
		...
		>
			...
			<div style={bodyStyle}>
				...
				<div
					style={{
						borderTop: "1px solid var(--doc-border-light, #f0eee9)",
						paddingTop: 12,
					}}
				>
					<div style={sectionHeadStyle}>
					...
					<ColorField
						...
						onChange={(hex) =>
							commit({ ...value, shading: { ...value.shading, fillHex: hex } })
						}
						allowNone
					/>
							...
							{SHADING_PATTERNS.map((p) => (
								<button
									...
									onClick={() =>
										commit({
											...value,
											shading: { ...value.shading, pattern: p.value },
										})
									}
