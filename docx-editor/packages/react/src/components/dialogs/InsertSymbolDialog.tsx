...
export function InsertSymbolDialog({
	...
	const filteredSymbols = useMemo(() => {
		if (!searchQuery.trim()) {
			...
			const category = SYMBOL_CATEGORIES.find(
	...
	const categories = [
		...
		...SYMBOL_CATEGORIES.map((c) => ({
			name: c.name,
			label: categoryLabelMap[c.name] || c.label,
		})),
	];
						...
						{symbolInfo && (
							...
							>
								...
								<div style={PREVIEW_INFO_STYLE}>
									...
									>
										{t("dialogs.insertSymbol.decimal", {
											value: symbolInfo.decimal,
										})}
									</div>
								</div>
							</div>
