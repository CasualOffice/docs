...
export const PasteSpecialDialog: React.FC<PasteSpecialDialogProps> = ({
	...
	const [clipboardContent, setClipboardContent] =
		useState<ParsedClipboardContent | null>(null);
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			switch (e.key) {
				...
				case "ArrowUp":
					...
					setSelectedIndex(
						(prev) =>
							(prev - 1 + PASTE_OPTION_DEFS.length) % PASTE_OPTION_DEFS.length,
					);
					break;
				case "Enter":
					e.preventDefault();
	...
	const getDialogStyle = useCallback((): React.CSSProperties => {
		...
		let y =
			position?.y ??
			(typeof window !== "undefined"
				? window.innerHeight / 2 - dialogHeight / 2
				: 100);
 ...
 */
export function usePasteSpecial(
	options: UsePasteSpecialOptions = {},
