...
export function TranslateDialog({
	...
	const [replaceStatus, setReplaceStatus] = useState<"idle" | "running">(
		"idle",
	);
				...
				<div style={bodyStyle}>
					...
					<div style={langRowStyle}>
						...
						<div style={sideStyle}>
							...
							{status === "loading" && (
								<PanelState kind="loading" message="Translating…" />
							)}
							{status === "error" && (
								<PanelState
									kind="error"
					...
					{onReplace && (
						<button
							...
							style={{
								...
								color: "white",
								opacity:
									status === "success" && replaceStatus === "idle" ? 1 : 0.6,
								cursor:
									status === "success" && replaceStatus === "idle"
										? "pointer"
										: "not-allowed",
							}}
