...
import { Slice, type Fragment as PMFragment } from "prosemirror-model";
import type { EditorView } from "prosemirror-view";
import { PanelState } from "../ui/PanelState";
import {
	translateFragment,
	TRANSLATE_LANGUAGES as LANGUAGES,
} from "../../lib/translate";
import {
	translateDocViaMarkdown,
	type ChunkTranslationState,
} from "../../lib/translateMarkdown";
...
export function TranslateDocumentDialog({
	...
	const [translatedBuffer, setTranslatedBuffer] = useState<ArrayBuffer | null>(
		null,
	);
	...
	const [liveChunkState, setLiveChunkState] =
		useState<ChunkTranslationState | null>(null);
	...
	useEffect(() => {
		...
		void (async () => {
			try {
				...
				if (controller.signal.aborted) return;
				const buf = await captureTranslatedBuffer(
					getView,
					onSave,
					translatedContent,
				);
			...
			>
				...
				<div style={bodyStyle}>
					...
					<div style={paneStyle}>
						...
						>
							...
							{previewStatus === "ready" &&
								translatedBuffer &&
								renderPreview(translatedBuffer)}
						</div>
					</div>
				</div>
				...
				<div style={footerStyle}>
					<span style={hintStyle}>
						Your open document is unchanged — only the downloaded copy is
						translated.
					</span>
					<button
						type="button"
...
function LiveTranslationPreview({
	...
	return (
		<div style={liveWrapStyle} data-testid="translate-doc-live-preview">
			...
			{state.inProgress && (
				<div
					style={liveInProgressBlockStyle}
					data-testid="translate-doc-live-inprogress"
				>
					<span style={liveSpinnerStyle} aria-hidden="true" />
