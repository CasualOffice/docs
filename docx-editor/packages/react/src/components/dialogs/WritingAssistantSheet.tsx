	...
	return (
		<>
			...
			>
				<div style={bodyStyle}>
					<p style={introStyle}>
						Runs entirely in your browser. Your document is never sent to a
						server.
					</p>
...
function FeatureRow({
	...
	return (
		...
		>
			<span
				style={checked ? toggleTrackOnStyle : toggleTrackStyle}
				aria-hidden="true"
			>
			...
			<span style={featureLabelStyle}>
				...
				{checked && state.phase === "downloading" && (
					<span
						style={subtleStyle}
						data-testid={`writer-progress-${feature.id}`}
					>
						Downloading… {Math.round(state.progress * 100)}%
					</span>
				)}
				{checked && state.phase === "loading" && (
					<span style={subtleStyle}>Loading model…</span>
				)}
			</span>
		</label>
	);
			...
			{caps && (
				...
				>
					...
					<li>
						...
						<span style={subtleStyle}>
							{caps.wasmSimd ? "available" : "unavailable"}
						</span>
					</li>
					{caps.deviceMemoryGb !== null && (
						<li>
							· Device memory{" "}
							<span style={subtleStyle}>{caps.deviceMemoryGb} GB reported</span>
						</li>
					)}
					{caps.storageQuotaMb !== null && caps.storageUsedMb !== null && (
						<li>
							· Browser storage{" "}
							<span style={subtleStyle}>
								{Math.round(caps.storageUsedMb)} MB used of{" "}
								{Math.round(caps.storageQuotaMb)} MB
							</span>
						</li>
					)}
					<li>
		...
		>
			<div style={consentDialogStyle} data-testid="writer-consent-dialog">
				...
				<p style={{ fontSize: 13, lineHeight: 1.5, marginTop: 10 }}>
					This downloads a ~95 MB model to your browser cache so the assistant
					can run on your device. Your document is never sent to a server.
				</p>
				<p
					style={{
						fontSize: 12,
