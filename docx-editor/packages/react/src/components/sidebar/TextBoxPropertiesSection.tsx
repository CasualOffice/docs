...
const presetBtn = (active: boolean): CSSProperties => ({
	...
	border: active
		? "1px solid var(--doc-primary, #1a73e8)"
		: "1px solid var(--doc-border, #dadce0)",
});
...
export function TextBoxPropertiesSection({
	...
	const [px, setPx] = useState(
		posOffsetH != null ? String(Math.round(posOffsetH)) : "",
	);
	const [py, setPy] = useState(
		posOffsetV != null ? String(Math.round(posOffsetV)) : "",
	);
	...
	useEffect(
		() => setPx(posOffsetH != null ? String(Math.round(posOffsetH)) : ""),
		[posOffsetH],
	);
	useEffect(
		() => setPy(posOffsetV != null ? String(Math.round(posOffsetV)) : ""),
		[posOffsetV],
	);
