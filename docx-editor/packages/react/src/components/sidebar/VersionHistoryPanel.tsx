		...
		>
          ...
          optional "Save version…" action only NAMES the current one. */}
			<div style={TAB_CAPTION_STYLE} data-testid="version-history-caption">
				Versions save automatically as you edit — on save and about every 10
				minutes. Open one to preview or restore it.
			</div>
			<VersionsTab
				docId={docId}
...
function VersionsTab({
	...
	if (!docId) {
		return (
			<PanelState
				kind="empty"
				message="Open a document to see its version history."
			/>
		);
	}
					...
					{groups.map(({ label, items }) => (
						<li key={label} style={GROUP_STYLE}>
							...
							<ol style={LIST_STYLE} role="list">
								{items.map((snap) => {
									const previousSnap =
										snap.id != null ? previousById.get(snap.id) : undefined;
									const canPreview =
										!!onPreviewVersion &&
										snap.serverVersion == null &&
										snap.data != null;
									return (
										<VersionRow
											key={snap.id}
											...
											onPreview={
												canPreview
													? () => handlePreview(snap, previousSnap)
													: undefined
											}
											onRename={
												snap.serverVersion != null
...
function VersionRow({
	...
	const menuItems: KebabItem[] = [
		...
		...(onRename
			? [{ icon: "edit_note", label: "Rename", onClick: onRename }]
			: []),
		...(onDelete
			? [{ icon: "delete", label: "Delete", onClick: onDelete, danger: true }]
			: []),
	];
		...
		>
            ...
            the right); only named versions surface their user-given name. */}
				<span style={AUTHOR_STYLE}>
					{snap.kind === "manual" ? snap.name : "Auto-saved"}
				</span>
				{snap.kind === "manual" && (
			...
			{(snap.author || (typeof snap.size === "number" && snap.size > 0)) && (
				<div style={SIZE_STYLE}>
					{snap.author && (
						<span data-testid="version-history-author">{snap.author}</span>
					)}
					{snap.author &&
			...
			{stats && (stats.added > 0 || stats.removed > 0) && (
				<div
					...
					data-testid="version-history-version-stats"
				>
					{stats.added > 0 && (
						<span style={STAT_ADD_STYLE}>+{stats.added}</span>
					)}
					{stats.added > 0 && stats.removed > 0 && <span>·</span>}
					{stats.removed > 0 && (
						<span style={STAT_REMOVE_STYLE}>-{stats.removed}</span>
					)}
...
function CurrentVersionRow({
	...
	return (
		...
		>
			...
			{active && (
				<MaterialSymbol name="check" size={16} style={{ marginLeft: "auto" }} />
			)}
		</button>
	);
}
...
function RowKebabMenu({ items }: { items: KebabItem[] }) {
	...
	const { containerRef, dropdownRef, dropdownStyle, handleMouseDown } =
		useFixedDropdown({
			isOpen: open,
	...
	return (
		<span ref={containerRef} style={{ display: "inline-flex" }}>
			...
			{open && (
				<div
					ref={dropdownRef}
					style={{ ...dropdownStyle, ...KEBAB_MENU_STYLE }}
					role="menu"
				>
					{items.map((item) => (
						<button
