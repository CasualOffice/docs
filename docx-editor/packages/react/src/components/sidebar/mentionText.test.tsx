...
describe("renderCommentText", () => {
	test("returns plain text when there are no @-mentions", () => {
		expect(flatten(renderCommentText("hello world", ["Alice"]))).toBe(
			"hello world",
		);
	});
	...
	test("chips a mention mid-text after whitespace", () => {
		expect(
			flatten(renderCommentText("hi @Bob can you check", ["Alice", "Bob"])),
		).toBe("hi [Bob] can you check");
	});

	test("leaves unknown @-handles as plain text", () => {
		expect(flatten(renderCommentText("hi @nobody", ["Alice"]))).toBe(
			"hi @nobody",
		);
	});
	...
	test("email addresses do not chip (email guard)", () => {
		expect(
			flatten(renderCommentText("mail alice@example.com", ["example"])),
		).toBe("mail alice@example.com");
	});

	test("matches longer names before shorter ones (Jane Doe vs Jane)", () => {
		expect(
			flatten(renderCommentText("cc @Jane Doe today", ["Jane", "Jane Doe"])),
		).toBe("cc [Jane Doe] today");
	});

	test('respects word boundary — @Janeway with author "Jane" stays plain', () => {
		expect(flatten(renderCommentText("hi @Janeway", ["Jane"]))).toBe(
			"hi @Janeway",
		);
	});
	...
	test("case-insensitive matching", () => {
		expect(flatten(renderCommentText("hi @ALICE", ["alice"]))).toBe(
			"hi [alice]",
		);
	});
});
