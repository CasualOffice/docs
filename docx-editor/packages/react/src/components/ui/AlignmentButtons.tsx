...
export function AlignmentButtons({
	...
	return (
		...
		>
			{!isOpen ? (
				<Tooltip content={ariaText}>{triggerButton}</Tooltip>
			) : (
				triggerButton
			)}
				...
				>
					<div style={{ display: "flex", gap: 2 }}>
						{ALIGNMENT_OPTIONS.map((option) => {
							...
							const optShortcut = option.shortcutKey
								? t(option.shortcutKey)
								: undefined;
							return (
								<button
									key={option.value}
...
export function getAlignmentIcon(value: ParagraphAlignment): ReactNode {
	const option = ALIGNMENT_OPTIONS.find((opt) => opt.value === value);
	return (
		option?.icon || <MaterialSymbol name="format_align_left" size={ICON_SIZE} />
	);
}
 ...
 */
export function getAlignmentShortcut(
	value: ParagraphAlignment,
): string | undefined {
	const option = ALIGNMENT_OPTIONS.find((opt) => opt.value === value);
