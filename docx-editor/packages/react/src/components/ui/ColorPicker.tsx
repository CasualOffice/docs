...
const S_CELL: CSSProperties = {
	...
	padding: 0,
	transition:
		"transform var(--doc-anim-fast), border-color var(--doc-anim-fast)",
};
...
export function ColorPicker({
	...
	const matrix = useMemo(
		() => generateThemeTintShadeMatrix(colorScheme),
		[colorScheme],
	);
	...
	const handleStandardColorSelect = useCallback(
		(hex: string) => {
			const picked: ColorValue | string =
				mode === "highlight" ? hex : { rgb: hex };
			setPickedColor(picked);
			onChange?.(picked);
			setIsOpen(false);
		},
					...
					>
						...
						<div
							style={{
								...S_COLOR_BAR,
								backgroundColor:
									swatchColor === "transparent" ? "#fff" : swatchColor,
								outline:
					...
					>
						...
						<div
							style={{
								...S_COLOR_BAR,
								backgroundColor:
									resolvedColor === "transparent" ? "#fff" : resolvedColor,
								outline:
						...
						<div style={S_CUSTOM_ROW}>
							...
							<button
								...
								style={{
									...
									cursor: /^[0-9A-Fa-f]{6}$/.test(customHex)
										? "pointer"
										: "default",
								}}
