...
export function HorizontalRuler({
	...
	const effectiveFirstLineIndent = hangingIndent
		? -firstLineIndent
		: firstLineIndent;
		...
		(e: MouseEvent) => {
			...
			if (anchor.marker === "leftMargin") {
				...
				const rounded = Math.round(
					Math.max(
						0,
						Math.min(anchor.startLeftMarginTwips + dxTwips, maxMargin),
					),
				);
				...
				const rounded = Math.round(
					Math.max(
						0,
						Math.min(anchor.startRightMarginTwips - dxTwips, maxMargin),
					),
				);
				setDragValue(rounded);
				...
				setDragValue(rounded);
				setDragPositionPx(
					leftMarginPx + indentLeftPx + twipsToPixels(rounded) * zoom,
				);
				...
				const rounded = Math.round(
					Math.max(
						0,
						Math.min(anchor.startLeftIndentTwips + dxTwips, maxIndent),
					),
				);
				setDragValue(rounded);
				...
				const rounded = Math.round(
					Math.max(
						0,
						Math.min(anchor.startRightIndentTwips - dxTwips, maxIndent),
					),
				);
				setDragValue(rounded);
				setDragPositionPx(
					pageWidthPx - rightMarginPx - twipsToPixels(rounded) * zoom,
				);
			...
			<div
				...
				onMouseDown={
					editable && onLeftMarginChange
						? (e) => handleDragStart(e, "leftMargin")
						: undefined
				}
			/>
			<div
				...
				onMouseDown={
					editable && onRightMarginChange
						? (e) => handleDragStart(e, "rightMargin")
						: undefined
				}
			/>
		...
		for (let i = 0; i <= totalEighths; i++) {
			...
			if (i % 8 === 0) {
				ticks.push({
					position: pxPos,
					height: 10,
					label: i / 8 > 0 ? String(i / 8) : undefined,
				});
			} else if (i % 4 === 0) {
		...
		for (let i = 0; i <= totalMm; i++) {
			...
			if (i % 10 === 0) {
				ticks.push({
					position: pxPos,
					height: 10,
					label: i / 10 > 0 ? String(i / 10) : undefined,
				});
			} else if (i % 5 === 0) {
				ticks.push({ position: pxPos, height: 6 });
			} else {
