...
const SIZE_CONFIG: Record<
	LoadingSize,
	{ size: number; strokeWidth: number; fontSize: number }
> = {
...
const DotsVariant: React.FC<{ size: number; color: string }> = ({
	size,
	color,
}) => {
...
const BarVariant: React.FC<{ size: number; color: string }> = ({
	size,
	color,
}) => {
	return (
		...
		>
			<div
				style={{
					...
					borderRadius: size / 8,
					animation:
						"docx-loading-bar 2.1s cubic-bezier(0.65, 0.815, 0.735, 0.395) infinite",
				}}
			/>
		</div>
	);
...
const PulseVariant: React.FC<{ size: number; color: string }> = ({
	size,
	color,
}) => {
	return (
