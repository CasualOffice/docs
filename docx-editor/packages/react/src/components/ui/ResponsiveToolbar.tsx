...
export function useResponsiveToolbar(
	...
	const calculateVisibleItems = useCallback(() => {
		...
		for (const item of sortedItems) {
			const itemWidth =
				item.minWidth ||
				itemWidthsRef.current.get(item.id) ||
				DEFAULT_ITEM_MIN_WIDTH;
			const widthWithGap = itemWidth + (count > 0 ? itemGap : 0);

			// Reserve space for overflow button if not all items will fit
			const reservedWidth =
				count < items.length - 1 ? overflowButtonWidth + itemGap : 0;
...
export const ResponsiveToolbar: React.FC<ResponsiveToolbarProps> = ({
	...
	const showOverflow =
		hasOverflow || alwaysShowOverflow || additionalOverflowItems.length > 0;
	...
	const defaultOverflowButton = (
		<button
			...
			style={{
				...
				border: "none",
				borderRadius: "4px",
				backgroundColor: isOverflowOpen
		...
		>
			...
			{showOverflow && (
				<div style={{ position: "relative", flexShrink: 0 }}>
					...
					{renderOverflowMenu ? (
						isOverflowOpen &&
						renderOverflowMenu(allOverflowItems, closeOverflow)
					) : (
						<OverflowMenu
