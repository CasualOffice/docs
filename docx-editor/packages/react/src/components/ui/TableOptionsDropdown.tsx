...
const MENU_ITEMS: MenuItem[] = [
	...
	{
		action: "borderOutside",
		labelKey: "table.borders.outside",
		icon: "border_outer",
	},
	{
		action: "borderInside",
		labelKey: "table.borders.inside",
		icon: "border_inner",
	},
	{
		action: "borderNone",
		labelKey: "table.borders.remove",
		icon: "border_clear",
	},
					...
					return (
						<Tooltip key={opt.value} content={label}>
							<button
								...
								onClick={() =>
									onAction({ type: "cellVerticalAlign", align: opt.value })
								}
							>
		...
		<div>
			<button
				...
				style={{
					...menuItemStyles,
					backgroundColor:
						hoveredItem === "main" ? "var(--doc-bg-hover)" : "transparent",
				}}
					...
					>
						{(["top", "bottom", "left", "right"] as const).map((side) => (
							...
							>
								...
								/>
								<span style={{ fontSize: 10, color: "var(--doc-text-muted)" }}>
									tw
								</span>
							</label>
						))}
...
function RowHeightRow({
	...
	const [heightRule, setHeightRule] = useState<"auto" | "atLeast" | "exact">(
		"atLeast",
	);
				...
				>
					...
					>
						...
						<select
							value={heightRule}
							onChange={(e) =>
								setHeightRule(e.target.value as typeof heightRule)
							}
							style={{
								flex: 1,
								padding: "2px 4px",
		...
		<>
			...
			<div style={alignmentRowStyles}>
				...
				{makeButton(
					"left",
					"format_align_left",
					t("tableAdvanced.alignTableLeft"),
				)}
				{makeButton(
					"center",
					"format_align_center",
					t("tableAdvanced.alignTableCenter"),
				)}
				{makeButton(
					"right",
					"format_align_right",
					t("tableAdvanced.alignTableRight"),
				)}
			</div>
		</>
	);
...
export function TableOptionsDropdown({
	...
	const currentJustification =
		(tableContext?.table?.attrs?.justification as
			| "left"
			| "center"
			| "right"
			| null
	...
	return (
		...
		>
			{resolvedTooltip && !isOpen ? (
				<Tooltip content={resolvedTooltip}>{button}</Tooltip>
			) : (
				button
			)}
