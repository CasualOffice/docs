...
export function DeleteTableIcon(): React.ReactElement {
	return (
		<MaterialSymbol name="delete" size={ICON_SIZE} className="text-red-600" />
	);
}
...
const TOOLBAR_STYLES: Record<string, CSSProperties> = {
	...
	button: {
		...
		lineHeight: "1",
		transition:
			"background-color var(--doc-anim-base), color var(--doc-anim-base)",
	},
	buttonCompact: {
		padding: "3px 5px",
	},
...
export function TableToolbarButton({
	...
	const isDeleteAction =
		typeof action === "string" && action.startsWith("delete");
			...
			if (cell.formatting?.vMerge !== "continue") {
				...
				if (cell.formatting?.vMerge === "restart") {
					for (
						...
						const continuation = getRowCellStartingAt(
							table.rows[nextRow],
							colIndex,
						);
						if (!continuation || continuation.formatting?.vMerge !== "continue")
							break;
						rowspan += 1;
					}
				}
...
function toAnchorCellFormatting(
	cell: TableCell,
	colspan: number,
	rowspan: number,
) {
	...
	for (let row = 0; row < layout.newRowCount; row++) {
		...
		for (let col = 0; col < newColCount; ) {
			...
			if (anchor) {
				cells.push({
					...anchor.data,
					formatting: toAnchorCellFormatting(
						anchor.data,
						anchor.colspan,
						anchor.rowspan,
					),
			...
			cells.push({
				...coveringAnchor.data,
				content: [],
				formatting: toContinuationFormatting(
					coveringAnchor.data,
					coveringAnchor.colspan,
				),
			});
		...
		newRows.push({
			type: "tableRow",
			formatting: sourceRow?.formatting
				? { ...sourceRow.formatting }
				: undefined,
			cells,
		});
	}
...
export function addColumn(
	...
	if (table.columnWidths && table.columnWidths.length > 0) {
		...
		const templateWidth =
			table.columnWidths[atIndex] || table.columnWidths[0] || 1440; // Default 1 inch
