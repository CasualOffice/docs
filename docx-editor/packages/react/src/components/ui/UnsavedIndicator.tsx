...
const getIndicatorStyles = (
	...
	const baseStyles: React.CSSProperties = {
		...
		animation:
			hasUnsavedChanges && showPulse
				? "docx-unsaved-pulse 2s ease-in-out infinite"
				: "none",
	};
...
export const UnsavedIndicator: React.FC<UnsavedIndicatorProps> = ({
	...
	return (
		<span
			...
			aria-label={
				hasUnsavedChanges
					? t("unsaved.unsavedAriaLabel")
					: t("unsaved.savedAriaLabel")
			}
		>
			{renderContent()}
		</span>
...
export function useUnsavedChanges(
	...
	const [lastSavedDocument, setLastSavedDocument] = useState<Document | null>(
		null,
	);
	...
	const serializeDocument = useCallback(
		(doc: Document | null | undefined): string | null => {
			if (!doc) return null;
			try {
	...
	const resetTracking = useCallback(
		(newDocument?: Document | null) => {
			const docToUse =
				newDocument !== undefined ? newDocument : currentDocument;
			setHasUnsavedChanges(false);
			setChangeCount(0);
