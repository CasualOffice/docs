...
export interface EmbedHostHandlers {
	...
	) => Promise<LoadResponseData> | LoadResponseData;
	/** Editor requests a save. Host persists, returns the response. */
	onSaveRequest?: (
		data: SaveRequestData,
	) => Promise<SaveResponseData> | SaveResponseData;
	...
	private async dispatch(env: CasualEnvelope): Promise<void> {
		switch (env.type) {
			...
			case "casual.load.request": {
				...
				const id = env.id ?? "";
				try {
					const resp = await this.handlers.onLoadRequest(
						env.data as LoadRequestData,
					);
			...
			case "casual.save.request": {
				...
				const id = env.id ?? "";
				try {
					const resp = await this.handlers.onSaveRequest(
						env.data as SaveRequestData,
					);
			...
			case "casual.signature.field.signed":
				this.handlers.onSignatureFieldSigned?.(
					env.data as SignatureFieldSignedData,
				);
				return;
