...
describe("isCasualEnvelope", () => {
	...
	it("rejects foreign types", () => {
		expect(
			isCasualEnvelope({ type: "other.hello", app: "docs", v: 1, data: {} }),
		).toBe(false);
	});
	it("rejects bad app values", () => {
		expect(
			isCasualEnvelope({ type: "casual.hello", app: "pdf", v: 1, data: {} }),
		).toBe(false);
	});
	it("rejects non-v1", () => {
		expect(
			isCasualEnvelope({ type: "casual.hello", app: "docs", v: 2, data: {} }),
		).toBe(false);
	});
...
describe("EmbedTransport", () => {
	...
	it("sendHello emits an editor.hello envelope", () => {
		...
		expect((env.data as { capabilities: string[] }).capabilities).toEqual([
			"save",
			"load",
		]);
	});
