...
export class EmbedTransport {
	private readonly opts: Required<
		Omit<EmbedTransportOptions, "parentWindow" | "hostWindow">
	> & {
	...
	constructor(opts: EmbedTransportOptions) {
		this.opts = {
			...
			parentWindow:
				...
				(typeof window !== "undefined"
					? (window.parent as unknown as NonNullable<
							EmbedTransportOptions["parentWindow"]
						>)
		...
		this.opts.hostWindow.addEventListener(
			"message",
			this.boundMessage as EventListener,
		);
	}
	...
	async requestSave(
		...
		return this.request<SaveResponseData>(
			"casual.save.request",
			req,
			timeoutMs,
			[req.bytes],
	...
	destroy(): void {
		...
		this.opts.hostWindow.removeEventListener(
			"message",
			this.boundMessage as EventListener,
		);
	...
	private async dispatch(env: CasualEnvelope): Promise<void> {
		...
		switch (env.type) {
			...
			case "casual.command.setReadOnly":
				await this.handlers.onCommandSetReadOnly?.(
					env.data as CommandSetReadOnlyData,
				);
				return;
			case "casual.command.setTheme":
				await this.handlers.onCommandSetTheme?.(
					env.data as CommandSetThemeData,
				);
				return;
			case "casual.command.setLocale":
				await this.handlers.onCommandSetLocale?.(
					env.data as CommandSetLocaleData,
				);
				return;
			case "casual.command.set.viewmode":
				await this.handlers.onCommandSetViewMode?.(
					env.data as CommandSetViewModeData,
				);
				return;
			...
			case "casual.signature.cancel":
				await this.handlers.onSignatureCancel?.(
					env.data as SignatureCancelData,
				);
				return;
			default:
			...
			const timer =
				timeoutMs > 0
					? setTimeout(() => {
							...
							reject(
								new Error(
									`Embed request ${type} timed out after ${timeoutMs}ms`,
								),
							);
						}, timeoutMs)
					: null;
