			...
			footer={
				<button
					...
					style={primaryButtonStyle(
						submitting || loading || !username || !password,
					)}
				>
					{submitting
						? mode === "login"
				...
				{mode === "login" && (
					<div style={forgotWrapStyle}>
						...
						{forgotOpen && (
							<div
								role="note"
								data-testid="personal-auth-forgot-panel"
								style={forgotPanelStyle}
							>
								Mode 3 doesn’t do email recovery — there’s no SMTP server on a
								single-node deploy. Ask the operator to ssh into the container
								and run:
