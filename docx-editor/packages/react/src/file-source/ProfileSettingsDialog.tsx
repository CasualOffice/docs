...
export function ProfileSettingsDialog({
	...
	const [saveError, setSaveError] = useState<PersonalFileSourceError | null>(
		null,
	);
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			...
			const basePrefs =
				load.status === "loaded" ? load.profile.preferences : {};
		...
		>
			...
			{load.status === "error" && (
				<div data-testid={`${testId}-load-error`} style={errorStyle}>
					{load.error?.message ??
						"Could not load your profile. Try closing and reopening."}
				</div>
			)}
			{load.status === "loaded" && (
				<form
				...
				>
					...
					<label style={labelStyle}>
						...
						<span style={hintStyle}>
							IANA tz string (leave blank to use the browser default)
						</span>
					</label>
					<label style={labelStyle}>
						...
						/>
						<span style={hintStyle}>
							BCP-47 tag (leave blank to match the browser)
						</span>
					</label>
					{saveError && (
