...
describe("PersonalFileSource", () => {
	...
	it("save() with id=null multipart-POSTs to /files and returns the minted id", async () => {
		...
		h.setRespond(() =>
			jsonRes(201, {
				file: file({ id: "doc_new", name: "Draft.docx", etag: "v1" }),
			}),
		);

		const result = await h.source.save(
			null,
		...
		expect(
			(h.calls[0].init?.headers as Record<string, string> | undefined)?.[
				"Content-Type"
			],
		).toBeUndefined();
	...
	it("save() with an existing id POSTs raw bytes to /files/:id with If-Match", async () => {
		const h = makeHarness();
		h.setRespond(() =>
			jsonRes(200, { file: file({ id: "doc_a", etag: "v5" }) }),
		);
	...
	it("rename() PATCHes { name } (204) and updates the recent observer", async () => {
		const h = makeHarness();
		h.setRespond(() =>
			jsonRes(200, { files: [file({ id: "doc_a", name: "old.docx" })] }),
		);
		await h.source.list();
	...
	it("delete() DELETEs and removes the entry from the observer", async () => {
		...
		h.setRespond(() =>
			jsonRes(200, {
				files: [
					file({ id: "doc_a", name: "a.docx" }),
					file({ id: "doc_b", name: "b.docx" }),
				],
			}),
		);
		await h.source.list();
	...
	it("getProfile() GETs /auth/profile and unwraps { profile }", async () => {
		...
		expect((profile.preferences as { showRulers: boolean })?.showRulers).toBe(
			true,
		);
	});
