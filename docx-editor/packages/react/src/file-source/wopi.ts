...
export class WopiFileSource implements FileSource {
	...
	async open(
		...
		// editor's context.
		if (id !== this.docId) {
			throw new WopiNotSupportedError(
				`open(${id}) — only the embed's docId is reachable`,
			);
		}
		const res = await this.fetchImpl(
			this.urlFor(`/wopi/files/${encodeURIComponent(id)}/contents`),
		);
		if (!res.ok) {
	...
	async save(
		...
		if (id !== null && id !== this.docId) {
			throw new WopiNotSupportedError(
				`save(${id}) — only the embed's docId is writable`,
			);
		}
		...
		const next =
			body.version ??
			res.headers.get("X-WOPI-ItemVersion") ??
			this.version ??
			"";
	...
	private async checkFileInfo(): Promise<CheckFileInfo | null> {
		const res = await this.fetchImpl(
			this.urlFor(`/wopi/files/${encodeURIComponent(this.docId)}`),
		);
