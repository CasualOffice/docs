	...
	for (const mark of node.marks) {
		...
		switch (mark.type.name) {
			case "fontSize": {
				...
				if (Number.isFinite(halfPt) && halfPt > 0)
					styles.push(`font-size:${halfPt / 2}pt`);
				break;
			}
			case "fontFamily": {
...
function renderMarkedText(node: ProseMirrorNode): string {
	...
	if (inlineStyle)
		out = `<span style="${escapeAttr(inlineStyle)}">${out}</span>`;
	return out;
}
...
function renderInlineNode(node: ProseMirrorNode): string {
	...
	if (node.type.name === "image") {
		...
		return src
			? `<img src="${escapeAttr(src)}" alt="" style="max-width:100%" />`
			: "";
	}
...
function renderInlineFragment(fragment: Fragment): string {
	...
	for (let i = 0; i < fragment.childCount; i++)
		out += renderInlineNode(fragment.child(i));
	return out;
}
