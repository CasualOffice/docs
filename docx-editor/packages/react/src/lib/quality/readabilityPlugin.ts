...
function buildDecorations(state: EditorState, enabled: boolean): DecorationSet {
	...
	state.doc.descendants((node, pos) => {
		...
		while ((match = re.exec(text)) != null) {
			...
			const trimmedStart =
				cursor +
				(sentenceText.length - sentenceText.replace(/^\s+/, "").length);
			sentenceRanges.push({
				from: trimmedStart,
				to: end,
		...
		if (cursor < text.length) {
			...
			if (tail.trim()) {
				const trimmedStart =
					cursor + (tail.length - tail.replace(/^\s+/, "").length);
				sentenceRanges.push({
					from: trimmedStart,
					to: text.length,
		...
		for (const s of sentenceRanges) {
			...
			const style =
				s.words > VERY_LONG_SENTENCE_WORDS ? VERY_LONG_STYLE : LONG_STYLE;
			const title =
 ...
 * caller dispatches.
 */
export function readabilityPluginSetEnabled(enabled: boolean): {
	[k: string]: unknown;
} {
	...
	return new Plugin<ReadabilityPluginState>({
		...
		state: {
			...
			apply(tr, value, _oldState, newState) {
				const meta = tr.getMeta(READABILITY_PLUGIN_KEY) as
					| { enabled?: boolean }
					| undefined;
				const nextEnabled =
					meta?.enabled != null ? meta.enabled : value.enabled;
