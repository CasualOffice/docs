...
function paragraphHeadingLevel(para: PMNode): number | null {
	...
	const size = Number(
		(fontSizeMark?.attrs as { size?: number } | undefined)?.size ?? 0,
	);
	if (size >= 36) return 1;
	if (size >= 30) return 2;
	if (size >= 24) return 3;
...
export function docToMarkdown(doc: PMNode): string {
	...
	doc.forEach((node) => {
		...
		if (node.type.name === "heading") {
			...
			parts.push(
				`${"#".repeat(Math.max(1, Math.min(6, level)))} ${node.textContent}`,
			);
			return;
		}
...
export async function translateDocViaMarkdown(
	...
	for (let i = 0; i < chunks.length; i++) {
		...
		const translated = await translateMarkdownChunkViaLlm(
			chunk,
			source,
			target,
			signal,
