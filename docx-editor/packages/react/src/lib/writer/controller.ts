...
function ensureWorker(): Worker {
	...
	worker.onmessage = (e: MessageEvent<WriterRes>) =>
 ...
 */
export function featureSupport(feature: FeatureSpec): {
	supported: boolean;
	reason?: string;
} {
...
export function isLlmReady(): boolean {
	...
	return (
		/-MLC$/i.test(state.loadedModelId) ||
		/^(Llama|Qwen|SmolLM|Phi)/i.test(state.loadedModelId)
	);
}
