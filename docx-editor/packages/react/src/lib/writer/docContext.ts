...
export function inferDocKind(text: string, headings: string[]): DocKind {
	...
	const has = (re: RegExp) =>
		hl.some((h) => re.test(h)) || re.test(lc.slice(0, 800));
...
export function summariseDocStructure(opts: DocContextOptions): string {
	...
	const headingNames = counts.headings
		.map((h) => h.text.trim())
		.filter(Boolean);
	...
	if (words === 0) {
		...
		if (counts.bullets > 0)
			decorationParts.push(
				`${counts.bullets} bullet point${counts.bullets === 1 ? "" : "s"}`,
			);
		if (counts.numbered > 0)
			decorationParts.push(
				`${counts.numbered} numbered item${counts.numbered === 1 ? "" : "s"}`,
			);
		if (counts.tables > 0)
			decorationParts.push(
	...
	if (sel.length > 0) {
		...
		const preview =
			sel.length > SELECTION_HINT_CAP
				? `${sel.slice(0, SELECTION_HINT_CAP)}…`
				: sel;
		parts.push(
