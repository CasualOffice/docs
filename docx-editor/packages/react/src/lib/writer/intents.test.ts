...
describe("intent quickClassify", () => {
	it('routes "create a table about programming languages" → insertTable', async () => {
		const out = await classifyIntent(
			"create a table about programming languages",
			{
				hasSelection: false,
			},
		);
	...
	it('routes "make a 5x3 table of stocks" → insertTable with rows/cols', async () => {
		const out = await classifyIntent("make a 5 by 3 table of stocks", {
			hasSelection: false,
		});
		expect(out.intent).toBe("insertTable");
	...
	it('routes "tabular data about cities" → insertTable', async () => {
		const out = await classifyIntent("tabular data about cities", {
			hasSelection: false,
		});
		expect(out.intent).toBe("insertTable");
	});
	...
	it('routes "Summarize this document" → summarize', async () => {
		const out = await classifyIntent("Summarize this document", {
			hasSelection: false,
		});
	...
	it('routes "find typos" → findIssues', async () => {
		const out = await classifyIntent("find typos in this paragraph", {
			hasSelection: true,
		});
	...
	it('routes "rewrite this concisely" with selection → rewrite + tone', async () => {
		const out = await classifyIntent("rewrite this concisely", {
			hasSelection: true,
		});
	...
	it('routes "polish this for readability" with selection → rewrite + readable', async () => {
		const out = await classifyIntent("polish this to be more readable", {
			hasSelection: true,
		});
		expect(out.intent).toBe("rewrite");
	...
	it('routes "rewrite this in plain English" with selection → rewrite + plain', async () => {
		const out = await classifyIntent("rewrite this in plain English", {
			hasSelection: true,
		});
		expect(out.intent).toBe("rewrite");
	...
	it('routes "/translate Spanish" → translate', async () => {
		const out = await classifyIntent("/translate Spanish", {
			hasSelection: true,
		});
	...
	it("defaults uncategorised messages to chat when LLM is offline", async () => {
		const out = await classifyIntent("Hey, what time is it?", {
			hasSelection: false,
		});
	...
	it('routes "create a resume from this" → transformDoc', async () => {
		const out = await classifyIntent("create a resume from this", {
			hasSelection: false,
		});
	...
	it('routes plain "make a resume" → transformDoc, not outline', async () => {
		const out = await classifyIntent("make me a resume", {
			hasSelection: false,
		});
		expect(out.intent).toBe("transformDoc");
	});
	...
	it('routes "look up Hemingway editor" → research', async () => {
		const out = await classifyIntent("look up Hemingway editor", {
			hasSelection: false,
		});
		expect(out.intent).toBe("research");
	...
	it('routes "define MLA format" → research', async () => {
		const out = await classifyIntent("define MLA format", {
			hasSelection: false,
		});
		expect(out.intent).toBe("research");
	...
	it('routes "create a cover letter from this" → transformDoc/cover-letter', async () => {
		const out = await classifyIntent("create a cover letter from this", {
			hasSelection: false,
		});
		expect(out.intent).toBe("transformDoc");
	...
	it('routes "draft a memo from this" → transformDoc/memo', async () => {
		const out = await classifyIntent("draft a memo from this", {
			hasSelection: false,
		});
		expect(out.intent).toBe("transformDoc");
	...
	it('routes "rewrite this as a blog post" → transformDoc/blog', async () => {
		const out = await classifyIntent("rewrite this as a blog post", {
			hasSelection: false,
		});
		expect(out.intent).toBe("transformDoc");
	...
	it('routes "turn this into a memorandum" → transformDoc/memo', async () => {
		const out = await classifyIntent("turn this into a memorandum", {
			hasSelection: false,
		});
		expect(out.intent).toBe("transformDoc");
		expect(out.transformTarget).toBe("memo");
	...
	it('still routes "outline a memo about Q3 goals" → outline, not transformDoc', async () => {
		...
		// restructured.
		const out = await classifyIntent("outline a memo about Q3 goals", {
			hasSelection: false,
		});
		expect(out.intent).toBe("outline");
	});
	...
	it('routes "format this as an academic paper" → transformDoc/academic', async () => {
		const out = await classifyIntent("format this as an academic paper", {
			hasSelection: false,
		});
		expect(out.intent).toBe("transformDoc");
	...
	it('routes "rewrite as a research paper in MLA style" → academic + MLA instruction', async () => {
		const out = await classifyIntent(
			"rewrite this as a research paper in MLA style",
			{
				hasSelection: false,
			},
		);
	...
	it('routes "make this into a Chicago-style academic paper" → academic + Chicago', async () => {
		const out = await classifyIntent(
			"make this into a Chicago-style academic paper",
			{
				hasSelection: false,
			},
		);
	...
	it('routes "create a slide deck from this" → transformDoc/slide-deck', async () => {
		const out = await classifyIntent("create a slide deck from this", {
			hasSelection: false,
		});
		expect(out.intent).toBe("transformDoc");
	...
	it('routes "turn this into a Pecha Kucha deck" → slide-deck + Pecha Kucha instruction', async () => {
		const out = await classifyIntent("turn this into a Pecha Kucha deck", {
			hasSelection: false,
		});
		expect(out.intent).toBe("transformDoc");
		expect(out.transformTarget).toBe("slide-deck");
