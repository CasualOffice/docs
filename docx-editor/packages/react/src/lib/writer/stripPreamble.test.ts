...
describe("stripModelPreamble", () => {
	...
	it('removes "Here is the rewrite:" variant', () => {
		expect(stripModelPreamble("Here is the rewrite: hello world.")).toBe(
			"hello world.",
		);
	});
	...
	it('removes "Here\'s a summary:" variant', () => {
		expect(
			stripModelPreamble("Here's a summary: bullet one. bullet two."),
		).toBe("bullet one. bullet two.");
	});

	it('removes "Sure! Here is" variant', () => {
		expect(
			stripModelPreamble("Sure! Here is your rewrite: actual content."),
		).toBe("actual content.");
	});
	...
	it('removes "Of course! Here\'s" variant', () => {
		expect(
			stripModelPreamble(
				"Of course! Here's the rewritten passage:\n\nReal text.",
			),
		).toBe("Real text.");
	});
	...
	it("strips wrapping quotes after preamble", () => {
		expect(stripModelPreamble('Here is the rewrite: "actual content"')).toBe(
			"actual content",
		);
	});

	it("leaves text without preamble unchanged", () => {
		expect(stripModelPreamble("Just a plain reply.")).toBe(
			"Just a plain reply.",
		);
	});
	...
	it("returns original when stripping would empty the text", () => {
		...
		expect(stripModelPreamble("Here is the rewrite:")).toBe(
			"Here is the rewrite:",
		);
	});
