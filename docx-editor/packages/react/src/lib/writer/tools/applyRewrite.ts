...
export const applyRewriteTool: Tool<RewriteArgs> = {
	name: "rewrite",
	description:
		"Rewrite the selection in a chosen tone and apply as a tracked change.",
	async execute(args, ctx): Promise<ToolResult> {
		const selection = ctx.getSelectionText().trim();
		if (!selection) {
			return {
				kind: "error",
				message: "Select the text you want to rewrite first.",
			};
		}
		...
		const toneHint =
			(args.tone && TONE_HINTS[args.tone.toLowerCase()]) || TONE_HINTS.polish;
...
Return ONLY a JSON object: {"rewrite": "<the rewritten text, plain prose, no markdown>"}.`;
		...
		try {
			...
			return {
				kind: "error",
				message: `Rewrite failed — ${(err as Error).message}`,
			};
		}
