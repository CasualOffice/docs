...
export const insertOutlineTool: Tool<OutlineArgs> = {
	name: "outline",
	description:
		"Insert a structured outline (memo, essay, report) into the document.",
	async execute(args, ctx): Promise<ToolResult> {
		...
		try {
			...
			return {
				kind: "error",
				message: `Couldn't draft the outline — ${(err as Error).message}`,
			};
		}
		...
		if (fragment.childCount === 0) {
			return {
				kind: "error",
				message: "Model returned an outline that produced no PM nodes.",
			};
		}
		return {
			kind: "proposal",
