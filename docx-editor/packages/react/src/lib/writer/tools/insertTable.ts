...
function buildTableNode(
	...
	const tableRows: PMNode[] = [
		rowType.create(
			{
				height: DEFAULT_ROW_HEIGHT_TWIPS,
				heightRule: "atLeast",
				isHeader: true,
			},
			headerCells,
	...
	for (const row of rows) {
		...
		tableRows.push(
			rowType.create(
				{ height: DEFAULT_ROW_HEIGHT_TWIPS, heightRule: "atLeast" },
				cells,
			),
		);
	}
...
export const insertTableTool: Tool<InsertTableArgs> = {
	name: "insertTable",
	description:
		"Generate a table of data for a topic and insert it at the cursor.",
	async execute(args, ctx): Promise<ToolResult> {
		...
		const semanticTableValidator: Validator<TableJson> = (o) => {
			...
			heads.forEach((h, i) => {
				...
				if (/^column\s*\d+$/i.test(h) || /^col\s*\d+$/i.test(h)) {
					issues.push({
						...
						text: h,
						reason:
							'is a generic "Column N" placeholder — give it a meaningful name',
					});
				}
			});
			const rows = o.rows ?? [];
			if (rows.length === 0) {
				issues.push({
					field: "rows",
					text: "",
					reason: "is empty — give at least one data row",
				});
			}
			return issues;
		};
		...
		try {
			table = await runJsonChatWithValidation<TableJson>(
				[
					{
						role: "system",
						content: buildSystemPrompt(rowsTarget, colsTarget),
					},
		...
		if (!tableNode) {
			return {
				kind: "error",
				message: "Editor schema is missing table nodes.",
			};
		}
