	...
	it("flags a bullet that does not start with an action verb", () => {
		const out = resumeSemanticValidator({
			...
			experience: [
				{
					role: "Staff Engineer",
					bullets: ["Was responsible for billing pipeline."],
				},
			],
		});
	...
	it("passes a bullet that does start with an action verb", () => {
		const out = resumeSemanticValidator({
			...
			experience: [
				{
					role: "Staff Engineer",
					bullets: ["Shipped a 3x throughput rewrite."],
				},
			],
		});
...
describe("coverLetterSemanticValidator", () => {
	...
	it("flags a body paragraph shorter than 40 chars", () => {
		...
		expect(
			out.some((i) => i.field === "body[0]" && /short/.test(i.reason)),
		).toBe(true);
	});
	...
	it("flags clichés in the opening", () => {
		...
		expect(
			out.some((i) => i.field === "opening" && /cliché/.test(i.reason)),
		).toBe(true);
	});
});
...
describe("memoSemanticValidator", () => {
	...
	it("flags subjects ending in terminal punctuation", () => {
		...
		expect(
			out.some(
				(i) => i.field === "subject" && /terminal punctuation/.test(i.reason),
			),
		).toBe(true);
	});
	...
	it("flags subjects longer than 80 chars", () => {
		...
		expect(out.some((i) => i.field === "subject" && /80/.test(i.reason))).toBe(
			true,
		);
	});
...
const cleanBlog: BlogJson = {
	...
	intro:
		"Five engineers, two databases, one weekend window — here is what actually mattered.",
	sections: [
		{ heading: "The bet", body: "We moved 50M rows of billing data live." },
		{
			heading: "The rollback we never used",
			body: "A backup branch sat warm for 72 hours.",
		},
	],
};
...
describe("blogSemanticValidator", () => {
	...
	it("flags filler-cliché intros", () => {
		const out = blogSemanticValidator({
			...cleanBlog,
			intro:
				"In today's fast-paced world, let's dive in to what really matters.",
		});
...
const cleanAcademic: AcademicJson = {
	...
	sections: [
		{
			heading: "Methods",
			body: "We instrumented 12 weeks of production traffic.",
		},
		{
			heading: "Results",
			body: "Latency degraded gracefully past the design ceiling.",
		},
	],
	references: [
		{ author: "Smith, J.", title: "Event sourcing patterns", year: "2024" },
	],
};
...
describe("academicSemanticValidator", () => {
	...
	it("flags non-4-digit years in references", () => {
		const out = academicSemanticValidator({
			...cleanAcademic,
			references: [
				{ author: "Smith, J.", title: "Stuff", year: "twenty twenty-four" },
			],
		});
