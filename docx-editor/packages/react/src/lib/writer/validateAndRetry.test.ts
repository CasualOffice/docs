...
describe("noPlaceholderValidator", () => {
	it("passes a clean resume", () => {
		const out = {
			...
			experience: [
				{ role: "Engineer", company: "Acme", dates: "Mar 2019 – Dec 2021" },
			],
		};
	...
	it("catches [Insert here] nested inside an array", () => {
		const out = {
			sections: [{ heading: "Goals", body: "See [Insert here] for details." }],
		};
		const issues = noPlaceholderValidator(out);
		expect(issues.length).toBe(1);
	...
	it("walks deeply nested structures and reports the full path", () => {
		const out = {
			experience: [
				{
					role: "Eng",
					bullets: ["Real bullet.", "Started at [Company Name]."],
				},
			],
		};
		const issues = noPlaceholderValidator(out);
...
describe("combineValidators", () => {
	it("concatenates issues from every validator", () => {
		const a: Validator<unknown> = () => [
			{ field: "x", text: "a", reason: "first" },
		];
		const b: Validator<unknown> = () => [
			{ field: "y", text: "b", reason: "second" },
		];
