		...
		if (responseFormat) {
			const reply = await loaded.llm.chat.completions.create({
				messages: messages as Parameters<
					typeof loaded.llm.chat.completions.create
				>[0]["messages"],
				temperature,
...
async function evict(m: LoadedModel): Promise<void> {
	if (m.engine === "web-llm" && m.llm) {
		...
		if (
			typeof (m.llm as unknown as { unload?: () => Promise<void> }).unload ===
			"function"
		) {
...
function classifyError(
	msg: string,
): WriterRes extends infer R
	? R extends { kind: "error"; code: infer C }
		? C
		: never
	: never {
