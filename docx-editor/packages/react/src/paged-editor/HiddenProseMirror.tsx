...
function assertScrollFlagShape(emptyTr: Transaction): void {
	...
	try {
		...
		if (
			typeof probe.updated !== "number" ||
			(probe.updated & PM_UPDATED_SCROLL) === 0
		) {
			console.warn(
...
function createInitialState(
	...
	const doc = document
		? toProseDoc(document, { styles: styles ?? undefined })
		: createEmptyDoc();
...
>(function HiddenProseMirror(props, ref) {
	...
	const portalTarget =
		browserDoc && "body" in browserDoc && browserDoc.body != null
			? browserDoc.body
			: null;
