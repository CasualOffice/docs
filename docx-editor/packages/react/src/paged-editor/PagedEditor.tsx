...
function runAfterPaint(fn: () => void, signal: AbortSignal): void {
	...
	const id1 = requestAnimationFrame(() => {
		...
		const id2 = requestAnimationFrame(() => {
			...
			signal.addEventListener("abort", () => cancelAnimationFrame(id3), {
				once: true,
			});
		});
		signal.addEventListener("abort", () => cancelAnimationFrame(id2), {
			once: true,
		});
	});
	signal.addEventListener("abort", () => cancelAnimationFrame(id1), {
		once: true,
	});
}
 ...
 */
function findPaintedPmStartAtOrBefore(
	pages: HTMLElement,
	pmPos: number,
): number | null {
 ...
 */
function columnWidthForSection(
	config: SectionLayoutConfig,
	colIndex = 0,
): number {
	const contentWidth =
		config.pageSize.w - config.margins.left - config.margins.right;
...
export function computeExtendedTopMargin(
	...
	const clearTop = Math.max(
		0,
		headerDistance + headerContentHeight + headerOverlapPull,
	);
...
export function measureTableBlock(
	...
	if (tableBlock.rows.length > 0) {
		columnWidths = normalizeTableColumnWidths(
			columnWidths,
			colCount,
			targetWidth,
		);
	...
	for (let rowIdx = 0; rowIdx < rows.length; rowIdx++) {
		...
		for (let cellIdx = 0; cellIdx < row.cells.length; cellIdx++) {
			...
			for (let blockIdx = 0; blockIdx < cell.blocks.length; blockIdx++) {
				...
				contentHeight += measureTableCellBlockVisualHeight(
					sourceBlock,
					blockMeasure,
				);
			}
...
function ensureParagraphCacheCapacity(paragraphCount: number): void {
	const target = Math.min(
		PARAGRAPH_CACHE_MAX,
		Math.max(PARAGRAPH_CACHE_MIN, paragraphCount * 2),
	);
...
function isWithinSidebar(target: EventTarget | null): boolean {
	...
	return !!(
		el?.closest(".docx-comments-sidebar") ||
		el?.closest(".docx-unified-sidebar")
	);
}
...
const PagedEditorComponent = forwardRef<PagedEditorRef, PagedEditorProps>(
	function PagedEditor(props, ref) {
		...
		const [tableChipPos, setTableChipPos] = useState<{
			x: number;
			y: number;
		} | null>(null);
		...
		const runLayoutPipeline = useCallback(
			(state: EditorState) => {
				...
				const applyPendingIncrementalScrollSnapshot = (
					onlyIfSnapshotJustWritten: boolean,
				) => {
					const pend = pendingScrollRestoreRef.current;
					if (
						pend?.renderKind !== "incremental" ||
						pend.scrollTopSnapshot == null
					)
						return;
					if (onlyIfSnapshotJustWritten) {
						const age =
							performance.now() -
							pendingIncrementalScrollSnapshotWrittenAtRef.current;
						if (age > 32) return;
					}
					const pe0 = pagesContainerRef.current;
					const sp0 = pe0
						? (getScrollContainer() ?? findVerticalScrollParentOrRoot(pe0))
						: null;
				...
				try {
					...
					if (extendHeader || extendFooter) {
						const extend = (m: PageMargins): PageMargins => {
							...
							if (extendHeader) {
								...
								out.top = computeExtendedTopMargin(
									m.top,
									headerDistance,
									headerContentHeight,
								);
					...
					if (pagesContainerRef.current && painterRef.current) {
						...
						const scrollParent =
							getScrollContainer() ?? findVerticalScrollParentOrRoot(pagesEl);
						...
						const docBgColor =
							currentDoc?.package.document.background?.color?.rgb ?? undefined;
						...
						const pageBackground = docBgColor
							? `#${docBgColor}`
							: "var(--doc-page-paper, #fff)";
						...
						const renderPagesKind = renderPages(
							newLayout.pages,
						...
						if (scrollParent?.isConnected) {
							...
							const scrollTopSnapshot =
								renderPagesKind === "incremental" || freeze
							...
							if (
								renderPagesKind === "incremental" &&
								scrollTopSnapshot != null
							) {
								pendingIncrementalScrollSnapshotWrittenAtRef.current =
									performance.now();
							}
						...
						if (stepTime > 500) {
							console.warn(
								`[PagedEditor] renderPages took ${Math.round(stepTime)}ms`,
							);
						}
					...
					if (
						...
						(window as unknown as PerfWindow).__layoutPerf
					) {
		...
		useLayoutEffect(() => {
			...
			const scrollParent =
				getScrollContainer() ??
				(pagesEl ? findVerticalScrollParentOrRoot(pagesEl) : null);
		...
		const updateSelectionOverlay = useCallback(
			(state: EditorState) => {
				...
				// in-table test; the DOM lookup only locates the painted box.
				{
					...
					if (pagesEl && viewportEl) {
						...
						if (inTable) {
							...
							const cells = pagesEl.querySelectorAll(
								".layout-table-cell[data-pm-start]",
							);
							...
							const tableEl = bestCell?.closest(
								".layout-table",
							) as HTMLElement | null;
							if (tableEl) {
				...
				// [data-pm-start, data-pm-end) range contains the caret.
				{
					...
					if (pagesEl && viewportEl) {
						const boxes = pagesEl.querySelectorAll(
							".layout-textbox[data-pm-start]",
						);
				...
				raf = requestAnimationFrame(() => {
					...
					setSelectedImageInfo((prev) => {
						...
						const holder = root.querySelector(
							`[data-pm-start="${prev.pmPos}"]`,
						);
						const live = (
							holder?.tagName === "IMG"
								? holder
								: (holder?.querySelector("img") ?? holder)
						) as HTMLElement | null;
		...
		const scrollToPositionImpl = useCallback(
			(pmPos: number, forParaIdScroll = false) => {
				...
				if (!forParaIdScroll) {
					...
					const pageShells =
						pages.querySelectorAll<HTMLElement>(".layout-page");
				...
				const scroller =
					getScrollContainer() ?? findVerticalScrollParentOrRoot(pages);
		...
		const scrollToPageImpl = useCallback(
			(pageNumber: number): void => {
				...
				const shell =
					pagesContainerRef.current?.querySelectorAll<HTMLElement>(
						".layout-page",
					)[pageNumber - 1];
			...
			(paraId: string): boolean => {
				...
				const inner =
					targetNode?.isTextblock === true
						? Math.min(
								startPos + 1 + targetNode.content.size,
		...
		const handlePagesMouseDown = useCallback(
			(e: React.MouseEvent) => {
				...
				if (target.classList.contains("layout-table-resize-handle")) {
					...
					resizeTablePmStartRef.current = parseInt(
				...
				if (
					...
					resizeRowTablePmStartRef.current = parseInt(
						target.dataset.tablePmStart ?? "0",
						10,
					);
				...
				if (target.classList.contains("layout-table-edge-handle-right")) {
					...
					resizeRightEdgePmStartRef.current = parseInt(
						target.dataset.tablePmStart ?? "0",
						10,
					);
		...
		const handlePagesContextMenu = useCallback(
			(e: React.MouseEvent) => {
				...
				const readImageNodeAt = (pos: number): ImageInfo | null => {
					...
					const wrapType =
						(node.attrs.wrapType as WrapType | undefined) ?? "inline";
				...
				if (hit) {
					...
					if (imageInfo) {
						imageInfo.inlinePositionEmu = captureInlinePositionEmu(
							hit.imageEl,
							zoom,
						);
					}
				...
				let spellcheckInfo: { from: number; to: number; word: string } | null =
					null;
				if (pmPos !== null) {
		...
		const handleEditorViewReady = useCallback(
			(view: EditorView) => {
				...
				const imeHost = view.dom.closest(
				...
				const syncImeCaret = () => {
					...
					const caretEl = view.dom.ownerDocument.querySelector(
						'[data-testid="caret"]',
					);
					if (!caretEl) return;
		...
		const totalHeight = useMemo(() => {
			...
			const pagesHeight = layout.pages.reduce(
				(sum, page) => sum + page.size.h,
				0,
			);
			...
			>
              ...
              endnote area). Double-click an entry to edit it. */}
					{document?.package?.endnotes &&
						document.package.endnotes.length > 0 && (
							<EndnoteSection
					...
					{onResizeTextBox &&
						...
						(() => {
							const box = textBoxResize
								? {
										...textBoxChipPos,
										width: textBoxResize.w,
										height: textBoxResize.h,
									}
								: textBoxChipPos;
							const startResize = (
								corner: "nw" | "ne" | "se" | "sw",
							...
							const HS = 9;
							const corners: Array<
								["nw" | "ne" | "se" | "sw", number, number, string]
							> = [
