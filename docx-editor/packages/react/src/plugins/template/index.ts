...
function selectTag(view: EditorView | null, tags: TemplateTag[], id: string) {
	...
	if (tag) {
		const tr = view.state.tr.setSelection(
			TextSelection.near(view.state.doc.resolve(tag.from)),
		);
		view.dispatch(tr);
		view.focus();
	}
...
export function createPlugin(
	...
	return {
		...
		getSidebarItems: (
			...
			const visibleTags = state.tags.filter(
				(t) => t.type !== "sectionEnd" && !t.insideSection,
			);
			...
			return visibleTags.map((tag) => ({
				...
				render: (props) =>
					React.createElement(TemplateChip, {
						...
						onSelect: (id: string) =>
							selectTag(context.editorView, state.tags, id),
					}),
			}));
		},
