...
export function createTemplatePlugin(): Plugin<TemplatePluginState> {
	return new Plugin<TemplatePluginState>({
		...
		props: {
			decorations(state) {
				return (
					templatePluginKey.getState(state)?.decorations ?? DecorationSet.empty
				);
			},
			...
			handleClick(view: EditorView, pos: number) {
				...
				if (clicked) {
					view.dispatch(
						view.state.tr.setMeta(templatePluginKey, {
				...
				if (current) {
					view.dispatch(
						view.state.tr.setMeta(templatePluginKey, { selectedId: undefined }),
					);
				}
				return false;
			},
			...
			handleDOMEvents: {
				mouseover(view: EditorView, event) {
					...
					if (id !== current) {
						view.dispatch(
							view.state.tr.setMeta(templatePluginKey, { hoveredId: id }),
						);
					}
					return false;
				},
