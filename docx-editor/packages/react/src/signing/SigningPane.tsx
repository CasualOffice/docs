...
export function SigningPane({
	...
	const active: SignatureField | null =
		snapshot.activeFieldIndex >= 0
			? snapshot.fields[snapshot.activeFieldIndex]
			: null;
	...
	return (
		...
		>
			...
			{!active && snapshot.canComplete && (
				<div
					style={completeBlockStyle}
					data-testid={`${testId}-complete-block`}
				>
	...
	return (
		<div style={editorStyle} data-testid={`${testId}-editor`}>
			<div style={editorHeaderStyle}>
				...
				{field.signer?.name && (
					<div style={editorSignerStyle}>{field.signer.name}</div>
				)}
			</div>
			{field.methods.length > 1 && (
				<div
					style={methodTabsStyle}
					role="tablist"
			...
			<div style={captureWrapStyle}>
				{method === "drawn" && (
					<DrawnSignaturePad onCapture={(c) => onCapture(c, "drawn")} />
				)}
				{method === "typed" && (
					<TypedSignatureField
...
function listItemStyle(active: boolean, signed: boolean): CSSProperties {
	return {
		...
		borderRadius: 6,
		background: active
			? "var(--doc-surface-2, #f1f5f9)"
			: signed
				? "transparent"
				: "transparent",
		border: active
			? "1px solid var(--doc-border, #cbd5e1)"
