...
describe("buildVersionDiffDoc", () => {
	...
	test("identical docs → no changes", () => {
		...
		const { hasChanges } = buildVersionDiffDoc(
			a,
			docJSON("hello world"),
			schema,
		);
	...
	test("author is stamped on the marks", () => {
		const { doc } = buildVersionDiffDoc(docJSON("a"), docJSON("a b"), schema, {
			author: "Alice",
		});
		let found = false;
		doc.descendants((node) => {
			for (const m of node.marks) {
				if (m.type.name === "insertion" && m.attrs.author === "Alice")
					found = true;
			}
		});
