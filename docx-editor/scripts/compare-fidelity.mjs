...
const DEFAULT_OUT = new URL("../fidelity-compare-report.md", import.meta.url)
	.pathname;
...
const ONLYOFFICE_IMAGE =
	process.env.ONLYOFFICE_DOCBUILDER_IMAGE ??
	"onlyoffice/documentbuilder:latest";
...
async function roundTripOnlyOffice(inputPath, workDir) {
	...
	if (res.status !== 0) {
		throw new Error(
			`docbuilder exit ${res.status}: ${res.stderr || res.stdout}`,
		);
	}
...
async function runFixture(fixturePath, name, engines, workRoot) {
	...
	const totalIn = [...countTags(originalXml).values()].reduce(
		(a, b) => a + b,
		0,
	);
...
function aggregate(rows, engines) {
	const totals = Object.fromEntries(
		engines.map((e) => [
			e,
			{ fixtures: 0, errors: 0, pristine: 0, totalIn: 0, dropped: 0 },
		]),
	);
...
function renderReport(rows, engines, totals, skipped) {
	...
	lines.push(
		'consolidating, e.g.), which shows up as a "drop" even though the',
	);
...
(async () => {
	...
	if (FIXTURE_FILTER) {
		fixtures = fixtures.filter((f) => FIXTURE_FILTER.has(basename(f, ".docx")));
	}
