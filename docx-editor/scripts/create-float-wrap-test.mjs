...
function buildDocumentXml() {
	...
	sections.push(
		para(
			"Comprehensive test for issues #143 and #188. Tests all wrap types, wrap text modes, position modes, and table cell scenarios.",
		),
	);
	...
	// ══════════════════════════════════════════════════════════════════════════
	sections.push(
		heading("Section 4: Floating Images in Table Cells (Issue #188)", 1),
	);
	sections.push(
		para(
			"Each cell contains a floating image with different wrap settings. Text in the cell should wrap around the image.",
		),
	);
  ...
  <wp:anchor distT="${distLarge}" distB="${distLarge}" distL="${distLarge}" distR="${distLarge}"
	...
	sections.push(
		para(
			"This paragraph should still wrap around the tall image above if it vertically overlaps.",
		),
	);
...
async function main() {
	...
	console.log("\nSections:");
	console.log(
		"  1. Wrap Types (page level): square, tight, through, topAndBottom, none",
	);
	console.log(
		"  2. Position Modes: align left/right/center, relative-to margin",
	);
	console.log("  3. Multiple Floating Images: side-by-side, stacked");
	console.log("  4. Table Cells: all wrap types in cells");
	console.log(
		"  5. Edge Cases: large/small images, large distances, empty paragraphs",
	);
	console.log("\nDone.");
}
