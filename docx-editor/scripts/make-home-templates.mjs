...
function rPr(opts = {}) {
	...
	if (opts.font)
		parts.push(`<w:rFonts w:ascii="${opts.font}" w:hAnsi="${opts.font}"/>`);
...
async function writeDocx(outPath, body) {
	...
	console.log(
		`  ${outPath.replace(projectRoot + "/", "")} (${buf.byteLength} bytes)`,
	);
}
...
const SAMPLE_BODY = [
	...
	h("What you can do", 2),
	bullet([
		r("Rich text — bold, italic, underline, strikethrough, colors, highlights"),
	]),
	...
	p(
		[
			r("Click anywhere and start typing — this entire document is editable.", {
				italic: true,
				color: "64748B",
			}),
		],
...
const RESUME_BODY = [
	p([r("Alex Morgan", { bold: true, sz: 56, color: "0F172A" })], {
		align: "center",
	}),
	p(
		[
			r(
				"Software Engineer · alex@example.com · (555) 123-4567 · linkedin.com/in/alexmorgan",
				{ color: "64748B" },
			),
		],
		{ align: "center" },
	...
	bullet([
		r(
			"Led the migration from REST to GraphQL across 12 services; reduced p99 latency by 40%.",
		),
	]),
	bullet([r("Built an offline-first sync layer used by 200k+ daily users.")]),
	bullet([
		r("Mentored two junior engineers through their first production launches."),
	]),
	p([r("")]),
	p([r("Software Engineer · BluePeak", { bold: true })]),
	p([
		r("Mar 2019 – Dec 2021 · San Francisco, CA", {
			color: "64748B",
			italic: true,
		}),
	]),
	bullet([
		r(
			"Owned the document-sharing pipeline; took the on-call burn-down from 12 pages/week to under 2.",
		),
	]),
	bullet([
		r(
			"Shipped the customer-facing audit-log feature requested by enterprise tier customers.",
		),
	]),
	h("Education", 2),
	...
	p([
		r(
			"TypeScript · React · Go · PostgreSQL · Redis · AWS · GitHub Actions · OpenTelemetry",
		),
	]),
].join("\n");
...
const MEETING_BODY = [
	...
	p([
		r("Attendees: ", { bold: true }),
		r("Alex Morgan, Priya Patel, Jordan Lee, Sam Rivera"),
	]),
	p([r("Apologies: ", { bold: true }), r("Casey Kim (PTO)")]),
	h("Agenda", 2),
...
const PROPOSAL_BODY = [
	p([r("Project Proposal", { bold: true, sz: 56, color: "0F172A" })], {
		align: "center",
	}),
	p(
		[
			r("Real-Time Document Collaboration Service", {
				color: "475569",
				sz: 28,
			}),
		],
	...
	bullet([
		r(
			"Ship a single-user .docx editor with end-to-end round-trip fidelity ≥ 90%",
		),
	]),
	bullet([
		r(
			"Support two-or-more concurrent editors per document with sub-second sync",
		),
	]),
	bullet([
		r("Integrate with at least one WOPI-compatible host (Nextcloud first)"),
	]),
	...
	table([
		...
		[
			"M1 · Yjs round-trip",
			"Apr 2026",
			"Two browsers edit one document locally",
		],
		[
			"M2 · Go gateway v0",
			"May 2026",
			"Self-contained Docker image with upload + share link",
		],
		[
			"M3 · WOPI integration",
			"Jul 2026",
			"Real host (Nextcloud) reads and writes documents",
		],
		[
			"M4 · Public preview",
			"Aug 2026",
			"Stable release, fidelity score above 90%",
		],
	]),
	p([r("")]),
	h("Open questions", 2),
	...
	bullet([
		r(
			"Tauri desktop build: ship alongside the web build, or separate cadence?",
		),
	]),
...
const TRAVEL_BODY = [
	p(
		[r("Tokyo + Kyoto — Spring 2026", { bold: true, sz: 52, color: "0F172A" })],
		{
			align: "center",
		},
	),
	p(
		[r("May 30 – June 8 · two travellers", { color: "64748B", italic: true })],
		{
			align: "center",
		},
	),
...
const RECIPE_BODY = [
	p(
		[
			r("Brown-Butter Chocolate Chip Cookies", {
				bold: true,
				sz: 48,
				color: "7C2D12",
			}),
	...
	p(
		[
			r("Makes 18 cookies · 35 min active · rest 2 h preferred", {
				italic: true,
				color: "64748B",
			}),
		],
	...
	numbered([
		r("Sift in flour, baking soda, and salt. Fold gently until just combined."),
	]),
	numbered([
		r(
			"Fold in the chocolate. Cover and rest in the fridge for at least 2 hours (overnight is better).",
		),
	]),
	numbered([
		r(
			"Heat oven to 180 °C / 350 °F. Scoop 45 g balls onto a lined sheet, well spaced.",
		),
	]),
	numbered([
		r(
			"Bake 11–12 min — edges set, centres still soft. Finish with flaky salt.",
		),
	]),
...
const MEMO_BODY = [
	p([r("MEMORANDUM", { bold: true, sz: 36, color: "0F172A" })], {
		align: "center",
	}),
	p([r("")]),
	table([
	...
	bullet([
		r("Open questions / pushback: reply on this thread or grab time directly"),
	]),
].join("\n");
...
const STATUS_BODY = [
	...
	p([
		r("Alex Morgan · Engineering · sent Friday EOD", {
			color: "64748B",
			italic: true,
		}),
	]),
	...
	bullet([
		r("Word-compat closing-border heuristic behind off-by-default flag (#395)"),
	]),
	h("In flight", 2),
	bullet([r("Go gateway scaffolding — WS frame parser landing Monday")]),
	bullet([
		r("Real Sample / Resume / Letter / Meeting / Proposal templates authored"),
	]),
	bullet([
		r("Demo Pages deploy now points at doc.schnsrw.live (DNS propagated)"),
	]),
	h("Blockers", 2),
	bullet([
		r(
	...
	h("Next week", 2),
	bullet([
		r("Land Go gateway v0 — accept WS, hold one Y.Doc per room, drop on idle"),
	]),
	bullet([
		r("Stand up the WOPI mock as a Docker service alongside the editor"),
	]),
...
const PRESS_BODY = [
	...
	p(
		[
			r("Casual Editor Launches Real-Time .docx Collaboration in the Browser", {
				bold: true,
				sz: 36,
			}),
		],
	...
	p([
		r(
			"“We wanted to prove that real-time collaboration doesn’t have to mean handing over your files,” said Alex Morgan, the project lead. ",
			{ italic: true },
		),
		r("“The host owns the file. We just orchestrate the editing.”", {
			italic: true,
		}),
	]),
	h("Availability", 2),
...
const ESSAY_BODY = [
	p(
		[
			r("The Quiet Revolution in Collaborative Editing", {
				bold: true,
				sz: 44,
				color: "0F172A",
			}),
	...
	p(
		[
			r("Alex Morgan · State University · ENG 301", {
				color: "64748B",
				italic: true,
			}),
		],
	...
	p([
		r(
			"Shapiro, M., et al. (2011). Conflict-free Replicated Data Types. INRIA RR-7506.",
			{ color: "475569" },
		),
	]),
	p([
		r("Jahns, K. (2019). Yjs — A CRDT Framework. https://yjs.dev", {
			color: "475569",
		}),
	]),
].join("\n");
...
const LAB_BODY = [
	p(
		[
			r("Lab Report: Pendulum Period vs. Length", {
				bold: true,
				sz: 40,
				color: "0F172A",
			}),
	...
	p([r("PHYS 201 · Section B · Bench 4", { color: "64748B", italic: true })], {
		align: "center",
	}),
	p(
		[
			r("Author: Alex Morgan · Partner: Jordan Lee · Date: March 5, 2026", {
				color: "64748B",
			}),
		],
		{
	...
	numbered([
		r(
			"Release bob from 10° displacement; measure time for 20 complete swings.",
		),
	]),
	...
	bullet([
		r("Stopwatch reaction time (~0.2 s; mitigated by 20-swing averaging)"),
	]),
	bullet([
		r(
			"Release-angle inconsistency (small-angle approximation breaks above ~15°)",
		),
	]),
...
const SYLLABUS_BODY = [
	...
	p(
		[
			r("Spring 2026 · Dr. Priya Patel · MW 13:00–14:20 · Engineering 312", {
				color: "64748B",
			}),
		],
		{
	...
	bullet([
		r(
			"Reason about clocks, ordering, and consistency in the presence of partial failure.",
		),
	]),
	...
	bullet([
		r(
			"Design and critique production-grade distributed-systems architectures.",
		),
	]),
	h("Schedule", 2),
	table([
	...
	p([
		...
		r(
			"Up to 48 hours late at 75% credit, beyond that 0. Two no-questions extensions per semester.",
		),
	]),
	p([
