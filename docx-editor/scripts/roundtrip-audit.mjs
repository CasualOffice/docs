...
const REPORT_PATH = new URL("../roundtrip-audit-report.md", import.meta.url)
	.pathname;
...
for (const fixture of fixtures) {
	...
	const row = { name, error: null, dropped: [] };
	try {
		...
		for (const [tag, count] of inCounts) {
			...
			if (after < count) {
				...
				if (vanished) {
					const entry = globalDiff.get(tag) ?? {
						fixtures: new Set(),
						totalDelta: 0,
					};
		...
		dropped.sort(
			(a, b) => Number(b.vanished) - Number(a.vanished) || b.delta - a.delta,
		);
		row.dropped = dropped;
	} catch (e) {
...
for (const r of rollup.slice(0, 10)) {
	console.log(
		`  ${r.tag.padEnd(28)} -${r.totalDelta} (in ${r.fixtures} fixtures)`,
	);
}
