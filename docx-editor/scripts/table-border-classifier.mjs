...
async function classifyDocx(buf) {
	...
	for (const { outer } of iterTables(docXml)) {
		tableIdx++;
		const tblPr =
			(outer.match(/<w:tblPr>([\s\S]*?)<\/w:tblPr>/) ?? [])[1] ?? "";
		...
		if (styleId && stylesXml) {
			...
			if (sm) {
				const styleBody = sm[1];
				const styleTblPr =
					(styleBody.match(/<w:tblPr>([\s\S]*?)<\/w:tblPr>/) ?? [])[1] ?? "";
				const styleTblBordersInner = tblBordersInside(styleTblPr);
				styleBordersBottom = summarizeBorderSide(
					styleTblBordersInner,
					"bottom",
				);
				styleBordersInsideH = summarizeBorderSide(
					styleTblBordersInner,
					"insideH",
				);
				const firstRowBody = findTblStylePr(stylesXml, styleId, "firstRow");
				if (firstRowBody) {
					const fbBordersInner =
						(firstRowBody.match(/<w:tcBorders>([\s\S]*?)<\/w:tcBorders>/) ??
							[])[1] ?? "";
					styleFirstRowBottom = summarizeBorderSide(fbBordersInner, "bottom");
				}
				const lastRowBody = findTblStylePr(stylesXml, styleId, "lastRow");
				if (lastRowBody) {
					const lbBordersInner =
						(lastRowBody.match(/<w:tcBorders>([\s\S]*?)<\/w:tcBorders>/) ??
							[])[1] ?? "";
		...
		for (const cellMatch of lastRow.match(
			/<w:tcBorders(?:\s[^>]*)?(?:\/>|>[\s\S]*?<\/w:tcBorders>)/g,
		) ?? []) {
			if (/^<w:tcBorders\s*\/>$/.test(cellMatch)) {
				lastRowHasEmptyTcBorders = true;
				lastRowCellBottoms.push("<empty />");
			} else {
				const inner = cellMatch.replace(
					/^<w:tcBorders[^>]*>|<\/w:tcBorders>$/g,
					"",
				);
...
async function main() {
	...
	console.log(
		`${files.length} fixtures, ${all.length} tables total, ${suspects.length} #395-suspect.`,
	);
}
